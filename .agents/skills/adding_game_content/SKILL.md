...
## Critical Rules
- **NEVER hand-edit auto-generated files**: `include/pm.h` (generated by `makedefs -p`), `include/onames.h` (`makedefs -o`), `include/vis_tab.h` and `src/vis_tab.c` (`makedefs -z`), `include/animoff.h` and `include/animtotals.h` (`makedefs -a`).
...
- **Always rebuild `makedefs`** after changes: Rebuild `pm.h` (`makedefs -p`) and `onames.h` (`makedefs -o`) before compiling the native library. See the `build_pipeline` skill for the complete `makedefs` flag reference.
...
- **`build_pipeline`** — Complete `makedefs` flag reference table, full build process, two-solution architecture
