...
%token	<i> CORRIDOR_ID GOLD_ID ENGRAVING_ID FOUNTAIN_ID THRONE_ID MAGIC_PORTAL_ID MODRON_PORTAL_ID LEVEL_TELEPORTER_ID LEVEL_TELEPORT_DIRECTION_TYPE LEVEL_TELEPORT_END_TYPE POOL_ID SINK_ID NONE
...
%token	<i> ALIGNMENT LEFT_OR_RIGHT CENTER TOP_OR_BOT ALTAR_TYPE ALTAR_SUBTYPE UP_OR_DOWN ACTIVE_OR_INACTIVE MAGIC_PORTAL_TARGET_TYPE_ID MAGIC_PORTAL_TARGET_TYPE SEEN_OR_UNSEEN
...
%token	<i> FEMALE_ID MALE_ID WAITFORU_ID PROTECTOR_ID CANCELLED_ID REVIVED_ID AVENGE_ID FLEEING_ID BLINDED_ID MAXHP_ID QUEST_COMPANION_ID LEVEL_ADJUSTMENT_ID KEEP_ORIGINAL_INVENTORY_ID
...
magic_portal_detail : MAGIC_PORTAL_ID ':' coord_or_var ',' MAGIC_PORTAL_TARGET_TYPE ',' SEEN_OR_UNSEEN ',' ACTIVE_OR_INACTIVE
		  {
		      add_opvars(splev, "iiio", VA_PASS4((int) $<i>5, (int) $<i>7, (int) $<i>9, SPO_MAGIC_PORTAL));
		  }
		;
...
lever_info	: ACTIVE_OR_INACTIVE
		...
		| MAGIC_PORTAL_TARGET_TYPE_ID ':' MAGIC_PORTAL_TARGET_TYPE
		  {
		      add_opvars(splev, "ii", VA_PASS2((int)$3, SP_L_V_PORTAL_TYPE));
		      $<i>$ = 0x2000;
		  }
		;
