...
static const int ascii_case_difference = 'a' - 'A';
static const char
    *Dont_Edit_Code =
...
do_date()
{
    ...
    if (get_gitinfo(githash, gitbranch)) {
        Fprintf(ofp, "#define GnollHack_GIT_SHA \"%s\"\n", githash);
        Fprintf(ofp, "#define GnollHack_GIT_BRANCH \"%s\"\n", gitbranch);
    }
#ifdef AMIGA
    {
        ...
        Fprintf(ofp, "\"\\0$VER: GnollHack %d.%d.%d (%d.%d.%d)\"\n",
...
#endif
#endif
    /* pattern matching method will be substituted by GnollHack at run time */
...
do_permonst()
{
    ...
    for (i = 0; mons[i].mlet; i++) {
        ...
        for (nam = c = tmpdup(mons[i].mname); *c; c++)
            if (*c >= 'a' && *c <= 'z')
                *c = (char)((int)*c - ascii_case_difference);
            else if (*c < 'A' || *c > 'Z')
                *c = '_';
...
static char *limit(name, pref) /* limit a name to 40 characters length */
char *name;
int pref;
{
...
do_objs()
{
    ...
    for (i = 0; !i || objects[i].oc_class != ILLOBJ_CLASS; i++) {
        ...
        case AMULET_CLASS:
            ...
            Fprintf(ofp, "#define\t");
            if (objects[i].oc_material == MAT_PLASTIC) {
