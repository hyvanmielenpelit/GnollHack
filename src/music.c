...
static void put_monsters_to_sleep(int, int);
static void charm_snakes(int);
static void calm_nymphs(int);
static void charm_monsters(int, int);
...
void
awaken_monsters(int distance, boolean isscary)
{
    struct monst *mtmp;
    int distm;
    int cnt = 0;
...
static void
put_monsters_to_sleep(int otyp, int distance)
{
    struct monst *mtmp;
...
charm_snakes(int distance)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (mtmp->data->mlet == S_SNAKE && is_mon_canmove(mtmp)
            && distu(mtmp->mx, mtmp->my) < distance) {
            was_peaceful = is_mon_peaceful(mtmp);
...
calm_nymphs(int distance)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (mtmp->data->mlet == S_NYMPH && is_mon_canmove(mtmp)
            && distu(mtmp->mx, mtmp->my) < distance) {
            set_mon_sleeping(mtmp, 0);
            set_mon_peaceful(mtmp, 1);
            set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MAVENGE, 0);
...
/*
 * Parameters:
 *   bugler: monster that played instrument
 */
/* Awake soldiers anywhere the level (and any nearby monster). */
void
awaken_soldiers(struct monst *bugler, boolean isscary)
{
    struct monst *mtmp;
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (is_mercenary(mtmp->data) && mtmp->data != &mons[PM_GUARD]) {set_mon_peaceful(mtmp, 0); mtmp->mfrozen = 0;
            set_mon_sleeping(mtmp, 0);
            set_mon_canmove(mtmp, 1);
            mtmp->mstrategy &= ~STRAT_WAITMASK;
            newsym(mtmp->mx, mtmp->my);
 ...
 */
static void
charm_monsters(int distance, int saving_throw_adjustment)
{
    ...
    for (x = start_x; x <= end_x; x++)
        for (y = start_y; y <= end_y; y++) {
            if ((mtmp = m_at(x, y)) != 0) {
                ...
                if (is_mon_undetected(mtmp) && is_hider(mtmp->data)) {
                    set_mon_undetected(mtmp, 0);
                    if (cansee(x, y))
            ...
            if (!rn2(max(3, 16 - force)))
                ...
                do_pit:
                    ...
                    if (mtmp) 
                    {
                        ...
                        {
                            boolean m_already_trapped = is_mon_trapped(mtmp);

                            set_mon_trapped(mtmp, 1);
                        ...
                        if (isok(x, y))
                            if (find_drawbridge(&x, &y)) 
                            {
                                ...
                                if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_LEARNED_CASTLE_TUNE))
                                {
                                    ...
                                    set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_LEARNED_CASTLE_TUNE, 1);
