    ...
    for (otmp = ochain; otmp; otmp = nobj)
    {
        ...
        { /* saving */
            ...
            if (objects[otmp->otyp].oc_uses_known)
                set_obj_known(otmp, 0);set_obj_dknown(otmp, 0); set_obj_bknown(otmp, 0);
            ...
            set_flag(otmp->obj_bitflags, OBJ_BITFLAGS_WAS_THROWN, 0);
...
make_bones:
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (((mptr->difficulty > maxmlev || (mptr->geno & G_UNIQ) != 0)
            ...
            && !((is_mon_peaceful(mtmp) && (mptr->geno & G_UNIQ) == 0) || is_mon_shk(mtmp) || is_mon_smith(mtmp) || is_mon_npc(mtmp) || is_mon_priest(mtmp))
            )
            || is_mon_wiz(mtmp) 
            ...
            mongone(mtmp);
        else if (!is_mon_peaceful(mtmp) && (is_watch(mptr) || mptr->msound == MS_LEADER || is_mon_shk(mtmp) || is_mon_smith(mtmp) || is_mon_npc(mtmp) || is_mon_priest(mtmp))) /* Shopkeepers are pacified upon loading */
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        mtmp->mlstmv = 0L;
        if (mtmp->mtame)
        {mtmp->mtame = 0; set_mon_peaceful(mtmp, 0);
    ...
    if (wizard)
        set_flag(level.flags.bitflags, LEVEL_BITFLAGS_WIZARD_BONES, 1);
