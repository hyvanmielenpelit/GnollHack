...
struct npc_subtype_definition npc_subtype_definitions[MAX_NPC_SUBTYPES] = 
{
    ...
    },
    {
        ...
        1, 100, 5,
        TRUE, CMAP_GNOMISH_MINES,
        NPC_SERVICE_GIVE_ORCISH_QUESTS | NPC_SERVICE_TEACH_SPECIAL_SPELLS | NPC_SERVICE_TEACH_WIZARD_SPELLS | NPC_SERVICE_IDENTIFY_ITEMS,
...
{
    struct enpc* enpc_p = is_mon_isnpc(npc) ? ENPC(npc) : 0;
...
{
    return (boolean)(npc && is_mon_isnpc(npc)
...
findnpc(char roomno)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (is_mon_isnpc(mtmp) && (ENPC(mtmp)->npc_room == roomno)
...
in_npc_room(int roomno, boolean ignore_already_there)
{
    ...
    if ((npc = findnpc((char)roomno)) != 0)
    {
        ...
        if (can_speak && !Deaf)
        {
            ...
            if (context.reviving && !is_mon_mpeaceful(npc))
            {
                /* Revival pacifies nearby NPCs */
                set_mon_mpeaceful(npc, 1);
...
clearnpcs(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (is_mon_isnpc(mtmp) && !on_level(&(ENPC(mtmp)->npc_room_level), &u.uz))
            mongone(mtmp);
    }
}
