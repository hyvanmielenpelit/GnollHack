...
static int mkroll_launch(struct trap *, xchar, xchar,
                                     short, int64_t);
static boolean isclearpath(coord *, int, schar, schar);
static void dofiretrap(struct obj *, int);
...
static int disarm_magical_trap(struct trap*);
...
static int try_lift(struct monst *, struct trap *, int,
                                boolean);
...
erode_obj(struct obj *otmp, const char *ostr, int type, int ef_flags)
{
    ...
    if (check_grease && is_obj_greased(otmp))
    {
    ...
    else if (!vulnerable || (is_obj_erodeproof(otmp) && is_obj_rknown(otmp)))
    {
    ...
    else if (is_obj_erodeproof(otmp) || (is_obj_blessed(otmp) && !rnl(4)) || is_obj_protected_by_property(otmp, victim, adtyp))
    {
        if (flags.verbose && (print || is_obj_erodeproof(otmp))
            && (uvictim || vismon || visobj))
...
struct trap *
maketrap(int x, int y, int typ, int permonstid, uint64_t mkflags)
{
    ...
    case TRAPDOOR:
        ...
        {
            ltype = get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_MAZE_LEV)
                ? ROOM
                : get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_CAVERNOUS_LEV) ? CORR : DOOR;
...
fall_through(boolean td)
{
    ...
    if (Sokoban && Can_fall_thru(&u.uz))
        ; /* KMH -- You can't escape the Sokoban level traps */
             ...
             || (!Can_fall_thru(&u.uz) && !get_flag(levl[u.ux][u.uy].rm_bitflags, RM_BITFLAGS_CANDIG)) || Flying
             || is_clinger(youmonst.data)
             || (Inhell && !get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED) && newlevel == bottom)) {
...
animate_statue(struct obj *statue, xchar x, xchar y, int cause, int *fail_reason)
{
    ...
    if (use_saved_traits) 
    {
        ...
        if (mon && mon->mtame && !is_mon_minion(mon))
            wary_dog(mon, TRUE);
    }
    else 
    {
...
void
set_utrap(uint64_t tim, uchar typ)
{
    u.utrap = tim;
    u.utraptype = typ;
...
void
dotrap(struct trap *trap, unsigned short trflags)
{
    int ttype = trap->ttyp;
    ...
    case RUST_TRAP:
        ...
        default:
            ...
            for (otmp = invent; otmp; otmp = otmp->nobj)
                if (is_obj_lamplit(otmp) && otmp != uwep && otmp != uarms)
                    (void) snuff_lit(otmp);
            if (uarmc)
    ...
    while (dist-- > 0 && !used_up) {
        ...
        if (style == ROLL) {
            ...
            if (otyp == BOULDER
                ...
                set_obj_trapped(otmp2, singleobj->otrapped);
                singleobj->otrapped = 0;
                place_object(singleobj, bhitpos.x, bhitpos.y);
                singleobj = otmp2;
                otmp2 = (struct obj *) 0;
...
static int
mkroll_launch(struct trap *ttmp, xchar x, xchar y, short otyp, int64_t ocount)
{
    struct obj *otmp;
    int tmp;
    schar dx, dy;
    ...
    if (!success) {
        ...
        if ((objects[otyp].oc_flags5 & O5_TILE_IS_TILESET_DEPENDENT) != 0)
        {
            ...
            otmp->special_tileset = is_lev_has_special_tileset(cc.x, cc.y) ? levl[cc.x][cc.y].special_tileset : get_current_cmap_type_index();
        }
        stackobj(otmp);
    }
    ttmp->launch.x = cc.x;
...
mintrap(struct monst *mtmp)
{
    ...
    if (!trap)
    {
        set_mon_trapped(mtmp, 0);      /* perhaps teleported? */
    } 
    ...
    else
    {
        ...
        case WEB:
            ...
            default:
                ...
                set_mon_trapped(mtmp, tear_web ? 0 : 1);
                break;
            ...
            else if (force_mintrap && !is_mon_trapped(mtmp)) 
            {
                if (in_sight) 
                {
...
{
    set_flag(mtmp->mon_bitflags, MON_BITFLAGS_DELAYED_KILLER_BY_YOU, by_you);
...
/*
 * Parameters:
 *   hmask, emask: might cancel timeout
 */
/* stop levitating */
int
float_down(int64_t hmask, int64_t emask)
{
 ...
 */
static void
dofiretrap(struct obj *box, int dice)
{
...
fire_damage(struct obj *obj, boolean force, xchar x, xchar y)
{
    ...
    if (Is_container(obj) && !oresist_fire(obj))
    {
        ...
        if ((!force && (Luck + 5) > rn2(chance))
            || (is_flammable(obj) && is_obj_erodeproof(obj)))
            return FALSE;
...
water_damage(struct obj *obj, const char *ostr, boolean force)
{
    ...
    else if (is_obj_greased(obj)) 
    {
        ...
        && (obj->otyp != OILSKIN_SACK || (is_obj_cursed(obj) && !rn2(3)))) 
    {
        if (carried(obj))
    ...
    else if (obj->oclass == POTION_CLASS)
    {
        if (obj->otyp == POT_ACID) {
            ...
            if (acid_ctx.ctx_valid)
                exploded = ((is_obj_dknown(obj) ? acid_ctx.dkn_boom
                                         : acid_ctx.unk_boom) > 0);
            /* First message is
             * "a [potion|<color> potion|potion of acid] explodes"
             * depending on is_obj_dknown(obj) (potion has been seen) and
    ...
    {
        if (is_poisonable(obj) && is_obj_trapped(obj))
        {
    ...
    while (near_capacity() > (Punished ? UNENCUMBERED : SLT_ENCUMBER)) {
        ...
        if (invc > 0) {
            ...
            for (obj = invent; obj; obj = obj->nobj) {
                 ...
                 * for obvious reasons.
                 */
                if (!(((objects[obj->otyp].oc_flags & O1_CANNOT_BE_DROPPED_IF_CURSED) && is_obj_cursed(obj)) || obj == uamul
                      ...
                      || (is_obj_cursed(obj) && (obj == uarmh || obj == uarms))
                      || welded(obj, &youmonst)))
                    otmp = obj;
...
int
succeed_untrap(int ttyp, boolean helping_another)
{
...
int
trap_difficulty(int trap_type, boolean helping_another, struct monst *mon)
{
...
int
untrap_probability(int trap_type, int skill_level, struct monst *mon)
{
    //int probability = 0;
    //switch (skill_level)
    //{
...
void
cnv_trap_obj(int otyp, int cnt, struct trap *ttmp, boolean bury_it, boolean set_found)
{
...
try_disarm(struct trap *ttmp, boolean force_failure)
{
    ...
    if (mtmp && (!is_mon_trapped(mtmp) || !holdingtrap)) 
    {
...
/* Janne Gustafsson */
static int
disarm_magical_trap(struct trap* ttmp)
{
    int fails = try_disarm(ttmp, FALSE);
    if (fails < 2)
    {
...
/* Helge Hafting */
static int
disarm_landmine(struct trap* ttmp)
{
    int fails = try_disarm(ttmp, FALSE);
    boolean madebyu = ttmp->madeby_u;
...
disarm_squeaky_board(struct trap *ttmp)
{
    ...
    bad_tool = (is_obj_cursed(obj)
                || ((obj->otyp != POT_OIL || is_obj_lamplit(obj))
            ...
            for (otmp = level.objects[x][y]; otmp; otmp = otmp->nexthere)
                if (Is_box(otmp)) 
                {
                    (void) safe_qbuf(qbuf, "There is ",
                        is_obj_tknown(otmp) && is_obj_trapped(otmp) ? " here.  Investigate the trap further?" : " here.  Check it for traps?", otmp,
...
int
check_all_box_traps(boolean force)
{
    struct obj* otmp;
    int boxcnt = 0;
    ...
    { /* deal_with_floor_trap */
        ...
        {
            for (otmp = level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere)
                if (Is_box(otmp) && !(is_obj_tknown(otmp) && !is_obj_trapped(otmp)))
                {
                    (void)safe_qbuf(qbuf, "There is ",
                        is_obj_tknown(otmp) && is_obj_trapped(otmp) ? " here.  Investigate the trap further?" : " here.  Check it for traps?", otmp,
                        doname, ansimpleoname, "a box");
...
int
check_box_trap(struct obj *otmp, boolean force)
{
    boolean confused = (Confusion || Hallucination);
    int ch = 0;

    if (!is_obj_tknown(otmp))
    {
        play_simple_player_sound(MONSTER_SOUND_TYPE_SEARCH);
        if (iflags.using_gui_sounds)
            delay_output_milliseconds(10 * ANIMATION_FRAME_INTERVAL);
    ...
    if ((is_obj_trapped(otmp)
        && (force || is_obj_tknown(otmp) || (!confused
...
/*
 * Parameters:
 *   noticed: set to true iff hero notices the effect;
 */
...
chest_trap(struct obj *obj, int bodypart, boolean disarm)
{
    ...
    set_obj_tknown(otmp, 1); /* You found it by triggering it! */
    set_obj_trapped(otmp, 0); /* trap is one-shot; clear flag first in case
...
void
b_trapped(const char *item, int bodypart, xchar x, xchar y)
{
...
lava_effects(void)
{
    ...
    if (!Fire_immunity)
    {
         ...
         */
        marked_in_use = TRUE;
        for (obj = invent; obj;obj = 1; obj->nobj)
...
const char*
get_trap_explanation(struct trap *trap)
{
...
int
get_shooting_trap_object(struct trap *trap)
{
    if (!trap)
        return 0;
