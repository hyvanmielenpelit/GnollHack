...
const struct Role roles[] = {
    ...
    { { "Barbarian", 0 },
      ...
      "the Duali Oasis",
      ROLE_BARBARIAN,
      "Melee warrior immune to poison",
      { "Extra score from melee weapons of artifact or legendary quality", "", "", "", ""},
      PM_BARBARIAN,
      NON_PM,
      PM_PELIAS,
      PM_CHIEFTAIN,
    ...
    { { "Healer", 0 },
      ...
      { "Recognizes corpse properties", "Extra score from learnt new spells", "", "", ""},
      PM_HEALER,
      NON_PM,
      PM_HIPPOCRATES,
      PM_ATTENDANT,
    ...
    { { "Knight", 0 },
      ...
      "the Isle of Glass",
      ROLE_KNIGHT,
      "Mounted armored melee warrior",
      { "Extra score of slain demons, devils, and chaotic dragons", "Alignment penalty if attacks disadvantaged monsters", "", "", ""},
      PM_KNIGHT,
      PM_PONY,
      PM_KING_ARTHUR,
      PM_PAGE,
    ...
    { { "Monk", 0 },
      ...
      ROLE_MONK,
      "Martial artist with priest spells",
      { 
          "50% extra score from all conducts",
          "Vegetarianism",
    ...
    { { "Priest", "Priestess" },
      ...
      ROLE_PRIEST,
      "Clerical spell caster and diviner",
      { "Extra score from learnt new spells", "Sees the beatitude of all objects", "Can pray twice as often as normal", "", ""},
      PM_PRIEST,
      NON_PM,
      PM_ARCH_PRIEST,
      PM_ACOLYTE,
    ...
    { { "Rogue", 0 },
      ...
      ROLE_ROGUE,
      "Poison-using thief who backstabs",
    ...
    { { "Ranger", 0 },
      ...
      ROLE_RANGER,
      "Archer excelling at ranged combat",
      { "Extra score from ranged weapons of at least artifact, elite, or mythic quality", "", "", "", ""},
      PM_RANGER,
    ...
    { { "Samurai", 0 },
      ...
      "the Shogun's Castle",
      ROLE_SAMURAI,
      "Melee and ranged combat warrior",
      { "Extra score from Japanese items of at least artifact, exceptional, or mythic quality", "Starts with a faithful dog", "", "", ""},
      PM_SAMURAI,
      PM_LITTLE_DOG,
      PM_LORD_SATO,
      PM_ROSHI,
    ...
    { { "Valkyrie", 0 },
      ...
      "the cave of Surtur",
      ROLE_VALKYRIE,
      "Melee warrior resistant to cold",
...
const struct Race races[] = {
    ...
    },
    {
        ...
        { 0, 0 },
        1,
        "Adept at spell casting but fragile",
...
const struct Align aligns[] = {
    { "law", "lawful", "Law", ROLE_LAWFUL, A_LAWFUL, "Worships a deity of order" },
...
role_menu_extra(int which, winid where, boolean preselect)
{
    ...
    if (constrainer) {
        ...
        add_menu(where, NO_GLYPH, &any, 0, 0, ATR_NONE, NO_COLOR, buf,
                 MENU_UNSELECTED);
    } else if (what) {
        any.a_int = RS_menu_arg(which);
        Sprintf(buf, "Pick%s %s first", (f >= 0) ? " another" : "", what);
        add_menu(where, NO_GLYPH, &any, RS_menu_let[which], 0, ATR_NONE, NO_COLOR, buf,
                 MENU_UNSELECTED);
    } else if (which == RS_filter) {
        any.a_int = RS_menu_arg(RS_filter);
        add_menu(where, NO_GLYPH, &any, '~', 0, ATR_NONE, NO_COLOR,
                 "Reset role/race/&c filtering", MENU_UNSELECTED);
    } else if (which == ROLE_RANDOM) {
        any.a_int = ROLE_RANDOM;
        add_menu(where, NO_GLYPH, &any, '*', 0, ATR_NONE, NO_COLOR, "Random",
                 preselect ? MENU_SELECTED : MENU_UNSELECTED);
    } else if (which == ROLE_NONE) {
        any.a_int = ROLE_NONE;
        add_menu(where, NO_GLYPH, &any, 'q', 0, ATR_NONE, NO_COLOR, "Quit",
                 preselect ? MENU_SELECTED : MENU_UNSELECTED);
    } else {
...
role_init(void)
{
    ...
    if (Role_if(PM_MONK))
        flags.paranoia_bits |= PARANOID_WIELDED_WEAPON | PARANOID_VEGAN;
