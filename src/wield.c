...
long mask;
{
    ...
    else 
    {
        ...
        if (will_weld(wep, &youmonst)) 
        {
            ...
            pline_ex(ATR_NONE, CLR_MSG_CRITICAL, "%s%s %s to your %s!", tmp, aobjnam(wep, "weld"),
...
const char *verb; /* "rub",&c */
{
    ...
    if (wep && welded(wep, &youmonst))
    {
        ...
        {
            play_sfx_sound(SFX_GENERAL_CANNOT);
            You_cant_ex(ATR_NONE, CLR_MSG_CRITICAL, "do that.");
        }
        return FALSE;
    }
...
register struct obj *obj;
{
    ...
    pline_ex(ATR_NONE, CLR_MSG_CRITICAL, "%s welded to your %s%s!", Yobjnam2(obj, "are"), bimanual(obj) ? "" : savewornmask & W_WEP ? "right " : "left ",
