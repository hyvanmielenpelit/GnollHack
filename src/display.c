...
magic_map_background(xchar x, xchar y, int show)
{
    ...
    set_rm_waslit(lev, is_rm_lit(lev));
...
map_background(xchar x, xchar y, int show)
{
    ...
    int new_floor_doodad_gui_glyph = maybe_get_replaced_glyph(new_floor_doodad_glyph, x, y, data_to_replacement_info(new_floor_doodad_glyph, LAYER_FLOOR_DOODAD, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
    ...
    int new_carpet_gui_glyph = maybe_get_replaced_glyph(new_carpet_glyph, x, y, data_to_replacement_info(new_carpet_glyph, LAYER_CARPET, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
    ...
    if (defsyms[symbol_index].layer != LAYER_FLOOR)
    {
        ...
        new_floor_gui_glyph = maybe_get_replaced_glyph(new_floor_glyph, x, y, data_to_replacement_info(new_floor_glyph, LAYER_FLOOR, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
...
map_trap(struct trap *trap, int show)
{
    ...
    boolean mtrapped = (mtmp && is_mon_trapped(mtmp));
    ...
    if (show)
    {
        ...
        if (mtrapped || utrapped)
            add_glyph_buffer_layer_flags(x, y, LFLAGS_T_TRAPPED, 0UL);
        else
            remove_glyph_buffer_layer_flags(x, y, LFLAGS_T_TRAPPED, 0UL);
...
map_object_core(struct obj *obj, int show, boolean chain_check, boolean add_detection_mark, boolean exclude_ascii)
{
    ...
    /* Replace */
    int gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, layer, obj, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
    ...
    if (is_hero_memory())
    {
        ...
        if (Hallucination && obj->otyp == STATUE)
        {
            ...
            new_gui_glyph = maybe_get_replaced_glyph(new_glyph, x, y, data_to_replacement_info(glyph, layer, obj, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
            obj_height = 0;
        }
    ...
    if (show)
    {
        ...
        if(in_pit)
            add_glyph_buffer_layer_flags(x, y, LFLAGS_O_IN_PIT, 0UL);
        if (isok(x, y) && uchain && obj == uchain && uchain->ox == x && uchain->oy == y)
            add_glyph_buffer_layer_flags(x, y, LFLAGS_O_CHAIN, 0UL);
    }
}
...
map_invisible(xchar x, xchar y)
{
    if (x != u.ux || y != u.uy) { /* don't display I at hero's location */
        ...
        show_monster_glyph_with_extra_info(x, y, GLYPH_INVISIBLE, (struct monst*)0, 0UL, 0UL, 0, 0);
    }
}
...
unmap_object(int x, int y)
{
    ...
    boolean waslit = is_lev_waslit(x, y);
    clear_hero_memory_at(x, y);

    if (levl[x][y].seenv)
    {
        set_lev_waslit(x, y, waslit);
...
display_monster(xchar x, xchar y, struct monst *mon, int sightflags, xchar worm_tail, boolean dropping_piercer)
{
    ...
    if (mon_mimic && (sightflags == PHYSICALLY_SEEN)) 
    {
        ...
        case M_AP_FURNITURE: 
        {
             ...
             * makemon.c.
             */
            int cmap_type = is_lev_has_special_tileset(x, y) ? levl[x][y].special_tileset : get_current_cmap_type_index();
        ...
        case M_AP_OBJECT: 
        {
            ...
            if (is_hero_memory())
            {
                ...
                {
                    new_glyph = random_obj_to_glyph(newsym_rn2);
                    new_gui_glyph = maybe_get_replaced_glyph(new_glyph, x, y, data_to_replacement_info(new_glyph, LAYER_OBJECT, &obj, mon, 0UL, 0UL, 0UL, MAT_NONE, 0));
                }
                obj.glyph = new_glyph;
    ...
    if (!mon_mimic || sensed) 
    {
        ...
        if (worm_tail)
            extra_mflags |= (LMFLAGS_WORM_TAIL | LMFLAGS_WORM_SEEN);

        if (dropping_piercer)
        {
            extra_mflags |= LMFLAGS_DROPPING_PIERCER;
            int gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_MISSILE, (struct obj*)0, mon, extra_flags, 0UL, 0UL, MAT_NONE, 0));
            show_gui_glyph_on_layer_and_ascii(x, y, glyph, gui_glyph, LAYER_MISSILE);
            show_extra_info(x, y, extra_flags, extra_mflags, 0, 0);
        }
        else
            show_monster_glyph_with_extra_info(x, y, glyph, /*worm_tail ? (struct monst*)0 :*/ mon, extra_flags, extra_mflags, 0, 0);
        clear_monster_layer_memory_at(x, y);
    }
}
...
display_warning(struct monst *mon)
{
    ...
    show_monster_glyph_with_extra_info(x, y, glyph, (struct monst*)0, 0UL, 0UL, 0, 0);
}
...
newsym_with_extra_info_and_flags(int x, int y, uint64_t disp_flags, uint64_t disp_mflags, int hit_tile_id, int damage_shown, uint64_t specific_newsym_flags)
{
    ...
    {
        if (x == u.ux && y == u.uy)
            display_self_with_extra_info_choose_ascii(disp_flags, disp_mflags, hit_tile_id, damage_shown, FALSE);
    ...
    /* Extra info shown */
    show_extra_info(x, y, disp_flags, disp_mflags, hit_tile_id, damage_shown);
    ...
    if (cansee(x, y)) 
    {
         ...
         * out of sight.
         */
        set_lev_waslit(x, y, is_rm_lit(lev)); /* remember lit condition */
        ...
        if (x == u.ux && y == u.uy)
        {
            ...
            if (see_self)
                display_self_with_extra_info_choose_ascii(disp_flags | extra_flags, disp_mflags | extra_mflags, hit_tile_id, damage_shown, location_has_boulder);
            else
            {
                show_extra_info(x, y, disp_flags | extra_flags, disp_mflags | extra_mflags, hit_tile_id, damage_shown);
...
//                _map_location(x, y, 1); /* map the location */
    ...
    else
    {
        ...
        else
        {
            ...
            add_glyph_buffer_layer_flags(x, y, LFLAGS_SHOWING_MEMORY, 0UL);
            ...
            add_glyph_buffer_layer_flags(x, y, lev->hero_memory_layers.layer_flags, lev->hero_memory_layers.monster_flags);
            ...
            if (mon)
            {
                ...
                {
                    add_glyph_buffer_layer_flags(x, y, 0UL, LMFLAGS_WORM_TAIL);
                }
            }
        }
    }
...
new_sym_end_here:
    ...
    if (x == u.ux && y == u.uy)
    {
        add_glyph_buffer_layer_flags(x, y, LFLAGS_UXUY, 0UL); /* Mark player location */
    }
    ...
    if (newsym_flags & NEWSYM_FLAGS_KEEP_OLD_ZAP_GLYPH)
    {
        ...
        add_glyph_buffer_layer_flags(x, y, zap_flags, 0UL);
    }
    ...
    {
        add_glyph_buffer_layer_flags(x, y, old_flags, old_mflags);
    }
...
tmp_at_with_obj(int x, int y, struct obj *obj, uint64_t missile_flags, uchar missile_material, short missile_special_quality)
{
    ...
    case DISP_ALWAYS:
        ...
        tglyph->style = x;
        tglyph->glyph = y;
        tglyph->gui_glyph = maybe_get_replaced_glyph(y, 0, 0, data_to_replacement_info(y, LAYER_MISSILE, obj, (struct monst*)0, 0UL, 0UL, missile_flags, missile_material, missile_special_quality));
        context.tether_x = 0;
        context.tether_y = 0;
        ...
        if (tglyph->style == DISP_BEAM || tglyph->style == DISP_BEAM_DIG || tglyph->style == DISP_ALL) {
            ...
            if (y == BACKTRACK && tglyph->sidx > 1) {
                ...
                for (i = tglyph->sidx - 1; i > 0; i--) {
                    ...
                    if(obj)
                        show_missile_info(tglyph->saved[i - 1].x, tglyph->saved[i - 1].y, is_obj_trapped(obj), obj->material, obj->special_quality, obj->elemental_enchantment, obj->exceptionality, obj->mythic_prefix, obj->mythic_suffix, obj->oeroded, obj->oeroded2, get_missile_flags(obj, TRUE), get_obj_height(obj), 0, 0);
    ...
    default: /* do it */
        ...
        if (tglyph->style == DISP_BEAM || tglyph->style == DISP_BEAM_DIG || tglyph->style == DISP_ALL) {
            ...
            if (tglyph->sidx) {
                ...
                if (obj)
                    show_missile_info(px, py, is_obj_trapped(obj), obj->material, obj->special_quality, obj->elemental_enchantment, obj->exceptionality, obj->mythic_prefix, obj->mythic_suffix, obj->oeroded, obj->oeroded2, get_missile_flags(obj, TRUE), get_obj_height(obj), 0, 0);
        ...
        if (obj)
            show_missile_info(x, y, is_obj_trapped(obj), obj->material, obj->special_quality, obj->elemental_enchantment, obj->exceptionality, obj->mythic_prefix, obj->mythic_suffix, obj->oeroded, obj->oeroded2, get_missile_flags(obj, TRUE), get_obj_height(obj), 0, 0);
...
show_extra_info(int x, int y, uint64_t disp_flags, uint64_t disp_mflags, int hit_tile_id, int damage_displayed)
{
    if (isok(x, y))
    {
        ...
        if (old_flags != gbuf[y][x].layers.layer_flags ||
            old_mflags != gbuf[y][x].layers.monster_flags)
        {
            gbuf[y][x].isnew = 1;
            if (gbuf_start[y] > x)
                gbuf_start[y] = x;
...
show_monster_glyph_with_extra_info_choose_ascii(int x, int y, int glyph, struct monst *mtmp, int x0, int y0, uint64_t disp_flags, uint64_t disp_mflags, int hit_tile_id, int damage_displayed, boolean exclude_ascii)
{
    if (isok(x, y))
    {
        ...
        /* Replace */
        int gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_MONSTER, mtmp ? (has_mobj(mtmp) ? MOBJ(mtmp) : 0) : 0, mtmp, 0UL, 0UL, 0UL, MAT_NONE, 0));
        ...
        {
            struct trap* t = 0;
            if (is_mon_trapped(mtmp) && (t = t_at(mtmp->mx, mtmp->my)) != 0 && (t->ttyp == PIT || t->ttyp == SPIKED_PIT))
                gbuf[y][x].layers.special_monster_layer_height = SPECIAL_HEIGHT_IN_PIT;
        ...
        if (mtmp && mtmp != &youmonst)
        {
            ...
            if (canspotmon(mtmp))
                extra_mflags |= LMFLAGS_CANSPOTMON;
        ...
        /* Add to layer */
        if(extra_flags != 0UL || extra_mflags != 0)
            add_glyph_buffer_layer_flags(x, y, extra_flags, extra_mflags);
...
update_tile_gui_info(boolean loc_is_you, struct monst *mtmp, int x, int y, uint64_t layer_flags UNUSED, uint64_t monster_flags)
{
    ...
    boolean ispeaceful = (monster_flags & LMFLAGS_PEACEFUL) != 0;
    boolean ispet = (monster_flags & LMFLAGS_PET) != 0;
    boolean isdetected = (monster_flags & LMFLAGS_DETECTED) != 0;
    ...
    if (loc_is_you && issteed)
    {
        ...
        int rider_gui_glyph = maybe_get_replaced_glyph(rider_glyph, x, y, data_to_replacement_info(rider_glyph, LAYER_MONSTER, (struct obj*)0, &youmonst, 0UL, 0UL, 0UL, MAT_NONE, 0));
...
refresh_m_tile_gui_info(struct monst *mtmp, boolean flush)
{
    ...
    {
        update_tile_gui_info(loc_is_you, mtmp, mtmp->mx, mtmp->my, gbuf[mtmp->my][mtmp->mx].layers.layer_flags, gbuf[mtmp->my][mtmp->mx].layers.monster_flags);
...
add_or_remove_glyph_to_layer(int x, int y, int glyph, boolean remove)
{
    if (isok(x, y))
    {
        ...
        if (glyph_is_cmap_or_cmap_variation(glyph))
        {
            ...
            if (!remove)
                gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, defsyms[cmap_idx].layer, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
        ...
        {
            if (!remove)
                gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_MONSTER, (struct obj*)0, m_at(x, y), 0UL, 0UL, 0UL, MAT_NONE, 0));
        ...
        {
            if (!remove)
                gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_MISSILE, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
        ...
        {
            if(!remove)
                gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_ZAP, (struct obj*)0, (struct monst*)0, 0UL, 0UL, gbuf[y][x].layers.missile_flags, MAT_NONE, 0));
        ...
        {
            if (!remove)
                gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_GENERAL_EFFECT, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
        ...
        {
            if (!remove)
                gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, LAYER_GENERAL_UI, (struct obj*)0, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
            /* Should not get here */
...
show_detection_everywhere(void)
{
    ...
    {
        for (y = 0; y < ROWNO; y++)
            add_glyph_buffer_layer_flags(x, y, LFLAGS_SHOWING_DETECTION, 0UL);
    }
...
show_memory_and_detection_everywhere(void)
{
    ...
    {
        for (y = 0; y < ROWNO; y++)
            add_glyph_buffer_layer_flags(x, y, LFLAGS_SHOWING_MEMORY | LFLAGS_SHOWING_DETECTION, 0UL);
    }
...
flush_screen(int cursor_on_u)
{
    ...
    for (y = 0; y < ROWNO; y++)
    {
        for (x = gbuf_start[y]; x <= gbuf_stop[y]; x++)
        {
            ...
            boolean is_lit_unknown_wall = (is_lev_waslit(x, y) && IS_NON_STONE_WALL(levl[x][y].typ) && wall_angle(&levl[x][y]) == S_stone);
            if (!is_lev_waslit(x, y) || is_lit_unknown_wall)
            {
...
back_to_glyph(xchar x, xchar y)
{
    ...
    boolean facing_right = is_rm_facing_right(ptr);
    int multiplier = facing_right ? -1 : 1;
    int cmap_type = is_rm_has_special_tileset(ptr) ? levl[x][y].special_tileset : get_current_cmap_type_index();

    switch (ptr->typ) {
    case UNDEFINED_LOCATION:
        idx = is_level_arboreal(level) ? S_tree : S_unexplored;
        break;
    case SCORR:
    case STONE:
    {
    ...
    break;
    case ROOM:
        idx = /*(!is_rm_waslit(ptr) || flags.dark_room) && !cansee(x, y) ? DARKROOMSYM: */ S_room;
        ...
        break;
    case GRASS:
        idx = /*(!is_rm_waslit(ptr) || flags.dark_room) && !cansee(x, y) ? DARKGRASSSYM :*/ S_grass;
        if (ptr->subtyp > 0 || ptr->vartyp > 0)
        {
            is_variation = TRUE;
            int var_offset = defsyms[idx].variation_offset;
    ...
    case GROUND:
        ...
        break;
    case CORR:
        idx = /* (is_rm_waslit(ptr) || flags.lit_corridor) ? */ S_litcorr /* : S_corr */;
        if (ptr->subtyp > 0 || ptr->vartyp > 0)
        {
            is_variation = TRUE;
            int var_offset = defsyms[idx].variation_offset;
    ...
    case DOOR:
    {
        ...
        if ((ptr->doormask & D_MASK)) {
            if (ptr->doormask & D_BROKEN)
                sym_idx = is_rm_horizontal(ptr) ? S_hbdoor : S_vbdoor;
            else if (ptr->doormask & D_ISOPEN)
                sym_idx = is_rm_horizontal(ptr) ? S_hodoor : S_vodoor;
            else if (ptr->doormask & D_PORTCULLIS)
                sym_idx = is_rm_horizontal(ptr) ? S_hoportcullis : S_voportcullis;
            else /* else is closed */
                sym_idx = is_rm_horizontal(ptr) ? S_hcdoor : S_vcdoor;
        }
        else
            sym_idx = S_ndoor;
    ...
    case WATER:
        ...
        break;
    case DBWALL:
        idx = is_rm_horizontal(ptr) ? S_hcdbridge : S_vcdbridge;
        break;
    case DRAWBRIDGE_UP:
    ...
    case DRAWBRIDGE_DOWN:
        idx = is_rm_horizontal(ptr) ? S_hodbridge : S_vodbridge;
        break;
    default:
...
get_floor_layer_glyph(xchar x, xchar y)
{
    ...
    case DOOR:
    case IRONBARS:
        idx = /* is_rm_waslit(ptr) ? */ S_room /* : DARKROOMSYM */;
        break;
    case TREE:
        idx = /* is_rm_waslit(ptr) ? */ S_grass /*: DARKGRASSSYM */;
        break;
    case LADDER:
    case STAIRS:
    case FOUNTAIN:
    ...
    case DRAWBRIDGE_DOWN:
        ...
        case DB_ICE:
            ...
            break;
        case DB_GROUND:
            idx = S_ground; // /* is_rm_waslit(ptr) ? */ S_room /*: DARKROOMSYM*/;
            break;
        default:
            impossible("Strange db-under: %d",
                ptr->drawbridgemask & DB_UNDER);
            idx = /* is_rm_waslit(ptr) ? */ S_room /*: DARKROOMSYM*/; /* something is better than nothing */
            break;
        }
        break;
    default:
...
set_wall_state(void)
{
    ...
    for (x = 0; x < COLNO; x++)
        for (lev = &levl[x][0], y = 0; y < ROWNO; y++, lev++) {
            switch (lev->typ) {
            case SDOOR:
                wmode = set_wall(x, y, (int) is_rm_horizontal(lev));
                break;
            case VWALL:
...
{
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_TILESET))
...
display_self_with_extra_info_choose_ascii(uint64_t displayed_flags, uint64_t displayed_mflags, int hit_tile_id, int dmg_received, boolean exclude_ascii)
{
    ...
    int cmap_type = is_lev_has_special_tileset(u.ux, u.uy) ? levl[u.ux][u.uy].special_tileset : get_current_cmap_type_index();
...
set_obj_glyph(struct obj *otmp)
{
    ...
    int gui_glyph = maybe_get_replaced_glyph(glyph, x, y, data_to_replacement_info(glyph, is_obj_drawn_in_front(otmp) ? LAYER_COVER_OBJECT : LAYER_OBJECT, otmp, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
    otmp->glyph = glyph;
