...
rm_waslit(void)
{
    ...
    if (IS_FLOOR(levl[u.ux][u.uy].typ) && is_lev_waslit(u.ux, u.uy))
        return TRUE;
    for (x = u.ux - 2; x < u.ux + 3; x++)
        for (y = u.uy - 1; y < u.uy + 2; y++)
            if (isok(x, y) && is_lev_waslit(x, y))
                return TRUE;
    return FALSE;
}
...
mkcavepos(xchar x, xchar y, int dist, boolean waslit, boolean rockit)
{
    ...
    //lev->doormask = 0;
    if (dist < 3)
        set_rm_lit(lev, (rockit ? FALSE : TRUE));
    if (waslit)
        set_rm_waslit(lev, (rockit ? FALSE : TRUE));
    //set_rm_horizontal(lev, FALSE);
                  ...
                  : (!issaw && closed_door(x, y))
                     ...
                     : IS_TREE(levl[x][y].typ)
                        ...
                        : (ispick && IS_ROCK(levl[x][y].typ)
                           && (!get_flag(level.flags.bitflags, LEVEL_BITFLAGS_ARBOREAL) || (IS_WALL_OR_SDOOR(levl[x][y].typ))))
                           ? DIGTYP_ROCK
                           : DIGTYP_UNDIGGABLE);
}
...
dig_check(struct monst *madeby, boolean verbose, int x, int y)
{
    ...
    else if (Is_waterlevel(&u.uz)) 
    {
        ...
        return FALSE;
               ...
               || (ttmp && ((trap_type_definitions[ttmp->ttyp].tdflags & TRAPDEF_FLAGS_NOT_OVERRIDDEN) || (!Can_dig_down(&u.uz) && !get_flag(levl[x][y].rm_bitflags, RM_BITFLAGS_CANDIG))))
...
dig(void)
{
    ...
    boolean ispick = wep && is_pick(wep);
    boolean isaxe = wep && is_axe(wep);
    boolean issaw = wep && is_saw(wep);
    ...
    if (context.digging.effort > 100) 
    {
        ...
        else if (IS_WALL(lev->typ)) 
        {
            ...
            else if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_CAVERNOUS_LEV) && !in_town(dpx, dpy)) 
            {
                ltype = CORR;
                lsubtype = get_initial_location_subtype(ltype);
                lvartype = get_initial_location_vartype(ltype, lsubtype);
...
schar
fillholetyp(int x, int y, boolean fill_if_any)
{
    int x1, y1;
...
digactualhole(int x, int y, struct monst *madeby, int ttyp)
{
    ...
    if (ttyp != PIT && (!Can_dig_down(&u.uz) && !get_flag(lev->rm_bitflags, RM_BITFLAGS_CANDIG))) {
...
dighole(boolean pit_only, boolean by_magic, coord *cc)
{
    ...
    nohole = (!Can_dig_down(&u.uz) && !get_flag(lev->rm_bitflags, RM_BITFLAGS_CANDIG));
...
//        else
        {
            ...
            else if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_CAVERNOUS_LEV)
...
static void
create_tree_remnants(int x, int y, boolean *done_feelnewsym_ptr)
{
    if (!isok(x, y))
        return;
...
void
zap_try_destroy_tree(int x, int y)
{
...
zap_dig(struct obj *origobj)
{
    ...
    //int prev_anim_counter_idx = -1;shopdoor = FALSE; shopwall = FALSE;
    maze_dig = get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_MAZE_LEV) && !Is_earthlevel(&u.uz);
    lzx = u.ux;
    lzy = u.uy;
    zx = u.ux + u.dx;
    zy = u.uy + u.dy;
    ...
    while (--digdepth >= 0)
    {
        ...
        else if (IS_ROCK(room->typ)) 
        {
            ...
            if (IS_WALL_OR_SDOOR(room->typ))
            {
                ...
                watch_dig((struct monst *) 0, zx, zy, TRUE);
                if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_CAVERNOUS_LEV) && !in_town(zx, zy)) 
                {
                    ltype = CORR;
...
void
zap_evaporation(struct obj *origobj)
{
    struct monst* mtmp;
...
int
unearth_objs(struct monst *mtmp, int x, int y, boolean verbose, boolean buriedsearchableonly)
{
...
/* ARGSUSED */
int
rot_organic(anything *arg, int64_t timeout UNUSED)
{
 ...
 */
int
rot_corpse(anything *arg, int64_t timeout)
{
    xchar x = 0, y = 0;
    struct obj *obj = arg->a_obj;
    ...
    if (on_floor) {
        ...
        if (mtmp && !OBJ_AT(x, y) && is_mon_undetected(mtmp)
...
void
uncatch_tree_objects(int x, int y)
{
    if (!isok(x, y))
        return;
