...
#define mon_wants_to_pick_up_obj(m, o) \
    ((m) && mon_can_move(m) && !(get_flag(m)->mon_bitflags, MON_BITFLAGS_ISSUMMONED) && mon_can_reach_floor(m) && !onnopickup((m)->mx, (m)->my, m) \
     && !((o) && couldsee((m)->mx, (m)->my) && (get_flag(o)->obj_bitflags, OBJ_BITFLAGS_WAS_THROWN)))
        ...
        if (!mon->notalktimer)
        {
            ...
            { 
                ...
                if(is_peaceful(mon) && (special_talk || (!is_undead(mon->data) && !is_demon(mon->data) && !In_endgame(&u.uz) && !Is_sanctum(&u.uz)
                    && !is_mon_shk(mon) && !is_mon_gd(mon) && !is_mon_priest(mon) && !is_mon_smith(mon) && !is_mon_npc(mon) && !is_watch(mon->data) && !is_mercenary(mon->data)
                    && !is_lord(mon->data) && !is_prince(mon->data) && mon->m_lev < 10
                    && !(is_mon_wiz(mon) || is_medusa(mon->data) || (mon->data->geno & G_UNIQ) != 0
...
onscary(int x, int y, struct monst *mtmp)
{
    ...
    if (is_mon_wiz(mtmp) || is_lminion(mtmp) || mtmp->data == &mons[PM_ANGEL] || (mtmp->data->geno & G_UNIQ)
        || is_rider(mtmp->data)
        || (is_mon_shk(mtmp) && inhishop(mtmp))
        || (is_mon_priest(mtmp) && inhistemple(mtmp))
        || (is_mon_smith(mtmp) && inhissmithy(mtmp))
        || (is_mon_npc(mtmp) && in_his_npc_room(mtmp))
        )
        return FALSE;
    ...
    boolean res = (sengr_at(Elbereth_word, x, y, TRUE)
            ...
            && !(is_mon_shk(mtmp) || is_mon_gd(mtmp) || is_blinded(mtmp)
...
onnopickup(int x, int y, struct monst *mtmp)
{
    ...
    return (((sengr_at(Gilthoniel_word, x, y, TRUE) && !Inhell) || (sengr_at(Morgoth_word, x, y, TRUE) && Inhell))
        && !(is_mon_shk(mtmp) || is_mon_gd(mtmp) || is_mon_wiz(mtmp))
        );
}
...
monflee(struct monst *mtmp, int fleetime, boolean first, boolean fleemsg)
{
    ...
    if (!first || !is_mon_fleeing(mtmp))
    {
        /* don't lose untimed scare */
        if (!fleetime)
            mtmp->mflee_timer = 0;
        else if (!is_mon_fleeing(mtmp) || mtmp->mflee_timer)
        {
            ...
            && M_AP_TYPE(mtmp) != M_AP_OBJECT) 
        {
            ...
            if (!mon_can_move(mtmp) || !mtmp->data->mmove)
            {
                ...
                else if (is_paralyzed(mtmp) && is_mon_canmove(mtmp))
...
dochug(struct monst *mtmp)
{
    ...
    if ((is_mon_sleeping(mtmp) && !disturb(mtmp)) || !mon_can_move(mtmp))
    {
        if (Hallucination)
            newsym(mtmp->mx, mtmp->my);
    ...
    if (is_fleeing(mtmp) && !rn2(40) && has_teleportation(mtmp) && !is_mon_wiz(mtmp)
        && !get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT))
    {
    ...
    else if (is_tentacled_one(mdat) && !rn2(20))
    {
        ...
        for (m2 = fmon; m2; m2 = nmon) 
        {
                ...
                || !rn2(10))
            {
                ...
                {
                    if (is_mon_sleeping(m2))
                    {
                        set_mon_sleeping(m2, 0);
                        refresh_m_tile_gui_info(mtmp, TRUE);
                    }
                }
            }
 ...
 toofar:
        ...
        && attacktype(mdat, AT_WEAP))
    {
        ...
        if (!(scared && mw_tmp && is_pick(mw_tmp))
            ...
            && !(is_mon_trapped(mtmp) && !nearby && select_rwep(mtmp))) 
        {
    ...
    if (!nearby || is_fleeing(mtmp) || scared || is_confused(mtmp) || is_stunned(mtmp)
        ...
        || (is_wanderer(mdat) && !rn2(4)) || ((Conflict || is_crazed(mtmp)) && !is_mon_wiz(mtmp))
        || (is_blinded(mtmp) && !rn2(4)) || is_peaceful(mtmp))
    {
        ...
        case 3: /* absolutely no movement */
            ...
            if (is_mon_gd(mtmp) && (DEADMONSTER(mtmp) || mtmp->mx == 0))
...
check_boss_fight(struct monst *mtmp)
{
    ...
    if ((windowprocs.wincap2 & WC2_SCREEN_TEXT) && (is_boss_monster(mtmp->data) || is_level_boss(mtmp)) && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_BOSS_FIGHT_STARTED) && !Hallucination
        && !DEADMONSTER(mtmp) && !is_peaceful(mtmp) && canspotmon(mtmp) && couldsee(mtmp->mx, mtmp->my) && !(mtmp->mstrategy & STRAT_WAITFORU) && !is_mon_sleeping(mtmp) && !is_cloned_wizard(mtmp))
    {
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_BOSS_FIGHT_STARTED, 1);
        newsym(mtmp->mx, mtmp->my);
        flush_screen(1);
...
m_move(struct monst *mtmp, int after)
{
    ...
    can_unlock = ((can_open && monhaskey(mtmp, TRUE)) || is_mon_wiz(mtmp) || is_rider(ptr));
 ...
 not_special:
    ...
    else 
    {
        ...
        boolean should_see = (couldsee(omx, omy)
                              && (is_lev_lit(gx, gy) || !is_lev_lit(omx, omy))
        ...
        if ((should_see && m_cannotsenseu(mtmp) && rn2(11))
            ...
            || (is_peaceful(mtmp) && !is_mon_shk(mtmp)) /* allow shks to follow */
...
#define SQSRCHRADIUS 5
 ...
 look_for_obj:
            ...
            for (otmp = fobj; otmp; otmp = otmp->nobj) 
            {
                ...
                if (xx >= lmx && xx <= oomx && yy >= lmy && yy <= oomy) 
                {
                    ...
                    if ((mtoo = m_at(xx, yy)) != 0
                        && (is_mon_sleeping(mtoo) || is_mon_undetected(mtoo)
                            || (mtoo->mappearance && !is_mon_wiz(mtoo))
                            || !mtoo->data->mmove))
                        continue;
    ...
    if (is_unicorn(ptr) && !get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT))
        flag |= NOTONL;
    if (passes_walls(ptr))
        ...
        flag |= BUSTDOOR;
    {
        ...
        if (!is_peaceful(mtmp) && get_flag(level.flags.bitflags, LEVEL_BITFLAGS_SHORTSIGHTED)
...
undesirable_disp(struct monst *mtmp, xchar x, xchar y)
{
    ...
    boolean is_pet = (mtmp && is_tame(mtmp) && !is_mon_minion(mtmp));
