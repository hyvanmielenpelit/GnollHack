					...
					if ((mtmp->cham == NON_PM) && !range2) { //Chameleons do not summon, others only in close range
						...
						else
						{
							if ((!rn2(2) || is_silenced(mtmp))
								&& (m_carrying(mtmp, TRIPLE_HEADED_FLAIL_OF_YEENOGHU) || m_carrying(mtmp, TRIPLE_HEADED_FLAIL) 
...
register struct obj* omonwep;
{
 ...
 do_stone:
	...
	if (damage > 0)
	{
        ...
        play_monster_hit_sound(mtmp, &youmonst, get_pm_attack_index(mtmp->data, mattk), omonwep, damage, 0);
        mdamageu(mtmp, damage, FALSE);
