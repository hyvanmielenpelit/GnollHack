...
mattacku(struct monst *mtmp)
{
    ...
    if (mdat->mlet == S_EEL && is_mon_undetected(mtmp) && cansee(mtmp->mx, mtmp->my))
    {
    ...
    boolean orig_mpeaceful = is_mon_peaceful(mtmp);
...
hitmu(struct monst *mtmp, struct attack *mattk, struct obj *omonwep)
{
     ...
     */
    if (is_mon_undetected(mtmp) && (hides_under(mdat) || mdat->mlet == S_EEL)) {
        set_mon_undetected(mtmp, 0);
...
cloneu(void)
{
    ...
    if (!mon)
        return NULL;
    set_flag(mon->mon_bitflags, MON_BITFLAGS_MCLONED, 1);
