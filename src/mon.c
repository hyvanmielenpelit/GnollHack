...
static struct permonst *accept_newcham_form(int);
static struct obj *make_corpse(struct monst *, unsigned, boolean);
static void lifesaved_monster(struct monst *);
static void save_traits_mon(struct monst*, boolean);
static struct monst* get_saved_traits_mon(struct monst*, boolean);
static void add_knight_slaying_score(struct monst*);

/* note: duplicated in dog.c */
#define LEVEL_SPECIFIC_NOCORPSE(mdat) \
    (Is_really_rogue_level(&u.uz)            \
     || ((get_flag(level.flags.bitflags, LEVEL_BITFLAGS_GRAVEYARD) || In_quest(&u.uz)) && is_undead(mdat) && rn2(3)))
...
sanity_check_single_mon(struct monst *mtmp, boolean chk_geno, const char *msg)
{
    ...
    else 
    {
        ...
        if (DEADMONSTER(mtmp)) {
            ...
            if (strcmp(msg, "fmon") || !is_mon_gd(mtmp))
...
mon_sanity_check(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (DEADMONSTER(mtmp) && !is_mon_gd(mtmp))
            continue;

        x = mtmp->mx, y = mtmp->my;
        if (!isok(x, y) && !(is_mon_gd(mtmp) && x == 0 && y == 0)) {
...
#define KEEPTRAITS(mon)                                                 \
    ((is_mon_shk(mon)) || (mon)->mtame || unique_corpstat((mon)->data)       \
 ...
 */
static struct obj *
make_corpse(struct monst *mtmp, unsigned corpseflags, boolean createcorpse)
{
    if (!mtmp)
        return (struct obj*) 0;
    ...
    if (!istame && !isquestmonster)
    {
        ...
        case PM_SKELETON_KING:
            if (!rn2(2))
            {
                obj = mksobj_found_at(BONE, x, y, TRUE, FALSE);
                obj->quan = rnd(2 + (mndx == PM_SKELETON_LORD ? 1 : 0));
                obj->owt = weight(obj);
            }
            if (!rn2(2))
                obj = mksobj_found_at(HUMAN_SKULL, x, y, TRUE, FALSE);
            break;
        ...
        case PM_HELL_HOUND_PUP:
            ...
            {
                obj = mksobj_found_at(PINCH_OF_SULFUROUS_ASH, x, y, TRUE, FALSE);
                if (ashquan > 1)
    ...
    case PM_ANCIENT_YELLOW_DRAGON:
        ...
        if (!istame && !isquestmonster)
        {
            ...
            if (oneinchance > 0 && basemonsterindex > 0 && (oneinchance == 1 || !rn2(oneinchance))) {
                ...
                if (obj)
                {
                    obj->enchantment = 0;set_obj_cursed(obj, FALSE); set_obj_blessed(obj, FALSE);
                    if (is_exceptional)
    ...
    case PM_PHASE_SPIDER:
        if (!istame && !isquestmonster)
        {
            ...
            {
                obj = mksobj_found_at(THREAD_OF_SPIDER_SILK, x, y, TRUE, FALSE);
    ...
    case PM_ARCHON:
        if (!istame && !isquestmonster)
        {
            if ((!mtmp->mrevived && !rn2(2)) || (mtmp->mrevived && !rn2(10)))
            {
                obj = mksobj_found_at(FEATHER, x, y, TRUE, FALSE);
                switch (mndx)
                {
                case PM_GARGANTUAN_COCKATRICE:
                case PM_ROC:
    ...
    case PM_HELL_BAT:
        ...
        {
            obj = mksobj_found_at(CLUMP_OF_BAT_GUANO, x, y, TRUE, FALSE);
    ...
    {
        sporequan++;
        if (!istame && !isquestmonster && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_MCLONED) && (!rn2(2) || mndx == PM_MUCILAGINOUS_CUBE))
        {
    ...
    case PM_GLASS_GOLEM:
        if (!istame && !isquestmonster)
        {
            if (!rn2(2))
            {
                num = d(1, 6);
                while (num--)
                    obj = mksobj_found_at((LAST_GEM + rnd(9)), x, y, TRUE, FALSE);
            }
        }
        free_mname(mtmp);
        free_umname(mtmp);
        ...
        if (!istame && !isquestmonster)
        {
            if (!rn2(2))
            {
                ...
                while (num--)
                {
                    ...
                    obj = mksobj_found_at(objid, x, y, TRUE, FALSE);
                }
            }
        }
        free_mname(mtmp);
    ...
    case PM_PAPER_GOLEM:
        if (!istame && !isquestmonster)
        {
            if (!rn2(2))
            {
                num = rnd(2);
                while (num--)
                    obj = mksobj_found_at(SCR_BLANK_PAPER, x, y, TRUE, FALSE);
            }
        }
        free_mname(mtmp);
        free_umname(mtmp);
    ...
    case PM_GEMSTONE_GOLEM:
        if (!istame && !isquestmonster)
        {
            if (!rn2(2))
            {
                ...
                while (num--)
                {
                    obj = mksobj_found_at(randomtruegem(), x, y, TRUE, FALSE);
                }
            }
        }
        break;
    ...
    case PM_BLACK_PUDDING:
        ...
        if (!istame && !isquestmonster && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_MCLONED) && !rn2(2))
        {
            obj = mksobj_found_at(HEAP_OF_SPORAL_POWDER, x, y, FALSE, FALSE);
            obj->quan = 1;
            obj->owt = weight(obj);
    ...
    default:
        ...
        else if (get_flag(mtmp->mon_bitflags, MON_BITFLAGS_LEAVES_NO_CORPSE)) 
        {
            return (struct obj*) 0;
        }
        else
...
/*
 * Parameters:
 *   reagentstyle: 0 = all, 1 = priest only, 2 = wizard (no priest specific reagents)
 */
int
random_reagent_otyp(boolean alsotruegems, boolean evenoreprobs, int reagentstyle)
{
    int otyp = 0;
...
int
mcalcmove(struct monst *mon, boolean return_expected_value)
{
...
update_monster_timeouts(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
                    ...
                    case STONED:
                        {
                            ...
                            {
                                minstapetrify(mtmp, get_flag(mtmp->mon_bitflags, MON_BITFLAGS_DELAYED_KILLER_BY_YOU));
        ...
        if (mtmp->mfrozen && !--mtmp->mfrozen)
            set_mon_canmove(mtmp, 1), need_update = TRUE;
...
movemon(void)
{
     ...
     * the chain are level migrations and arrival_from_mydogs_and_migrating_mons().  I believe arrival_from_mydogs_and_migrating_mons()
    ...
    for (mtmp = fmon; mtmp; mtmp = nmtmp)
    {
        ...
        /* one dead monster needs to perform a move after death: vault
           ...
           is_mon_gd(mon) flag so that dmonsfree() will get rid of mon) */
        if (is_mon_gd(mtmp) && !mtmp->mx) 
        {
        ...
        else if (mtmp->data->mlet == S_EEL && !is_mon_undetected(mtmp)
        ...
        if ((Conflict || is_crazed(mtmp) || is_mon_protecting(mtmp)) && !is_mon_wiz(mtmp) && !is_blinded(mtmp)) 
        {
            /* Note:
...
meatmetal(struct monst *mtmp)
{
         ...
         otmp = otmp->nexthere) 
    {
        ...
        {
            if (rust_causing_and_ironvorous(mtmp->data) && is_obj_erodeproof(otmp)) 
            {
...
mpickgold(struct monst *mtmp)
{
    ...
    if ((gold = g_at(mtmp->mx, mtmp->my)) != 0) 
    {
        ...
        if (cansee(mtmp->mx, mtmp->my)) 
        {
            if (flags.verbose && !is_mon_gd(mtmp))
...
int
can_carry_core(struct monst *mtmp, struct obj *otmp, boolean steed_ok)
{
...
int
mfndpos(struct monst *mon, coord *poss, int64_t *info, int64_t flag)
{
    xchar x = mon->mx;
    xchar y = mon->my;
...
/* return number of acceptable neighbour positions */
int
mfndpos_xy(struct monst *mon, xchar x, xchar y, coord *poss, int64_t *info, int64_t flag)
{
        ...
        for (ny = max(0, y - 1); ny <= maxy; ny++) 
        {
                ...
                && (lavaok || !is_lava(nx, ny))) 
            {
                ...
                else
                {
                    ...
                    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_TEMPLE) && *in_rooms(nx, ny, TEMPLE)
                        && !*in_rooms(x, y, TEMPLE)
...
/* Monster against monster special attacks; for the specified monster
   ...
   next to each other. */
static int64_t
mm_aggression(struct monst *magr, struct monst *mdef)
{
    /* peaceful NPCs should not attack anyone; if they are attacked, they will retaliate */
    if (is_peaceful(magr) && (is_mon_smith(magr) || is_mon_priest(magr) || is_mon_npc(magr) || is_mon_shk(magr)))
        return 0L;

    /* peaceful NPCs should not be attacked by anyone */
    if (is_peaceful(mdef) && (is_mon_smith(mdef) || is_mon_priest(mdef) || is_mon_npc(mdef) || is_mon_shk(mdef)))
        return 0L;
    ...
    if (mon_has_bloodlust(magr))
    {
        if(is_peaceful(mdef) && !is_tame(mdef) && is_living(mdef->data) && !is_vampshifter(mdef) && !is_demon(mdef->data)
            && !is_mon_shk(mdef) && !is_mon_priest(mdef) && !is_mon_smith(mdef) && !is_mon_npc(mdef) && !is_mon_wiz(mdef))
            return ALLOW_M;
    }
#endif
...
mm_displacement(struct monst *magr, struct monst *mdef)
{
    ...
    if ((pa->mflags3 & M3_DISPLACES) != 0 && (pd->mflags3 & M3_DISPLACES) == 0
        ...
        && !is_mon_trapped(mdef) && (!mdef->wormno || !count_wsegs(mdef))
...
boolean
mon_somewhat_near(struct monst *mon, int x, int y)
{
    int distance = dist2(mon->mx, mon->my, x, y);
...
void
m_detach(struct monst *mtmp, struct permonst *mptr, boolean is_mon_dead)
{
    boolean onmap = (mtmp->mx > 0);

    char bcbuf[BUFSZ];
    Sprintf(bcbuf, "m_detach: mnum=%d (%s), mid=%u, wormno=%u, mx=%d, my=%d, peaceful=%d, tame=%d, moves=%lld", mtmp->mnum, mtmp->data->mname, mtmp->m_id, mtmp->wormno, mtmp->mx, mtmp->my, is_peaceful(mtmp), is_tame(mtmp), (long long)moves);
    debugprint("%s", bcbuf);
...
lifesaved_monster(struct monst *mtmp)
{
    ...
    if (lifesave) 
    {
        ...
        mtmp->mcarrying = 0;
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTODROP, 1);
        if (mtmp->mtame && !is_mon_minion(mtmp)) 
        {
...
void
mondead_with_flags(struct monst *mtmp, uint64_t mondeadflags)
{
    if (!mtmp)
        return;
    ...
    if (is_mon_gd(mtmp) && !grddead(mtmp))
        return;
    ...
    if (mtmp->m_id == quest_status.leader_m_id)
        set_flag(quest_status.bitflags, QSCORE_BITFLAGS_LEADER_IS_DEAD, TRUE);
    ...
    if (is_medusa(mtmp->data))
    {
        ...
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_KILLED_MEDUSA))
            context.botl = 1;
        ...
        set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_KILLED_MEDUSA, 1);
    }
    else if (mtmp->data == &mons[PM_YACC])
    {
        issue_achievement(GUI_ACHIEVEMENT_DEFEATED_YACC);
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_KILLED_YACC))
            context.botl = 1;
        ...
        set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_KILLED_YACC, 1);
    }
    else if (mtmp->data == &mons[PM_DEMOGORGON])
    {
        issue_achievement(GUI_ACHIEVEMENT_DEFEATED_DEMOGORGON);
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_KILLED_DEMOGORGON))
            context.botl = 1;
        ...
        set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_KILLED_DEMOGORGON, 1);
    }
    ...
    if (mtmp->data == &mons[PM_HIGH_PRIEST] && Is_sanctum(&u.uz)
        && is_mon_priest(mtmp) && has_epri(mtmp) && EPRI(mtmp)->shralign == A_NONE)
    {
...
static void
add_knight_slaying_score(struct monst *mtmp)
{
    if (!mtmp)
        return;
...
boolean
corpse_chance(struct monst *mon, struct monst *magr, boolean was_swallowed)
{
    struct permonst *mdat = mon->data;
    int i, tmp;
    ...
    if (((bigmonst(mdat) || mdat == &mons[PM_LIZARD]) && !get_flag(mon->mon_bitflags, MON_BITFLAGS_MCLONED))
        || is_golem(mdat) || is_mplayer(mdat) || is_rider(mdat) || is_mon_shk(mon))
        return TRUE;
...
mongone(struct monst *mdef)
{
    ...
    /* dead vault guard is actually kept at coordinate <0,0> until
       his temporary corridor to/from the vault has been removed */
    if (is_mon_gd(mdef) && !grddead(mdef))
        return;
    if(mdef->iswiz)
...
void
monkilled(struct monst *mdef, const char *fltxt, int adtyp, int xkill_flags)
{
    if (!mdef)
        return;
...
void
killed_by_stoning(struct monst *mtmp)
{
    xkilled(mtmp, XKILL_GIVEMSG | XKILL_STONED);
}

void
killed_by_disintegration(struct monst *mtmp)
{
...
/*
 * Parameters:
 *   xkill_flags: 1: suppress message, 2: suppress corpse, 4: pacifist
 */
/* the player has killed the monster mtmp */
void
xkilled(struct monst *mtmp, int xkill_flags)
{
    if (!mtmp)
        return;
    ...
    if (is_mon_trapped(mtmp) && (t = t_at(x, y)) != 0
        && is_pit(t->ttyp)) 
    {
    ...
    {
        stackobj(mksobj_found_at(SCR_MAIL, x, y, FALSE, FALSE));
    }
    ...
    if (accessible(x, y) || is_pool(x, y)) 
    {
        ...
        if (!rn2(6) && !(mvitals[mndx].mvflags & MV_NOCORPSE)
            ...
            && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_MCLONED) && !mtmp->mrevived)
        {
            otmp = mkobj_with_flags(RANDOM_CLASS, TRUE, TRUE, TRUE, mtmp, MAT_NONE, 0L, 0L, MKOBJ_FLAGS_FOUND_THIS_TURN | (is_lord(mdat) || is_prince(mdat) || (mdat->geno & G_UNIQ) ? MKOBJ_FLAGS_ALSO_RARE : 0UL));
...
cleanup:
    ...
    { /* REAL BAD! */
        ...
        pline_ex(ATR_NONE, get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QCOMPLETED) ? CLR_MSG_WARNING : CLR_MSG_NEGATIVE, "That was %sa bad idea...",
              get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QCOMPLETED) ? "probably " : "");
    } 
...
void
m_into_limbo(struct monst *mtmp, boolean has_effects)
{
    if (!mtmp)
        return;
...
void
mnexto2(struct monst *mtmp, boolean has_effects)
{
    coord mm;
...
/*
 * Parameters:
 *   move_other, has_effects: make sure mtmp gets to x, y! so move m_at(x, y)
 */
/* mnearto()
 ...
 */
int
mnearto(struct monst *mtmp, xchar x, xchar y, boolean move_other, boolean has_effects)
{
...
setmangry(struct monst *mtmp, boolean via_attack)
{
    ...
    if (couldsee(mtmp->mx, mtmp->my)) 
    {
        if (humanoid(mtmp->data) || is_speaking(mtmp->data) || is_mon_shk(mtmp) || is_mon_gd(mtmp))
        {
        ...
        && mtmp->data == &mons[quest_info(MS_LEADER)])
    {
        ...
        for (mon = fmon; mon; mon = mon->nmon) 
        {
            if (DEADMONSTER(mon))
                continue;
            if (mon->data == q_guardian && is_mon_peaceful(mon))
            {
                set_mon_peaceful(mon, 0);
        ...
        && (mtmp->mon_flags & MON_FLAGS_LEVEL_BOSS) != 0)
    {
        ...
        for (mon = fmon; mon; mon = mon->nmon) 
        {
            if (DEADMONSTER(mon))
                continue;
            if ((mon->mon_flags & MON_FLAGS_BOSS_HOSTILITY) != 0 && is_mon_peaceful(mon))
            {
                set_mon_peaceful(mon, 0);
                last_mon = mon;
                newsym(mon->mx, mon->my);
    ...
    if (!context.mon_moving) 
    {
        ...
        for (mon = fmon; mon; mon = mon->nmon) 
        {
                ...
                && !is_blinded(mon) && m_canseeu(mon)) 
            {
                ...
                if (humanoid(mon->data) || is_mon_shk(mon) || is_mon_priest(mon) || is_mon_smith(mon) || is_mon_npc(mon)) 
                {
                    ...
                    else 
                    {
                        ...
                        if (is_mon_shk(mon) || is_mon_priest(mon) || is_mon_smith(mon) || is_mon_npc(mon))
                            continue;
...
wakeup(struct monst *mtmp, boolean via_attack)
{
    ...
    else if (context.forcefight && !context.mon_moving && is_mon_undetected(mtmp))
    {
...
restartcham(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) 
    {
        ...
        if (is_mimic(mtmp->data) && is_mon_sleeping(mtmp)
...
restrap(struct monst *mtmp)
{
    ...
    if (is_cancelled(mtmp) || M_AP_TYPE(mtmp) || cansee(mtmp->mx, mtmp->my)
        ...
        || (is_mon_trapped(mtmp) && (t = t_at(mtmp->mx, mtmp->my)) != 0
            && !is_pit(t->ttyp))
...
hideunder(struct monst *mtmp)
{
    ...
    if (mtmp == u.ustuck) {
        ; /* can't hide if holding you or held by you */
                    ...
                    : (is_mon_trapped(mtmp) && (t = t_at(x, y)) != 0
                       && !is_pit(t->ttyp))) {
...
hide_monst(struct monst *mon)
{
    ...
    if ((is_hider(mon->data) || hider_under)
        && !(is_mon_undetected(mon) || M_AP_TYPE(mon))) {
...
decide_to_shapeshift(struct monst *mon, int shiftflags)
{
    ...
    int mndx, subtype = 0;
    unsigned was_female = is_mon_female(mon);
...
{
    return (is_mon_shk(mon) || is_mon_priest(mon) || is_mon_smith(mon) || is_mon_npc(mon) || is_mon_gd(mon)
            || mon->m_id == quest_status.leader_m_id);
}
...
select_newcham_form(struct monst *mon)
{
    ...
    case PM_SANDESTIN:
        if (rn2(7))
            mndx = pick_nasty(mon->data->difficulty, mon_rndtype(mon));
        break;
    ...
    get_generated_monster_minmax_levels(3, &minmlev, &maxmlev, 0, mon_rndtype(mon));
...
#define mon_poly_ok(idx, maxlvl) (mndx == NON_PM && idx >= LOW_PM && polyok(&mons[idx]) && !(mvitals[idx].mvflags & MV_GONE) && mons[idx].difficulty <= maxlvl && mons[idx].difficulty > mon->data->difficulty)
    ...
    if (mndx == NON_PM && has_polymorph_control(mon))
    {
        ...
        if (maxmlev > mon->data->difficulty)
        {
            ...
            if (mndx == NON_PM)
            {
                testidx = pick_nasty(mon->data->difficulty, mon_rndtype(mon));
                if (mon_poly_ok(testidx, maxmlev))
                    mndx = testidx;
            }
        }
...
mgender_from_permonst(struct monst *mtmp, struct permonst *mdat)
{
    if (is_male(mdat)) {
        ...
        if (!rn2(10))
            set_mon_female(mtmp, !is_mon_female(mtmp));
    }
}

/*
 * Parameters:
 *   polyspot: change is the result of wand or spell of polymorph
 *   msg: "The oldmon turns into a newmon!"
 */
...
void
golemeffects(struct monst *mon, int damtype, double damage)
{
    double heal = 0;
    int slow = 0;
...
/*
 * Parameters:
 *   isfemale: 1 = female, 2 = neuter, transgender or the like
 */
const char* 
pm_monster_name(struct permonst *ptr, uchar isfemale)
{
    if (!ptr)
        return "";
...
const char* mon_monster_name(struct monst *mon)
{
    if (!mon)
        return "";
...
const char* corpse_monster_name(struct obj *corpse)
{
...
const char* pm_female_name(struct permonst *ptr)
{
    if (!ptr)
        return "";
 ...
 */
const char* pm_general_name(struct permonst *ptr, uchar gender)
{
    if (!ptr)
        return "";
...
const char* pm_common_name(struct permonst *ptr)
{
    if (!ptr)
        return "";
...
const char* mon_common_name(struct monst *mon)
{
    if (!mon)
        return "";
...
const char* corpse_common_name(struct obj *corpse)
{
    if (!corpse || !(corpse->otyp == CORPSE || corpse->otyp == STATUE || corpse->otyp == EGG))
        return "";
...
const char* pm_plural_name(struct permonst *ptr, uchar style)
{
    if (!ptr)
        return "";
...
boolean 
is_female_corpse_or_statue(struct obj *corpse)
{
    if (!corpse || !(corpse->otyp == CORPSE || corpse->otyp == STATUE || corpse->otyp == EGG))
        return FALSE;
...
boolean
is_corpse_or_statue_facing_right(struct obj *corpse)
{
    if (!corpse || !(corpse->otyp == CORPSE || corpse->otyp == STATUE || corpse->otyp == EGG))
        return FALSE;
    ...
    if (mtmp)
    {
        return is_mon_facing_right(mtmp);
    }
    else
    {
...
int
count_mon_runeswords(struct monst *mon)
{
    if (!mon || !mon->minvent)
        return 0;
...
static void
save_traits_mon(struct monst *mtmp, boolean save_mextra)
{
    if (is_mon_priest(mtmp))
 ...
 */
static struct monst*
get_saved_traits_mon(struct monst *mtmp, boolean copyof)
{
...
revert_mon_polymorph(struct monst *mtmp, boolean override_mextra, boolean polyspot, boolean msg)
{
    ...
    unsigned int oldfemale = is_mon_female(mtmp);
    ...
    if (has_mmonst(mtmp))
    {
        ...
        if (mtraits)
        {
            ...
            set_mon_female(mtmp, mtraits->female);
