					...
					else if (u_depth >= 5 && u_depth <= 10 && !context.made_orc_and_a_pie2 && !rn2(5))
					{
						...
						if (mon)
						{
							...
							if (!m_carrying(mon, ORCISH_ARROW))
								m_initthrow(mon, ORCISH_ARROW, 10, 12, TRUE, 0);
 ...
 fill_vault:
...

    {
        ...
        if (wizard && nh_getenv("SHOPTYPE"))
            mkroom(SHOPBASE);
		else if (u_depth > depth(&oracle_level) && u_depth <= min(depth(&medusa_level), depth(&oracle_level) + 4) && !context.made_deserted_shop
