...
static void
do_room_or_subroom(struct mkroom *croom, int lowx, int lowy, int hix, int hiy, boolean lit, schar rtype, boolean special, int floortyp, int floorsubtyp, int mtype, int tileset, short decotyp, boolean is_room)
{
    int x, y;
    struct rm *lev;
    ...
    if (!special) 
    {
        for (x = lowx - 1; x <= hix + 1; x++)
            for (y = lowy - 1; y <= hiy + 1; y += (hiy - lowy + 2)) 
            {
                ...
                set_lev_horizontal(x, y, 1); /* For open/secret doors. */
                if (tileset >= 0)
                {
        ...
        for (x = lowx - 1; x <= hix + 1; x += (hix - lowx + 2))
            for (y = lowy; y <= hiy; y++) 
            {
                ...
                levl[x][y].special_quality = 0;
                /* Retain floortype from stone */
                set_lev_horizontal(x, y, 0); /* For open/secret doors. */
                if (tileset >= 0)
                {
                    set_lev_has_special_tileset(x, y, 1);
                    levl[x][y].special_tileset = (uchar)tileset;
        ...
        if (IS_FLOOR(floortyp))
        {
            for (x = lowx; x <= hix; x++)
            {
                ...
                for (y = lowy; y <= hiy; y++)
                {
                    ...
                    lev->special_quality = 0;lev->floortyp = 0; lev->floorsubtyp = 0; lev->floorvartyp = 0;
                    if (tileset >= 0)
                    {
                        set_rm_has_special_tileset(lev, 1);
        ...
        if (is_room) 
        {
            ...
            if (tileset >= 0)
            {
                set_lev_has_special_tileset(lowx - 1, lowy - 1, 1);
                set_lev_has_special_tileset(hix + 1, lowy - 1, 1);
                set_lev_has_special_tileset(lowx - 1, hiy + 1, 1);
                set_lev_has_special_tileset(hix + 1, hiy + 1, 1);
...
void
add_room(int lowx, int lowy, int hix, int hiy, boolean lit, schar rtype, boolean special, int floortyp, int floorsubtyp, int mtype, int tileset, short decotyp)
{
    struct mkroom *croom;
...
void
add_subroom(struct mkroom *proom, int lowx, int lowy, int hix, int hiy, boolean lit, schar rtype, boolean special, int floortyp, int floorsubtyp, int mtype, int tileset, short decotyp)
{
    struct mkroom *croom;
...
join(int a, int b, boolean nxcor)
{
    ...
    if (!dig_corridor(&org, &dest, nxcor, get_flag(level.flags.bitflags, LEVEL_BITFLAGS_ARBOREAL) || get_flag(level.flags.bitflags, LEVEL_BITFLAGS_SWAMPY) ? GRASS : CORR, STONE))
        return;
...
static void
dosdoor(xchar x, xchar y, struct mkroom *aroom, int type, uchar dmask)
{
                ...
                else
                {
                    ...
                    if (!rn2(3) && IS_WALL(levl[xx][yy].typ)) 
                    {
                        ...
                        /* HWALL .horizontal value retained */
                         if (is_lev_horizontal(xx, yy) && isok(xx, yy - 1) && IS_FLOOR(levl[xx][yy - 1].typ))
                        {
...
make_niches(void)
{
    ...
    boolean ltptr = (!get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT) && dep > 15),
            vamp = (dep > 5 && dep < 25);
...
clear_level_structures(void)
{
    ...
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_COURT, 0);set_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_MORGUE, 0); set_flag(level.flags.bitflags, LEVEL_BITFLAGS_GRAVEYARD, 0);
    ...
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_HARDFLOOR, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOMMAP, 0);
    set_hero_memory(1);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_SHORTSIGHTED, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_SOKOBAN_RULES, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_MAZE_LEV, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_CAVERNOUS_LEV, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_ARBOREAL, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_SWAMPY, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_DESERT, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_WIZARD_BONES, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_CORRMAZE, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_MAPPING_DOES_NOT_REVEAL_SPECIAL, 0);
    set_flag(level.flags.bitflags, LEVEL_BITFLAGS_NO_SPECIAL_LEVEL_NAMING_CHECKS, 0);
...
makelevel(void)
{
 ...
 fill_vault:
            ...
            set_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_VAULT, 1);
            ++room_threshold;
            fill_room(&rooms[nroom - 1], FALSE);
            set_room_tileset(&rooms[nroom - 1]);
            mk_knox_portal(vault_x + w, vault_y + h);
            if (!get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT) && !rn2(2))
                makevtele();
 ...
 skip0:
    ...
    for (croom = rooms; croom->hx > 0; croom++) {
        ...
        if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET) || !rn2(3)) 
        {
            x = somex(croom);
            y = somey(croom);
...
mineralize(int kelp_pool, int kelp_moat, int goldprob, int gemprob, boolean skip_lvl_checks)
{
    ...
    if (!skip_lvl_checks
        && (In_hell(&u.uz) || In_V_tower(&u.uz) || Is_really_rogue_level(&u.uz)
            || get_flag(level.flags.bitflags, LEVEL_BITFLAGS_ARBOREAL)
...
#endif
{
    ...
    if ((rtype != OROOM) || do_ordinary)
        ...
        for (x = lowx - 1; x <= hix + 1; x++)
            for (y = lowy - 1; y <= hiy + 1; y += (hiy - lowy + 2)) {
                set_flag(levl[x][y].rm_bitflags, RM_BITFLAGS_EDGE, 1);
                if (levl[x][y].roomno)
...
void
place_branch(branch *br, xchar x, xchar y)
{
    coord m;
    d_level *dest;
    boolean make_stairs;
        ...
        do {
            ...
            else
            {
                ...
                case SLP_GAS_TRAP:
                    ...
                    break;
                case LEVEL_TELEP:
                    if (lvl < MINIMUM_DGN_LEVEL_LEVELTELE || get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT))
                        kind = NO_TRAP;
                    break;
                case SPIKED_PIT:
...
void
mkmodronportal(int subtyp, coord *tm, coord *portal_tm, uint64_t portal_flags)
{
    struct trap* t;
...
void
mkstairs(xchar x, xchar y, char up, struct mkroom *croom, int subtyp)
{
    if (!x) {
...
static void
clear_nearby_fireplaces(xchar x, xchar y)
{
    int i;
    for (i = 0; i < 4; i++)
    {
...
static void
mkaltar(struct mkroom *croom, boolean match_player_alignment)
{
    coord m;
    int tryct = 0;
    aligntyp al;
...
void
create_level_light_sources(void)
{
...
void
maybe_create_location_light_source(xchar x, xchar y)
{
    int lr = get_location_light_range(x, y);
    if (lr != 0)
    {
...
void
create_level_sound_sources(void)
{
    for (xchar x = 1; x < COLNO; x++)
    {
        for (xchar y = 0; y < ROWNO; y++)
        {
            ...
            if (sound_type != GHSOUND_NONE && sound_type < MAX_GHSOUNDS && (levl[x][y].flags & L_INITIALLY_UNLIT) == 0)
            {
                ...
                set_flag(levl[x][y].rm_bitflags, RM_BITFLAGS_MAKINGSOUND, TRUE);
            }
        }
    }
}
...
void
maybe_create_location_sound_source(xchar x, xchar y)
{
    double volume = 0.0;
    enum soundsource_ambient_subtypes subtype = SOUNDSOURCE_AMBIENT_GENERAL;
    enum ghsound_types sound_type = get_location_ambient_sound_type(x, y, &volume, &subtype);
...
void
maybe_create_location_light_and_sound_sources(xchar x, xchar y)
{
...
void
define_level_location_floor_types(boolean fountain_on_grass, boolean fountain_on_ground, boolean tree_on_ground, boolean throne_on_ground)
{
    for (xchar x = 1; x < COLNO; x++)
    {
        for (xchar y = 0; y < ROWNO; y++)
