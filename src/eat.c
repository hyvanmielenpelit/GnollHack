...
static void corpse_pre_effect(int, uchar);
static void givit(int, struct permonst *);
static void temporary_givit(int, int);
static void corpse_after_effect(int, uchar);
...
static void food_pre_effect(struct obj *);
static void food_after_effect(struct obj *);
...
static NEARDATA const char comestibles[] = { FOOD_CLASS, REAGENT_CLASS, 0 };
...
const char*
is_nonedible_corpse_material(struct permonst *ptr, struct obj *obj)
{
    ...
    if (is_metallic(obj))
    {
        ...
        else if (rust_causing_and_ironvorous(ptr) && is_rustprone(obj))
        {
            ...
            //    return is_obj_rknown(obj) ? "rustproof" : "not tasty metal";
            //else
            return (const char*)0;
        }
        else
...
void
display_nutrition_floating_text(int x, int y, int nutr)
{
    if (!isok(x, y))
        return;
...
void
display_famine_floating_text(int x, int y, int nutrloss)
{
    if (!isok(x, y))
        return;
...
unsigned
mon_nutrition_factor(struct obj *otmp, struct monst *mtmp, boolean is_numerator)
{
    if (!mtmp || !otmp)
        return 1U;
...
unsigned
mon_obj_nutrition_value(struct obj *otmp, struct monst *mtmp)
{
    if (!otmp)
        return 0U;
...
int
adjust_nutrition_base_value(int basevalue, struct obj *otmp, struct monst *mtmp)
{
    if (!otmp || !mtmp)
        return basevalue;
    unsigned mult = mon_nutrition_factor(otmp, mtmp, TRUE);
...
unsigned
mon_obj_oeaten_value(struct obj *otmp, struct monst *mtmp)
{
    if (!otmp)
        return 0U;
...
done_eating(boolean message)
{
    ...
    if (carried(piece))
    {
        if (!(piece->speflags & SPEFLAGS_ADDED_TO_YOUR_BILL) && is_obj_unpaid(piece) && costly_spot(u.ux, u.uy))
        {
            struct monst* shkp = 0;
...
int
eat_brains(struct monst *magr, struct monst *mdef, boolean visflag, double *dmg_p)
{
...
static NEARDATA int64_t ate_brains = 0L;
 ...
 */
static void
corpse_pre_effect(int pm, uchar gender)
{
    if (pm < LOW_PM)
        return;
...
static void
temporary_givit(int type, int duration)
{
    if (duration <= 0)
        return;
...
static void
corpse_after_effect(int pm, uchar gender UNUSED)
{
    int tmp = 0;
...
costly_tin(int alter_type)
{
    ...
    if (carried(tin) ? is_obj_unpaid(tin)
                     : (costly_spot(tin->ox, tin->oy) && !is_obj_no_charge(tin))) {
        if (tin->quan > 1L) {
...
/*
 * Parameters:
 *   disp: we're just displaying so leave things alone
 */
static int
tin_variety(struct obj *obj, boolean disp)
{
    int r;
...
consume_tin(const char *mesg)
{
    ...
    if (is_obj_otrapped(tin) || (is_obj_cursed(tin) && r != HOMEMADE_TIN && !rn2(8))) 
    {
...
start_tin(struct obj *otmp)
{
    ...
    else if (is_obj_blessed(otmp)) 
    {
        ...
        tmp = (uwep && is_obj_blessed(uwep) && uwep->otyp == TIN_OPENER) ? 0 : rn2(2);
        if (!tmp)
    ...
    else if (uwep) 
    {
        ...
        case TIN_OPENER:
            ...
            tmp = rn2(is_obj_cursed(uwep) ? 3 : !is_obj_blessed(uwep) ? 2 : 1);
            break;
        case DAGGER:
        case ELVEN_DAGGER:
        case ORCISH_DAGGER:
    ...
    if (!tmp) {
        ...
        set_occupation(opentin, "opening the tin", ATR_NONE, CLR_MSG_ATTENTION, objects[otmp->otyp].oc_soundset, OCCUPATION_OPENING_TIN, OCCUPATION_SOUND_TYPE_START, 0);
    }
    return;
}
...
static void
start_eating(struct obj *otmp, boolean resume)
{
 ...
 */
static void
food_pre_effect(struct obj *otmp)
{
    switch (otmp->otyp) {
    case FOOD_RATION:
        if (u.uhunger <= 200)
...
static void
food_after_effect(struct obj *otmp)
{
...
int64_t
get_rotted_status(struct obj *obj)
{
...
int
magic_nutrition(struct obj *otmp)
{
    if (!otmp)
        return 0;
...
boolean
is_obj_vegan_food(struct obj *otmp)
{
    if (!otmp)
        return TRUE;
...
int
check_vegan_food(struct obj *otmp)
{
...
doeat(void)
{
    ...
    if (!is_obj_normally_edible(otmp)) 
    {
        ...
        if (otmp->oclass == WEAPON_CLASS && is_obj_opoisoned(otmp)) 
        {
            play_occupation_immediate_sound(objects[otmp->otyp].oc_soundset, OCCUPATION_EATING, OCCUPATION_SOUND_TYPE_START);
            pline_ex(ATR_NONE, CLR_MSG_NEGATIVE, "Ecch - that must have been poisonous!");
            if (!Poison_resistance) 
...
void
update_hunger_status(boolean incr)
{
    unsigned newhs;
    int h = u.uhunger;
 ...
 */
struct obj *
floorfood(const char *verb, int corpsecheck)
{
    struct obj *otmp;
    char qbuf[QBUFSZ];
    char c;
...
static
int get_corpse_reqtime(struct obj *otmp)
{
...
static
int get_food_nmod(struct obj *otmp, int reqtime)
{
    if (!otmp)
        return 0;
