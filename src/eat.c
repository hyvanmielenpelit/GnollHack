...
struct obj *otmp;
{
    ...
    if (objects[otmp->otyp].oc_edible_subtype == EDIBLETYPE_HALLUCINATING || objects[otmp->otyp].oc_edible_subtype == EDIBLETYPE_SICKENING || stoneorslime || (cadaver && (has_sickening_corpse(&mons[mnum]) || has_mummy_rotted_corpse(&mons[mnum])) && !Sick_resistance))
    {
    ...
    if (newhs == FAINTING) {
        ...
        if (u.uhs <= WEAK || rn2(20 - uhunger_div_by_10) >= 19) {
            if (!is_fainted() && multi >= 0 /* %% */) {
                ...
                stop_occupation();
                You_ex(ATR_NONE, CLR_RED, "faint from lack of food.");
                incr_itimeout(&HDeaf, duration);
                context.botl = TRUE;
                play_environment_ambient_sounds();
                nomul(-duration);
    ...
    if (newhs != u.uhs) {
        ...
        case HUNGRY:
            if (Hallucination) {
                ...
                You_ex(ATR_NONE, CLR_YELLOW, (!incr) ? "only feel hungry now."
                            : (u.uhunger < 145)
                                  ? "feel hungry."
