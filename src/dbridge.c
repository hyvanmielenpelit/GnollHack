...
boolean flag;
{
    ...
    full_location_transform(x, y, lev1typ, 0, 0, lev1flags, 0, 0, levl[x][y].floortyp, levl[x][y].floorsubtyp, levl[x][y].floorvartyp, FALSE, lev1horizontal, 0, 0, FALSE);
    full_initial_location_transform(x2, y2, lev2typ, lev2flags, 0, 0, ROOM, FALSE, lev2horizontal, 0, 0, FALSE);
...
int x, y;
{
    ...
    full_location_transform(x, y, DRAWBRIDGE_UP, lev1->subtyp, lev1->vartyp, lev1->flags, 0, 0, lev1->floortyp, lev1->floorsubtyp, lev1->floorvartyp, lev1->facing_right, lev1->horizontal, 0, 0, FALSE);
    ...
    full_location_transform(x2, y2, DBWALL, 0, 0, W_NONDIGGABLE, 0, 0, lev2->floortyp, lev2->floorsubtyp, lev2->floorvartyp, lev2->facing_right, lhoriz, 0, 0, FALSE);
...
int x, y;
{
    ...
    full_location_transform(x, y, DRAWBRIDGE_DOWN, lev1->subtyp, lev1->vartyp, lev1->flags, 0, 0, lev1->floortyp, lev1->floorsubtyp, lev1->floorvartyp, lev1->facing_right, lev1->horizontal, 0, 0, FALSE);
    lev2 = &levl[x2][y2];
    full_location_transform(x2, y2, DOOR, 0, 0, D_PORTCULLIS, 0, 0, lev2->floortyp, lev2->floorsubtyp, lev2->floorvartyp, lev2->facing_right, lev2->horizontal, 0, 0, FALSE);
...
boolean is_disintegrated;
{
    ...
    else
    {
        ...
        full_location_transform(x, y, trtyp, trsubtyp, trvartyp, ((lev1->drawbridgemask & DB_ICE) ? ICED_MOAT : 0), 0, 0, lev1->floortyp, lev1->floorsubtyp, lev1->floorvartyp, FALSE, FALSE, 0, 0, FALSE);
    }
    wake_nearto(x, y, 500);
    full_location_transform(x2, y2, DOOR, 0, 0, D_PORTCULLIS, 0, 0, lev2->floortyp, lev2->floorsubtyp, lev2->floorvartyp, lev2->facing_right, lev2->horizontal, 0, 0, FALSE);
    if ((t = t_at(x, y)) != 0)
