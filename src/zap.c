...
boolean stop_at_first_hit_object;
{
    ...
    while (range-- > 0)
	{
        ...
        if ((weapon != ZAPPED_WAND || (weapon == ZAPPED_WAND && displayedobjtype != STRANGE_OBJECT)) && weapon != INVIS_BEAM)
		{
            ...
            {                
                show_missile_info(bhitpos.x, bhitpos.y, obj->opoisoned, obj->elemental_enchantment, obj->exceptionality, obj->mythic_quality, obj->oeroded, obj->oeroded2, get_missile_flags(obj, tethered_weapon));
                if (tethered_weapon)
