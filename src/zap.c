...
bhitm(struct monst *mtmp, struct obj *otmp, struct monst *origmonst)
{
    ...
    case POT_POLYMORPH:
        ...
        else if (!check_magic_resistance_and_inflict_damage(mtmp, otmp, origmonst, FALSE, 0, 0, NOTELL))
        {
            ...
            else if (newcham_ex(mtmp, (struct permonst *) 0, 0, polyspot, give_msg, get_obj_polymorph_duration(otmp), FALSE, FALSE, FALSE, FALSE) != 0
    ...
    case SPE_CANCELLATION:
        ...
        if ((mtmp->cham != NON_PM || has_mmonst(mtmp)) && !has_unchanging(mtmp))
            revert_mon_polymorph(mtmp, FALSE, TRUE, TRUE, TRUE);
