...
bhitm(struct monst *mtmp, struct obj *otmp, struct monst *origmonst)
{
    ...
    case SPE_TELEPORT_MONSTER:
        res = 1;
        debugprint("bhitm teleport: mnum=%d, isshk=%d", mtmp->mnum, (int)is_mon_shk(mtmp));
        if (disguised_mimic)
            seemimic(mtmp);
...
get_mon_location(struct monst *mon, xchar *xp, xchar *yp, int locflags)
{
    if (mon == &youmonst) {
        ...
        *yp = u.uy;
        return TRUE;
    } else if (mon->mx > 0 && (!is_mon_buried(mon) || locflags)) {
        *xp = mon->mx;
        *yp = mon->my;
        return TRUE;
    } else { /* migrating or buried */*xp = 0; *yp = 0;
        return FALSE;
    }
}
...
montraits(struct obj *obj, coord *cc, boolean adjacentok, int mnum_override, int mnum_replaceundead, uint64_t mmflags)
{
    ...
    if (mtmp2) 
    {
        ...
        if (mtmp->m_id) {
            ...
            if (get_flag(quest_status.bitflags, QSCORE_BITFLAGS_LEADER_IS_DEAD)
                /* leader_is_dead implies leader_m_id is valid */
                && mtmp2->m_id == quest_status.leader_m_id)
                set_flag(quest_status.bitflags, QSCORE_BITFLAGS_LEADER_IS_DEAD, FALSE);
        }
        mtmp2->mx = mtmp->mx;
        mtmp2->my = mtmp->my;
        mtmp2->mux = mtmp->mux;
        ...
        set_mon_fleeing(mtmp2, is_mon_fleeing(mtmp));
        set_mon_buried(mtmp2, is_mon_buried(mtmp));
        set_mon_undetected(mtmp2, is_mon_undetected(mtmp));
        mtmp2->mflee_timer = mtmp->mflee_timer;
        mtmp2->mlstmv = mtmp->mlstmv;
        mtmp2->m_ap_type = mtmp->m_ap_type;
        set_flag(mtmp2->mon_bitflags, MON_BITFLAGS_LEAVES_NO_CORPSE, get_flag(mtmp->mon_bitflags, MON_BITFLAGS_LEAVES_NO_CORPSE));
        set_flag(mtmp2->mon_bitflags, MON_BITFLAGS_DELAYED_KILLER_BY_YOU, get_flag(mtmp->mon_bitflags, MON_BITFLAGS_DELAYED_KILLER_BY_YOU));
        ...
        mtmp2->meating = 0;
        set_mon_leashed(mtmp2, 0);
        set_mon_trapped(mtmp2, 0);
        set_mon_sleeping(mtmp2, 0);
        mtmp2->mfrozen = 0;
        set_mon_canmove(mtmp2, 1);
...
revive(struct obj *corpse, boolean by_hero, int animateintomon, boolean replaceundead)
{
    ...
    if (!x || !y
         ...
         */
        || (container && (is_obj_locked(container) || container_nesting > 2
    ...
    else if (has_omonst(corpse))
    {
        ...
        if (mtmp && mtmp->mtame && !is_mon_minion(mtmp) && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISFAITHFUL))
            wary_dog(mtmp, TRUE);
    }
    else 
    {
    ...
    */
    ...
    if (animateintomon >= 0)
    {
        ...
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_DISREGARDS_ENEMY_STRENGTH, TRUE);
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_DISREGARDS_OWN_HEALTH, TRUE);
    }
...
obj_resists(struct obj *obj, int ochance, int achance)
{
    ...
    {
        int roll = rn2(100);
        return (boolean) (roll < (obj->oartifact ? achance : ochance));
    }
}
...
poly_obj(struct obj *obj, int id)
{
    ...
    { /* preserve symbol */
        ...
        if (obj->otyp == UNICORN_HORN && is_obj_broken(obj))
            magic_obj = 0;
...
#undef USES_CORPSENM
    ...
    if (erosion_matters(otmp)) 
    {
        ...
        if (is_damageable(otmp))
            set_obj_erodeproof(otmp, is_obj_erodeproof(obj));
    }

    /* Keep chest/box traps and poisoned ammo if we may */
    if (is_obj_trapped(obj) && Is_box(otmp))
        set_obj_trapped(otmp, TRUE);

    if (is_obj_trapped(obj) && is_poisonable(otmp))
        set_obj_trapped(otmp, TRUE);
...
zapnodir(struct obj *obj)
{
    ...
    case SPE_RAISE_SKELETON_LORD:
        mtmp = summoncreature(obj->otyp, PM_SKELETON_LORD, "%s raises before you.", MM_UNDEAD_SUMMON_ANIMATION | MM_NO_MONSTER_INVENTORY,
            SUMMONCREATURE_FLAGS_CAPITALIZE | SUMMONCREATURE_FLAGS_DISREGARDS_STRENGTH | SUMMONCREATURE_FLAGS_DISREGARDS_HEALTH | SUMMONCREATURE_FLAGS_FAITHFUL);
        break;
...
bhit(int ddx, int ddy, int range, int radius, enum bhit_call_types weapon, int (*fhitm)(struct monst*, struct obj*, struct monst*), int (*fhito)(struct obj*, struct obj*, struct monst*), int (*fhitt)(struct trap*, struct obj*, struct monst*), struct obj **pobj, struct monst *origmonst, uchar hit_only_one, boolean stop_at_first_hit_object)
{
    ...
    while (range-- > 0)
    {
        ...
        if ((weapon != ZAPPED_WAND || (weapon == ZAPPED_WAND && displayedobjtype != STRANGE_OBJECT)) && weapon != INVIS_BEAM)
        {
            ...
            tmp_at(bhitpos.x, bhitpos.y);
            if (obj && ((is_poisonable(obj) && is_obj_trapped(obj)) || obj->material != objects[obj->otyp].oc_material || obj->special_quality != 0 || obj->elemental_enchantment || obj->exceptionality || obj->mythic_prefix || obj->mythic_suffix || obj->oeroded || obj->oeroded2 || tethered_weapon || get_obj_height(obj) > 0 || is_obj_lamplit(obj)))
            {                
                show_missile_info(bhitpos.x, bhitpos.y, is_obj_trapped(obj), obj->material, obj->special_quality, obj->elemental_enchantment, obj->exceptionality, obj->mythic_prefix, obj->mythic_suffix, obj->oeroded, obj->oeroded2, get_missile_flags(obj, tethered_weapon), get_obj_height(obj), 0, 0);
                if (tethered_weapon)
        ...
        if (is_pool(x, y) || is_lava(x, y)) 
        {
            ...
            else 
            {
                ...
                else if ((mon = m_at(x, y)) != 0) 
                {
                    ...
                    if (is_swimmer(mon->data) && is_mon_undetected(mon)) 
                    {
...
//        def_case:
    ...
    if ((mon = m_at(x, y)) != 0) 
    {
        ...
        {
            setmangry(mon, TRUE);
            if (is_mon_priest(mon) && *in_rooms(mon->mx, mon->my, TEMPLE))
                ghod_hitsu(mon);
            if (is_mon_shk(mon) && !*u.ushops)
                hot_pursuit(mon);
        }
    }
    return rangemod;
...
fracture_rock(struct obj *obj, boolean verbose)
{
    ...
    obj->owt = weight(obj);set_obj_dknown(obj, 0); set_obj_bknown(obj, 0); set_obj_rknown(obj, 0); set_obj_nknown(obj, 0);
...
destroy_one_item(struct obj *obj, int osym, int dmgtyp, boolean forcedestroy)
{
    ...
    if (!skip) 
    {
        if (is_obj_in_use(obj))
            --quan; /* one will be used up elsewhere */for (i = 0L, cnt = 0L; i < quan; i++)
...
uint64_t scflags;
{
    ...
    if (mon)
    {
        set_flag(mon->mon_bitflags, MON_BITFLAGS_ISSUMMONED, markassummoned);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_DISREGARDS_ENEMY_STRENGTH, disregardstrength);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_DISREGARDS_OWN_HEALTH, disregardhealth);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_HASBLOODLUST, bloodlust);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_ISPACIFIST, pacifist);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_ISPROTECTOR, protector);

        if(!get_flag(mon->mon_bitflags, MON_BITFLAGS_ISFAITHFUL))
            set_flag(mon->mon_bitflags, MON_BITFLAGS_ISFAITHFUL, faithful);
...
summondemon(int spl_otyp)
{
    ...
    if (mon)
    {
        ...
        set_flag(mon->mon_bitflags, MON_BITFLAGS_DISREGARDS_ENEMY_STRENGTH, TRUE);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_DISREGARDS_OWN_HEALTH, FALSE);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_HASBLOODLUST, TRUE);
...
summondemogorgon(int spl_otyp)
{
    ...
    if (mon)
    {
        ...
        {
            (void)tamedog(mon, (struct obj*)0, TAMEDOG_FORCE_ALL, FALSE, 0, FALSE, FALSE);
            //set_mon_peaceful(mon, TRUE);
            //set_flag(mon->mon_bitflags, MON_BITFLAGS_ISPROTECTOR, TRUE);
        }
        mon->summonduration = d(objects[spl_otyp].oc_spell_dur_dice, objects[spl_otyp].oc_spell_dur_diesize) + objects[spl_otyp].oc_spell_dur_plus;
        begin_summontimer(mon);
        if (!Blind)
...
summonbahamut(int spl_otyp UNUSED)
{
    ...
    if (mon)
    {
        ...
        set_flag(mon->mon_bitflags, MON_BITFLAGS_DISREGARDS_ENEMY_STRENGTH, TRUE);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_DISREGARDS_OWN_HEALTH, FALSE);
        set_flag(mon->mon_bitflags, MON_BITFLAGS_HASBLOODLUST, FALSE);
        if (!Blind)
