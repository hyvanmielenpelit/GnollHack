...
static boolean trading_items(const char*);
...
static void add_inventory_menu_item(winid, struct obj*, char, boolean, boolean, int, boolean, boolean, boolean, int*);
static boolean is_special_show_condition_satisfied(int);
...
loot_xname(struct obj *obj)
{
    ...
    saveo.blessed = is_obj_blessed(obj), saveo.cursed = is_obj_cursed(obj);
    ...
    if (obj->oclass == POTION_CLASS) {
        ...
        if (obj->otyp == POT_WATER)
            set_obj_blessed(obj, saveo.blessed), set_obj_cursed(obj, saveo.cursed);
    }
    if (obj->otyp == TOWEL) {
...
sortloot_cmp(const genericptr vptr1, const genericptr vptr2)
{
    ...
    nam1 = sli1->str;
    if (!nam1)
        nam1 = dupstr(loot_xname(obj1; sli1->str = dupstr(loot_xname(obj1));
    nam2 = sli2->str;
    if (!nam2)
        nam2 = dupstr(loot_xname(obj2; sli2->str = dupstr(loot_xname(obj2));
    ...
    val1 = is_obj_rknown(obj1) && is_obj_erodeproof(obj1);
    val2 = is_obj_rknown(obj2) && is_obj_erodeproof(obj2);
    if (val1 != val2)
        return val2 - val1; /* bigger is better */
...
/*
 * Parameters:
 *   olist: previous version might have changed *olist, we don't
 *   mode: flags for sortloot_cmp()
 *   by_nexthere: T: traverse via obj->nexthere, F: via obj->nobj
 */
/*
 ...
 */
Loot *
sortloot(struct obj **olist, unsigned mode, boolean by_nexthere, boolean (*filterfunc)(struct obj*))
{
    Loot *sliarray;
    struct obj *o;
    unsigned n, i;
 ...
 */
void
sortloot(struct obj **olist, unsigned mode, boolean by_nexthere)
{
...
merged(struct obj **potmp, struct obj **pobj)
{
    ...
    if (mergable(otmp, obj)) {
        ...
        if (!has_uoname(otmp) &&has_uoname(obj))
            otmp = uoname(otmp; *potmp = uoname(otmp, UONAME(obj));
        if (!has_oname(otmp) &&has_oname(obj))
            otmp = oname(otmp; *potmp = oname(otmp, ONAME(obj));
        debugprint("merged");
...
addinv_core1(struct obj *obj)
{
    ...
    {
        if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET) && no_wizard_or_debug)
            impossible("already have amulet?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET, 1);
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_AMULET))
            context.botl = 1;
        if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_AMULET))
        {
            ...
            custompline_ex_prefix(ATR_NONE, CLR_MSG_HINT, "QUEST UPDATE", ATR_NONE, NO_COLOR, " - ", ATR_BOLD, CLR_MSG_HIGHLIGHT, 0, "Sacrifice the Amulet of Yendor on the Astral Plane");
            if (iflags.using_gui_sounds)
            {
                delay_output_milliseconds(500);
                play_sfx_sound(SFX_HINT);
        ...
        set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_AMULET, 1);
    }
    else if (obj->otyp == CANDELABRUM_OF_INVOCATION) 
    {
        if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH) && no_wizard_or_debug)
            impossible("already have candelabrum?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH, 1);
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH))
            context.botl = 1;
        if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH))
        {
            ...
            set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH, 1);
            invocation_ritual_quest_update(TRUE);
        }
    }
    else if (obj->otyp == BELL_OF_OPENING)
    {
        if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL) && no_wizard_or_debug)
            impossible("already have silver bell?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL, 1);
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL))
            context.botl = 1;
        if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL))
        {
            ...
            set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL, 1);
            invocation_ritual_quest_update(TRUE);
        }
    }
    else if (obj->otyp == SPE_BOOK_OF_THE_DEAD)
    {
        if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BOOK) && no_wizard_or_debug)
            impossible("already have the book?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BOOK, 1);
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK))
            context.botl = 1;
        if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK))
        {
            ...
            set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK, 1);
            invocation_ritual_quest_update(TRUE);
        }
    } 
    else if (obj->oartifact) 
    {
        ...
        {
            if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_QUESTART) && no_wizard_or_debug)
                impossible("already have quest artifact?");
            set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_QUESTART, 1);
            artitouch(obj);
        }
        else if (obj->otyp == SPE_BOOK_OF_MODRON)
        {
            if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_PRIME_CODEX) && no_wizard_or_debug)
                impossible("already have Prime Codex?");
            set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_PRIME_CODEX, 1);
            if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_PRIME_CODEX))
                context.botl = 1;
            ...
            set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_PRIME_CODEX, 1);
        }
        ...
        if (Role_if(PM_BARBARIAN))
        {
            ...
            if (u.ualignbase[A_ORIGINAL] == A_CHAOTIC)
            {
                if (obj->oartifact == ART_STORMBRINGER)
                    set_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ROLE_ACHIEVEMENT_1, 1);
                if (obj->oartifact == ART_MOURNBLADE)
                    set_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ROLE_ACHIEVEMENT_2, 1);
            }
            else
            {
        ...
        if (check_achievement && get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ROLE_ACHIEVEMENT_1) && get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ROLE_ACHIEVEMENT_2) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT))
        {
    ...
    {
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MINES_LUCKSTONE))
            context.botl = 1;
        ...
        set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MINES_LUCKSTONE, 1);
        obj->speflags &= ~(SPEFLAGS_MINES_PRIZE);
        //set_obj_nomerge(obj, 0);
    }
    else if (is_soko_prize(obj)) 
    {
        if (flags.showscore && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_FINISH_SOKOBAN))
            context.botl = 1;
        ...
        set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_FINISH_SOKOBAN, 1);
...
void
play_hint_sound_with_delay(boolean dodelay)
{
    if (iflags.using_gui_sounds)
    {
        if (dodelay)
...
{
    if ((get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_HEARD_OF_INVOCATION_RITUAL)) && !get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED))
    {
        if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH))
        {
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK))
        {
            play_hint_sound_with_delay(dodelay);
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH))
        {
            play_hint_sound_with_delay(dodelay);
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH))
        {
            play_hint_sound_with_delay(dodelay);
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH))
        {
            play_hint_sound_with_delay(dodelay);
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK))
        {
            play_hint_sound_with_delay(dodelay);
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL))
        {
            play_hint_sound_with_delay(dodelay);
        ...
        else if (get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN))
        {
            play_hint_sound_with_delay(dodelay);
            if (!get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_UVIBRATED))
...
/*
 * Parameters:
 *   otmp: object to be identified if any state change happens
 */
void
update_all_character_properties(struct obj *otmp, boolean verbose)
{
...
addinv(struct obj *obj)
{
    ...
    obj_was_thrown = get_flag(obj->obj_bitflags, OBJ_BITFLAGS_WAS_THROWN);
    set_flag(obj->obj_bitflags, OBJ_BITFLAGS_WAS_THROWN, 0);       /* not meaningful for invent */
 ...
 */
struct obj *
hold_another_object(struct obj *obj, const char *drop_fmt, const char *drop_arg, const char *hold_msg, boolean doprinv)
{
    char buf[BUFSZ];
...
freeinv_core(struct obj *obj)
{
    ...
    else if (obj->otyp == AMULET_OF_YENDOR) 
    {
        ...
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET, 0);
    }
    else if (obj->otyp == CANDELABRUM_OF_INVOCATION) 
    {
        if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH) && no_wizard_or_debug)
            impossible("don't have candelabrum?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH, 0);
    }
    else if (obj->otyp == BELL_OF_OPENING)
    {
        if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL) && no_wizard_or_debug)
            impossible("don't have silver bell?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL, 0);
    } 
    else if (obj->otyp == SPE_BOOK_OF_THE_DEAD)
    {
        if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BOOK) && no_wizard_or_debug)
            impossible("don't have the book?");
        set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BOOK, 0);
    } 
    else if (obj->oartifact) 
    {
        if (is_quest_artifact(obj)) 
        {
            ...
            set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_QUESTART, 0);
        }
        else if (obj->otyp == SPE_BOOK_OF_MODRON)
        {
            if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_PRIME_CODEX) && no_wizard_or_debug)
                impossible("don't have Prime Codex?");
            set_flag(u.uhave.bitflags, UHAVE_BITFLAGS_PRIME_CODEX, 0);
        }
    }
...
void
delobj_with_flags(struct obj *obj, uint64_t newsym_flags)
{
    boolean update_map;
...
struct obj*
noncursed_sobj_at(int otyp, int x, int y)
{
    struct obj* otmp;
...
/* try to find a particular type of object at designated map location */
struct obj*
any_obj_at(int otyp, int x, int y)
{
    if (!isok(x, y))
        return (struct obj*)0;
...
struct obj*
otyp_in_objchn(int otyp, struct obj *objchn)
{
    struct obj* otmp;
...
struct obj *
carrying_fitting_unlocking_tool_for_box(struct obj *box)
{
    if (!box)
        return (struct obj*)0;
...
struct obj*
carrying_fitting_unlocking_tool_for_door(struct rm *door)
{
    if (!door)
        return (struct obj*)0;
...
struct obj*
o_on_memory(unsigned int id, struct obj *objchn)
{
    struct obj* temp;
...
struct obj*
m_on_memory(unsigned int mid, struct obj *objchn)
{
    struct obj* temp;
...
void
reduce_item_cooldown(struct obj *objchn)
{
...
static boolean
trading_items(const char *action)
{
 ...
 */
struct obj*
getobj(const char *let, const char *word, int show_weights, const char *headertext)
{
...
/*
 * Parameters:
 *   getobjflags: 1 = cost is specified; 2 = header text is about cost
 */
struct obj *
getobj_ex(const char *let, const char *word, int show_weights, boolean show_quick, const char *headertext, boolean (*validitemfunc)(struct obj*), int64_t cost, unsigned int getobjflags)
{
    struct obj *otmp;
    char ilet = 0;
...
/* clang-format on */
    ...
    *ap = '\0';
    ...
    if (!foo && !allowall && !allownone) 
    {
        ...
        if(writeendremark)
            pline_ex(ATR_NONE, CLR_MSG_HINT, "(You cannot write on unidentified blank %s.)", unidbuf);
        return (struct obj *) 0;
    }
...
void
construct_getobj_letters(const char *let, const char *word, boolean (*validitemfunc)(struct obj*), char *lets, char *altlets, char *buf, size_t lets_size UNUSED, size_t altlets_size, size_t buf_size, int *foo_ptr, xchar *foox_ptr, char **bp_ptr, xchar *allowcnt_ptr, boolean *usegold_ptr, boolean *allowall_ptr, boolean *allownone_ptr, boolean *useboulder_ptr, struct obj *otmp_only)
{
    struct obj* otmp;
            ...
            || (useboulder && otmp->otyp == BOULDER))
        {
            ...
            if (
                ...
                || (!strcmp(word, "set as quick bag") /* exclude if already a quick bag */
                    && (otmp->o_id == context.quick_bag_obj_oid || !Is_container(otmp) || (Is_box(otmp) && (!is_obj_lknown(otmp) || is_obj_locked(otmp))) || (!objects[otmp->otyp].oc_name_known && !is_obj_cknown(otmp)) || (objects[otmp->otyp].oc_name_known && !Is_proper_container(otmp)) || Is_specialized_container(otmp) || Is_container_with_closed_lid(otmp)))
...
boolean
acceptable_getobj_obj(struct obj *otmp, const char *let, const char *word)
{
    if (!otmp || !let || !word)
        return FALSE;
...
boolean
is_worn_correctly(struct obj *otmp)
{
    if (otmp->owornmask == 0)
        return FALSE;
...
/*
 * Parameters:
 *   combo, show_quick: combination menu flag
 */
...
/*
 * Parameters:
 *   objchn: *objchn might change
 *   olets, word: olets is an Obj Class char array
 */
/*
 ...
 */
int
askchain(struct obj **objchn, const char *olets, int allflag, int (*fn)(struct obj*), int (*ckfn)(struct obj*), int mx, const char *word)
{
...
int
count_unidentified(struct obj *objchn, boolean (*filterfunc)(struct obj*), boolean bynexthere)
{
    int unid_cnt = 0;
    struct obj *obj;
...
int
count_objects_in_class(struct obj *objchn, char oclass, boolean (*filterfunc)(struct obj*), boolean bynexthere)
{
    int cnt = 0;
    struct obj* obj;
    for (obj = objchn; obj; obj = (bynexthere ? obj->nexthere : obj->nobj))
...
int
count_unknown(struct obj *objchn, boolean (*filterfunc)(struct obj*), boolean bynexthere)
{
    int unid_cnt = 0;
    struct obj* obj;
...
/*
 * Parameters:
 *   id_limit: 0 = all, >0 max no of items identified
 *   learning_id: true if we just read unknown identify scroll
 */
/* dialog with user to identify a given number of items; 0 means all */
/* returns the number of items identified */
int
identify_pack(int id_limit, boolean learning_id)
{
    struct obj *obj;
 ...
 */
void
prinv(const char *prefix, struct obj *obj, int64_t quan)
{
    //if (!prefix)
    //    prefix = "";
...
void
prinvc(const char *prefix, struct obj *obj, int64_t quan)
{
    prinv_ex(prefix, obj, quan, ATR_NONE, CLR_MSG_HINT, ATR_NONE, NO_COLOR, TRUE, FALSE);
}

void
prinv_ex(const char *prefix, struct obj *obj, int64_t quan, int prefix_attr, int prefix_color, int attr, int color, boolean apply_menucolor, boolean apply_to_separator)
{
    if (!prefix)
        prefix = "";
...
/*
 * Parameters:
 *   txt: text to print instead of obj
 *   let: inventory letter
 *   dot: append period; (dot && cost => Iu)
 *   cost: cost (for inventory of unpaid or expended items)
 *   quan: if non-0, print this quantity, not obj->quan
 */
char *
xprname(struct obj *obj, const char *txt, char let, boolean dot, int64_t cost, int64_t quan)
{
#ifdef LINT /* handle static char li[BUFSZ]; */
    char li[BUFSZ];
#else
...
int
display_item_command_menu_by_invlet(char invlet, int64_t pickcnt, boolean *return_to_inv_ptr)
{
    struct obj* otmp = 0;
    struct obj* otmp2;
 ...
 */
static boolean
is_special_show_condition_satisfied(int i)
{
...
int
display_item_command_menu(struct obj *otmp, int64_t pickcnt, boolean *return_to_inv_ptr)
{
    if (!otmp)
        return 0;
...
static void
add_inventory_menu_item(winid win, struct obj *otmp, char ilet, boolean wizid, boolean loadstonecorrectly, int show_weights, boolean show_quick, boolean want_reply, boolean comparison_stats, int *wtcount_ptr)
{
...
/*
 * Parameters:
 *   xtra_choice: "fingers", pick hands rather than an object
 */
/*
 ...
 */
static char
display_pickinv(const char *lets, char *xtra_choice, char *query, boolean want_reply, int64_t *out_cnt, int show_weights, boolean show_quick, const char *headertext, uchar addinventoryheader, boolean wornonly)
{
...
void
add_weight_summary(winid win, int total_ounce_weight, int show_weights)
{
...
int
enclevelminimumweight(enum encumbrance_types enclevel)
{
    int weight;
    int wt = weight_cap();
...
int
enclevelmaximumweight(enum encumbrance_types enclevel)
{
    int weight;
    int wt = weight_cap();
...
void
add_weight_summary_putstr(winid win, int total_ounce_weight, int show_weights)
{
    if (show_weights > SHOWWEIGHTS_NONE)
    {
        char buf[BUFSZ];
 ...
 */
char
display_inventory(const char *lets, boolean want_reply, int show_weights, boolean show_quick)
{
...
char
display_inventory_with_header(const char *lets, boolean want_reply, int64_t *out_cnt, int show_weights, boolean show_quick, boolean isequipment, boolean wornonly)
{
    return display_pickinv(lets, (char*)0, (char*)0,
 ...
 */
int
count_unpaid(struct obj *list, boolean (*filterfunc)(struct obj*), boolean bynexthere)
{
    int count = 0;
    struct obj* otmp;
...
int
count_objects(struct obj *list, boolean (*filterfunc)(struct obj*), boolean bynexthere)
{
    int count = 0;
 ...
 */
int
count_buc(struct obj *list, int type, boolean (*filterfunc)(struct obj*), boolean bynexthere)
{
    int count = 0;
...
void
tally_BUCX(struct obj *list, boolean by_nexthere, int *bcp, int *ucp, int *ccp, int *xcp, int *ocp, int *tcp)
{
    /* Future extensions:
...
int64_t
count_contents(struct obj *container, boolean nested, boolean quantity, boolean everything)
{
...
/* count everything inside a container, or just shop-owned items inside */
int64_t
count_contained_contents(struct obj *container, boolean nested, boolean quantity, boolean everything)
{
    struct obj* otmp, * topc;
    boolean shoppy = FALSE;
    int64_t count = 0L;
...
dounpaid(void)
{
    ...
    count = count_unpaid(invent, 0, FALSE);otmp = (struct obj *; marker = (struct obj *; contnr = (struct obj *) 0;
...
dfeature_at(int x, int y)
{
    ...
    else if (IS_BRAZIER(ltyp))
    {
        ...
        /*
        Sprintf(altbuf, "%s%s", is_rm_lamplit(lev) ? "lit " : "unlit ",
...
const char*
get_floor_doodad_explanation_at(int x, int y)
{
...
/*
 * Parameters:
 *   obj_cnt: obj_cnt > 0 implies that autopickup is in progress
 */
/* look at what is here; if there are many objects (pile_limit or more),
   don't show them unless obj_cnt is 0 */
int
look_here(int obj_cnt, boolean picked_some, boolean explicit_cmd)
{
    struct obj *otmp;
    struct trap *trap;
...
void
print_things_here_to_window(void)
{
...
mergable(struct obj *otmp, struct obj *obj)
{
    ...
    if (is_obj_unpaid(obj) != is_obj_unpaid(otmp) || obj->enchantment != otmp->enchantment || obj->elemental_enchantment != otmp->elemental_enchantment
        ...
        || is_obj_no_charge(obj) != is_obj_no_charge(otmp) || is_obj_broken(obj) != is_obj_broken(otmp)
        || is_obj_trapped(obj) != is_obj_trapped(otmp) || is_obj_lamplit(obj) != is_obj_lamplit(otmp)
    ...
    if ((obj->oclass == WEAPON_CLASS || obj->oclass == ARMOR_CLASS)
        && (is_obj_erodeproof(obj) != is_obj_erodeproof(otmp)
 ...
 */
void
useupf(struct obj *obj, int64_t numused)
{
    useupf_with_flags(obj, numused, 0UL);
}

void
useupf_with_flags(struct obj *obj, int64_t numused, uint64_t newsym_flags)
{
    struct obj *otmp;
 ...
 */
static NEARDATA const char *names[MAX_OBJECT_CLASSES] = {
...
static NEARDATA size_t invbufsiz = 0;

/*
 * Parameters:
 *   prefix_style: 1 = unpaid, 2 = unidentified
 */
char *
let_to_name(char let, uchar prefix_style, boolean showsym)
{
...
const char*
adjusted_dfeature_at(int x, int y)
{
    if (!isok(x, y))
        return 0;
...
int
here_window_display_style(const char *dfeature, struct engr *engraving, struct obj *first_obj_here)
{
    int total_count = 0;
    struct obj* otmp_cnt;
...
struct obj*
contains_otyp(struct obj *container, int otyp)
{
    if (!container)
        return (struct obj*)0;
