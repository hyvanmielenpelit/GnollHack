            ...
            || (useboulder && otmp->otyp == BOULDER))
        {
            ...
            if (
                ...
                || (!strcmp(word, "set as quick engrave item") /* exclude if already a quick engrave item */
                    && (otmp->o_id == context.quick_engrave_obj_oid || Is_container(otmp)))
                ...
                || (!strcmp(word, "set as quick bag") /* exclude if already a quick bag */
                    && (otmp->o_id == context.quick_bag_obj_oid || !Is_container(otmp) || (Is_box(otmp) && (!otmp->lknown || otmp->olocked)) || (!objects[otmp->otyp].oc_name_known && !otmp->cknown) || Is_specialized_container(otmp) || Is_container_with_closed_lid(otmp)))
