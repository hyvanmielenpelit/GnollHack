...
#endif
static boolean escape_sequence_key_start_allowed = FALSE;
...
extern int dodropmany(void);         /**/
extern int doautostash(void);        /**/
...
extern int dotoggledecorations(void);/**/
...
extern int dostash(void);            /**/
extern int dostashfloor(void);       /**/
...
static int wiz_save_monsters(void);
static int wiz_save_tiledata(void);
static int wiz_count_tiles(void);
static int wiz_save_glyph2tiles(void);
static int wiz_save_quest_texts(void);
#endif
...
#endif
static size_t size_monst(struct monst *, boolean);
static size_t size_obj(struct obj *);
static void count_obj(struct obj *, int64_t *, size_t *,
                                  boolean, boolean);
static void obj_chain(winid, const char *, struct obj *,
                                  boolean, int64_t *, size_t *);
static void mon_invent_chain(winid, const char *, struct monst *,
                                         int64_t *, size_t *);
static void mon_chain(winid, const char *, struct monst *,
                                  boolean, int64_t *, size_t *);
...
static int doviewpetstatistics(struct monst*);

static void enlght_out(const char *, int);
...
static void game_enlightenment(int, int);
...
static void add_command_menu_items(winid, int);
static void check_gui_special_effect(void);
static int print_monster_abilities(winid, int);
static int print_steed_abilities(winid, int);
static void print_weapon_skill_line_core(enum p_skills, boolean, int);
static void print_weapon_skill_line(struct obj*, boolean, int);

static const char *readchar_queue = "";
static coord clicklook_cc;
static coord clickpole_cc;
...
static
int
print_monster_abilities(winid win, int abilitynum)
{
    anything any;
...
static
int
print_steed_abilities(winid win, int abilitynum)
{
    if (!u.usteed)
        return 0;
...
wiz_makemap(void)
{
    if (wizard) {
        ...
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
            if (is_mon_gd(mtmp)) { /* vault is going away; get rid of guard */
                set_mon_gd(mtmp, 0);
                mongone(mtmp);
            }
            if (DEADMONSTER(mtmp))
        ...
        move_monsters_to_mydogs(TRUE, TRUE); /* (pets-only; normally we'd be using 'FALSE' here) */
            ...
            for (i = 0; i < 2000; i++)
            {
                ...
                if (mon)
                {
                    ...
                    *descbuf = 0;
                    ...
                    int default_glyph = base_cmap_to_glyph(get_flag(level.flags.bitflags, LEVEL_BITFLAGS_ARBOREAL) ? S_tree : S_unexplored);
...
wiz_map_levltyp(void)
{
...

    {
        ...
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_VAULT))
            Strcat(dsc, " vault");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SHOP))
            Strcat(dsc, " shop");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_TEMPLE))
            Strcat(dsc, " temple");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SMITHY))
            Strcat(dsc, " smithy");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_NPC_ROOM))
            Strcat(dsc, " residence");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_COURT))
            Strcat(dsc, " throne");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_ZOO))
            Strcat(dsc, " zoo");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_MORGUE))
            Strcat(dsc, " morgue");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_BARRACKS))
            Strcat(dsc, " barracks");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_ARMORY))
            Strcat(dsc, " armory");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_BEEHIVE))
            Strcat(dsc, " hive");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_GARDEN))
            Strcat(dsc, " garden");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_LIBRARY))
            Strcat(dsc, " library");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_DRAGONLAIR))
            Strcat(dsc, " dragonlair");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SWAMP))
            Strcat(dsc, " swamp");
        /* level flags */
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOTELEPORT))
            Strcat(dsc, " noTport");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HARDFLOOR))
            Strcat(dsc, " noDig");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_NOMMAP))
            Strcat(dsc, " noMMap");
        if (!is_hero_memory())
            Strcat(dsc, " noMem");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_SHORTSIGHTED))
            Strcat(dsc, " shortsight");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_GRAVEYARD))
            Strcat(dsc, " graveyard");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_MAZE_LEV))
            Strcat(dsc, " maze");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_IS_CAVERNOUS_LEV))
            Strcat(dsc, " cave");
        if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_ARBOREAL))
            Strcat(dsc, " tree");
        if (Sokoban)
...
/* Save monster list */
static int
wiz_save_monsters(void) /* Save a csv file for monsters */
{
    if (wizard) 
    {
        int i, j;
...
/* Save tile data */
static int
wiz_save_tiledata(void) /* Save a csv file for tile data */
{
#ifdef USE_TILES
    if (wizard)
    {
...
/* Count tiles */
static int
wiz_count_tiles(void) /* Save a csv file for tile data */
{
#ifdef USE_TILES
    if (wizard)
    {
...
static int
wiz_save_glyph2tiles(void) /* Save a csv file for tile data */
{
#ifdef USE_TILES
    if (wizard)
    {
...
/* Save monster list */
static int
wiz_save_quest_texts(void) /* Save a csv file for monsters */
{
    if (wizard)
    {
...
static const char *levltyp[MAX_LEVTYPE + 2] = {
    "stone", 
    "vertical wall", 
    "horizontal wall", 
...
static void
enlght_out(const char *buf, int attr)
{
    if (en_via_menu) {
        anything any;
...
static void
enlght_line(const char *start, const char *middle, const char *end, const char *ps, boolean parentheses)
{
    char buf[BUFSZ];
...
/*
 * Parameters:
 *   propindx: index of a property which can be conveyed by worn item
 */
...
boolean 
object_stats_known(struct obj *obj)
{
    if (!obj)
        return FALSE;
...
/*
 * Parameters:
 *   resultbuf: should be at least [7] to hold "18/100\0"
 */
/* format a characteristic value, accommodating Strength's strangeness */
static char *
attrval(int attrindx, int attrvalue, char resultbuf[])
{
 ...
 *   final: ENL_GAMEINPROGRESS:0, ENL_GAMEOVERALIVE, ENL_GAMEOVERDEAD
 */
void
enlightenment(int mode, int final)
{
...
static void
game_enlightenment(int mode UNUSED, int final)
{
    char buf[BUFSZ];
...
status_enlightenment(int mode, int final)
{
    ...
    if (!uwep && (P_SKILL_LEVEL(P_MARTIAL_ARTS) > P_UNSKILLED || Martial_prowess))
    {
        ...
        int hitbonus = weapon_skill_hit_bonus(uwep, wtype, FALSE, FALSE, FALSE, 0, TRUE, FALSE); /* Gives only pure skill bonuses */
        int dmgbonus = weapon_skill_dmg_bonus(uwep, wtype, FALSE, FALSE, FALSE, 0, TRUE, FALSE); /* Gives only pure skill bonuses */
...
static
void
print_weapon_skill_line_core(enum p_skills wtype, boolean printweaponstats, int final)
{
    char buf[BUFSZ];
    if (wtype != P_NONE)
    {
        ...
        if (printweaponstats)
        {
            int hitbonus = weapon_skill_hit_bonus((struct obj*)0, wtype, FALSE, FALSE, FALSE, 0, TRUE, FALSE); /* Gives only pure skill bonuses */
            int dmgbonus = weapon_skill_dmg_bonus((struct obj*)0, wtype, FALSE, FALSE, FALSE, 0, TRUE, FALSE); /* Gives only pure skill bonuses */
...
static
void
print_weapon_skill_line(struct obj *wep, boolean printweaponstats, int final)
{
    if (!wep)
        return;
...
static
void
add_command_menu_items(winid menuwin, int cmdflag)
{
    const struct ext_func_tab* efp;
    int n = 0;
...
/*
 * Parameters:
 *   via_enlghtmt: englightment line vs topl message
 *   msgflag: for variant message phrasing
 */
void
youhiding(boolean via_enlghtmt, int msgflag)
{
    char *bp, buf[BUFSZ];
...
show_conduct(int final)
{
    ...
    if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ASCENDED) || !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_AMULET))
    {
        added_goals = TRUE;
        ...
        else if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ASCENDED))
        {
            Sprintf(goalbuf, "on a mission to sacrifice the Amulet of Yendor on the high altar to %s on the Astral Plane", u_gname());
            you_are(goalbuf, "");
            if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ENTERED_ELEMENTAL_PLANES) && !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ENTERED_ASTRAL_PLANE))
            {
    ...
    boolean looking_for_menorah = !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH) && (context.quest_flags & QUEST_FLAGS_HEARD_OF_MENORAH);
    boolean looking_for_bell = !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL) && (context.quest_flags & QUEST_FLAGS_HEARD_OF_BELL);
    boolean looking_for_book = !get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK) && (context.quest_flags & QUEST_FLAGS_HEARD_OF_BOOK);
    boolean seeking_to_enter_sanctum = !get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED) && ((context.quest_flags & QUEST_FLAGS_HEARD_OF_AMULET_IN_SANCTUM) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_HEARD_OF_INVOCATION_RITUAL) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN));
    boolean seeking_to_enter_gehennom = !get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED) && !get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_GEHENNOM_ENTERED) && (context.quest_flags & QUEST_FLAGS_HEARD_OF_AMULET_IN_GEHENNOM);
    boolean on_nh_quest = ((get_flag(quest_status.bitflags, QSCORE_BITFLAGS_GOT_QUEST) || get_flag(quest_status.bitflags, QSCORE_BITFLAGS_MET_LEADER) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QCALLED)) && !(get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QCOMPLETED) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QEXPELLED) || get_flag(quest_status.bitflags, QSCORE_BITFLAGS_LEADER_IS_DEAD)));
    boolean added_quests = FALSE;

    if (!final /* Do not print intermediate quests / related game hints / spoilers in dumplog */
        && (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) || looking_for_menorah || looking_for_bell || looking_for_book || seeking_to_enter_sanctum || seeking_to_enter_gehennom || on_nh_quest))
    {
        added_quests = TRUE;
        if(added_goals)
        ...
        if (!(get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QCOMPLETED) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QEXPELLED) || get_flag(quest_status.bitflags, QSCORE_BITFLAGS_LEADER_IS_DEAD)))
        {
            if (get_flag(quest_status.bitflags, QSCORE_BITFLAGS_GOT_QUEST))
            {
                if (get_flag(quest_status.bitflags, QSCORE_BITFLAGS_KILLED_NEMESIS) && get_flag(quest_status.bitflags, QSCORE_BITFLAGS_TOUCHED_ARTIFACT))
                {
                    Sprintf(goalbuf, "defeated %s and %s to return %s to %s", neminame(), final ? "needed" : "need", the(artiname(urole.questarti)), ldrname());
                    you_have(goalbuf, "");
                }
                else if (get_flag(quest_status.bitflags, QSCORE_BITFLAGS_KILLED_NEMESIS))
                {
                    Sprintf(goalbuf, "defeated %s and %s to recover %s", neminame(), final ? "needed" : "need", the(artiname(urole.questarti)));
                    you_have(goalbuf, "");
                }
                else if (get_flag(quest_status.bitflags, QSCORE_BITFLAGS_TOUCHED_ARTIFACT))
                {
            ...
            else if (get_flag(quest_status.bitflags, QSCORE_BITFLAGS_MET_LEADER))
            {
                if (is_pure(FALSE) <= 0)
                {
            ...
            else if (get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_QCALLED))
            {
        ...
        if (seeking_to_enter_sanctum)
        {
            ...
            {
                if (!get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN) && context.quest_flags & QUEST_FLAGS_HEARD_ORACLE_KNOWS_MORE_DETAILS)
                {
                ...
                if (get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN))
                {
    ...
    if (get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_CROWNED) && u.uevent.uhand_of_elbereth > 0)
    {
    ...
    if (get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ELBERETH_KNOWN))
    {
    ...
    const char* chest_name = objects[MAGIC_CHEST].oc_name_known || !OBJ_DESCR(objects[MAGIC_CHEST]) ? OBJ_NAME(objects[MAGIC_CHEST]) : OBJ_DESCR(objects[MAGIC_CHEST]);
    if (Role_if(PM_ARCHAEOLOGIST))
    {
        ...
        int64_t score_percentage = ((artifacts.score + artifacts2.score + statues.score + statues2.score + (artobjects.score + artobjects2.score) * ARCHAEOLOGIST_ART_OBJECT_SCORE_MULTIPLIER + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * ARCHAEOLOGIST_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
    ...
    else if (Role_if(PM_BARBARIAN))
    {
        ...
        int64_t score_percentage = ((valuables.score + valuables2.score + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * BARBARIAN_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_CAVEMAN))
    {
        ...
        int64_t score_percentage = ((amulets.score + amulets2.score + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * CAVEMAN_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_RANGER))
    {
        ...
        int64_t score_percentage = ((valuables.score + valuables2.score + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * RANGER_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_ROGUE))
    {
        ...
        int64_t score_percentage = ((ROGUE_PER_GOLD_SCORE * (valuableworth + valuableworth2) + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * ROGUE_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_SAMURAI))
    {
        ...
        int64_t score_percentage = ((valuables.score + valuables2.score + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * SAMURAI_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_VALKYRIE))
    {
        ...
        int64_t score_percentage = ((valuables.score + valuables2.score + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * VALKYRIE_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_HEALER) || Role_if(PM_PRIEST) || Role_if(PM_WIZARD))
    {
        ...
        int64_t score_percentage = ((score_gained + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * (Role_if(PM_HEALER) ? HEALER_ROLE_ACHIEVEMENT_SCORE : Role_if(PM_PRIEST) ? PRIEST_ROLE_ACHIEVEMENT_SCORE : WIZARD_ROLE_ACHIEVEMENT_SCORE)) * 100) / MAXIMUM_ROLE_SCORE;
        score_percentage = min(100, score_percentage);
    ...
    else if (Role_if(PM_MONK))
    {
        ...
        int64_t score_percentage = ((0L + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * MONK_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
        int64_t score_percentage_upon_ascension = ((score_upon_ascension + (int64_t)get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) * MONK_ROLE_ACHIEVEMENT_SCORE) * 100) / MAXIMUM_ROLE_SCORE;
...
struct ext_func_tab extcmdlist[] = {
    ...
    { M('?'), "?", "list all extended commands", doextlist, IFBURIED | GENERALCMD },
    ...
    { M('c'), "commands", "list of additional actions", docommandmenu, IFBURIED | GENERALCMD | AUTOCOMPLETE },
    { M(3) /*M('c')*/, "conduct", "list voluntary challenges you have maintained", doconduct, IFBURIED | AUTOCOMPLETE },
...
#if defined (USE_TILES) && !defined(GNH_MOBILE)
    { M('_'), "grid", "toggle tile grid on/off", dotogglegrid, IFBURIED | AUTOCOMPLETE },
#endif
    { M('h'), "handedness", "toggle swap weapon handedness", doswaphandedness, IFBURIED | AUTOCOMPLETE | INCMDMENU },
    ...
    { M(0), "levelchange", "change experience level", wiz_level_change, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
...
#endif
    { 'X', "mix", "prepare a spell from material components", domix, AUTOCOMPLETE | INSPELLMENU },
    ...
    { SPBOOK_SYM, "spellmenu", "spell main menu", dospellmainmenu, AUTOCOMPLETE | IFBURIED | INCMDMENU },
...
#if defined (USE_TILES) && !defined(GNH_MOBILE)
    { M(':'), "umark", "toggle tile player mark graphics on/off", dotoggleumark, IFBURIED | AUTOCOMPLETE },
#endif
...
uchar
cmd_from_txt(const char *command)
{
    if (!command)
        return '\0';
...
int
ext_cmd_from_txt(const char *ext_command)
{
    if (!ext_command)
        return -1;
...
/*
 * Parameters:
 *   keys_used: boolean keys_used[256]
 */
int
dokeylist_putcmds(winid datawin, boolean docount, int cmdflags, int exflags, boolean *keys_used)
{
    int i;
    char buf[BUFSZ];
    char buf2[BUFSZ];
...
static void
count_obj(struct obj *chain, int64_t *total_count, size_t *total_size, boolean top, boolean recurse)
{
    int64_t count;
    size_t size;
    struct obj *obj;
...
static void
obj_chain(winid win, const char *src, struct obj *chain, boolean force, int64_t *total_count, size_t *total_size)
{
    char buf[BUFSZ];
    int64_t count = 0L;
    size_t size = 0;
...
static void
mon_invent_chain(winid win, const char *src, struct monst *chain, int64_t *total_count, size_t *total_size)
{
    char buf[BUFSZ];
    int64_t count = 0;
    size_t size = 0;
...
static void
contained_stats(winid win, const char *src, int64_t *total_count, size_t *total_size)
{
    char buf[BUFSZ];
    int64_t count = 0;
    size_t size = 0;
...
static void
mon_chain(winid win, const char *src, struct monst *chain, boolean force, int64_t *total_count, size_t *total_size)
{
    char buf[BUFSZ];
    int64_t count = 0L;
    size_t size = 0;
...
static void
misc_stats(winid win, int64_t *total_count, size_t *total_size)
{
...
boolean
bind_specialkey(char key, const char *command)
{
    if (!command)
        return FALSE;
...
int
getdir_ex(const char *s, boolean self_always_ok)
{
    char dirsym;
    int is_mov;
...
/*
 * Parameters:
 *   win: should specify a window which is using a fixed-width font...
 *   centerchar: '.' or '@' or ' '
 */
static void
...
/*
 * Parameters:
 *   spkey: NHKF_ code for prefix key, if one was used, or for ESC
 */
...
static boolean
help_dir(char sym, int spkey, const char *msg)
{
...
/*
 * Parameters:
 *   historical: whether to include in message history: True => yes
 */
char
get_count(char *allowchars, char inkey, int64_t maxcount, int64_t *count, boolean historical)
{
    char qbuf[QBUFSZ];
    int key;
    int64_t cnt = 0L;
...
/*
 * Parameters:
 *   ynflags: 1 means use upper side for half-sized tile
 */
/*
 ...
 */
char
yn_function_core(int style, int attr, int color, int glyph, const char *title, const char *query, const char *resp, char def, const char *resp_desc, const char *introline, uint64_t ynflags)
{
...
char
yn_function(const char *query, const char *resp, char def, const char *resp_desc)
{
...
char
yn_function_end(const char *query, const char *resp, char def, const char *resp_desc)
{
    return yn_function_core(YN_STYLE_END, ATR_NONE, NO_COLOR, NO_GLYPH, (const char*)0, query, resp, def, resp_desc, (const char*)0, 0UL);
}

char
yn_function_mon(struct monst *mtmp, const char *query, const char *chars, char def, const char *descs)
{
    if (!mtmp)
...
char
yn_query_mon(struct monst *mtmp, const char *query)
{
    return yn_function_mon(mtmp, query, ynchars, 'n', yndescs);
}

char
ynq_mon(struct monst *mtmp, const char *query)
{
    return yn_function_mon(mtmp, query, ynqchars, 'q', ynqdescs);
}

char
ynaq_mon(struct monst *mtmp, const char *query)
{
...
/* for paranoid_confirm:quit,die,attack prompting */
boolean
paranoid_query_ex(int attr, int color, boolean be_paranoid, const char *title, const char *prompt)
{
...
dolight(void)
{
    if (get_location_light_range(u.ux, u.uy) != 0)
    {
        ...
        else
        {
            ...
            {
                debugprint("dolight1");
                set_lev_lamplit(u.ux, u.uy, FALSE);
    ...
    if (get_location_light_range(cc.x, cc.y) != 0)
    {
        ...
        {
            debugprint("dolight2");
            set_lev_lamplit(cc.x, cc.y, FALSE);
...
void
clear_found_this_turn_at(int x, int y)
{
    if (!isok(x, y))
        return;
...
void
create_context_menu(enum create_context_menu_types menu_type)
{
...
dosetquickbag(void)
{
    ...
    else if (Is_box(obj) && is_obj_lknown(obj) && is_obj_locked(obj))
    {
        play_sfx_sound(SFX_GENERAL_CANNOT);
...
struct monst*
spotted_linedup_monster_in_way(int x1, int y1, int x2, int y2)
{
