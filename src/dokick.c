...
kickdmg(struct monst *mon, boolean clumsy)
{
    ...
    dmg += weapon_skill_dmg_bonus(uarmf, P_MARTIAL_ARTS, FALSE, FALSE, TRUE, 0, TRUE, FALSE, FALSE, TRUE);
    ...
    if (is_shade(mon->data) && !shade_glare(uarmf, mon))
        dmg = 0;
    ...
    if (is_shade(mon->data) && !shade_glare(uarmf, mon) && !specialdmg) {
 ...
 ouch:
            ...
            if (isok(x, y)) {
                ...
                if (is_drawbridge_wall(x, y) >= 0) {
                    ...
                    pline_The_ex(ATR_NONE, CLR_MSG_ATTENTION, "drawbridge is unaffected.");
