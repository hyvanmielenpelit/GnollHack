...
deadbook(struct obj *book2)
{
    ...
    if (invocation_pos(u.ux, u.uy) && !On_stairs(u.ux, u.uy)) {
        ...
        if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL) || !get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH)) {
            pline_ex(ATR_NONE, CLR_MSG_WARNING, "A chill runs down your %s.", body_part(SPINE));
            if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL))
                You_hear("a faint chime...");
            if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH))
        ...
        if (arti_cursed) {
            ...
            if (!get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED))
            {
                ...
                set_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED, 1);
            }
            /* in case you haven't killed the Wizard yet, behave as if
               you just did */
            set_flag(u.uevent.bitflags, UEVENT_BITFLAGS_UKILLED_WIZARD, 1); /* wizdead() */
...
{
    You("finish reading one paragraph of the Prime Codex...");set_obj_aknown(book2, 1); set_obj_nknown(book2, 1); set_obj_dknown(book2, 1);
    ...
    else 
    {
        ...
        if (objects[booktype].oc_subtyp == BOOKTYPE_NOVEL) 
        {
            ...
            if (read_tribute("books", tribtitle, 0, (char *) 0, 0, spellbook->o_id))
            {
                ...
                makeknown(booktype);
                if (!get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_READ_TRIBUTE)) 
                {
                    /* give bonus of 20 xp and 4*20+0 pts */
                    more_experienced(20, 0);
                    newexplevel();
                    set_flag(u.uevent.bitflags, UEVENT_BITFLAGS_READ_TRIBUTE, 1); /* only once */
                }
                if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_READ_DISCWORLD_NOVEL))
                {
                    achievement_gained("Read a Discworld Novel");
                    set_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_READ_DISCWORLD_NOVEL, 1);
                }
            }
            return 1;
        }
...
spelleffects(int spell, boolean atme, struct monst *targetmonst, boolean *stop_readchar_ptr)
{
    ...
    */
    ...
    if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET) && u.uen >= energy) {
    ...
    if (!get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_ROLE_ACHIEVEMENT) && spell_successful && effect_happened && (
