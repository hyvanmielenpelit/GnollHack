	...
	if (!(context.spbook.delay && spellbook == context.spbook.book) && spellbook->otyp != SPE_BLANK_PAPER)
	{
		...
		*Namebuf2 = highc(*Namebuf2);
		...
		if (!confused && !hallucinated)
		{
			if (!objects[spellbook->otyp].oc_name_known)
			{
				if (objects[spellbook->otyp].oc_aflags & SFLAGS_SPELLBOOK_MUST_BE_READ_TO_IDENTIFY)
				{
	...
	*buf = highc(*buf);
	...
	if (objects[booktype].oc_dir > 0)
	{
		...
		case IMMEDIATE:
			...
			break;
		case RAY:
			if(objects[booktype].oc_aflags & SFLAGS_SPELL_EXPLOSION_EFFECT)
	...
	/* Flags */
	if (objects[booktype].oc_aflags & SFLAGS_SPELL_BYPASSES_MAGIC_RESISTANCE)
	{
