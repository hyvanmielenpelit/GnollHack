...
attack_checks(struct monst *mtmp, struct obj *wep)
{
    ...
    if (!canspotmon(mtmp)
        ...
        && !(!Blind && is_mon_undetected(mtmp) && hides_under(mtmp->data))) {
    ...
    if (is_mon_undetected(mtmp) && !canseemon(mtmp)
        && !glyph_is_warning(glyph)
        && (hides_under(mtmp->data) || mtmp->data->mlet == S_EEL)) {set_mon_undetected(mtmp, 0); set_mon_sleeping(mtmp, 0);
     ...
     */
    if ((is_mon_undetected(mtmp) || M_AP_TYPE(mtmp)) && sensemon(mtmp)) {
...
check_caitiff(struct monst *mtmp)
{
    ...
    if (Role_if(PM_KNIGHT) && u.ualign.type == A_LAWFUL
        && (!mon_can_move(mtmp)
            || (is_fleeing(mtmp) && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_MAVENGE) && mtmp != u.ustuck))) 
    {
...
hmon(struct monst *mon, struct obj *obj, int thrown, int dieroll, boolean *obj_destroyed)
{
    ...
    anger_guards = (is_peaceful(mon)
                    && (is_mon_priest(mon) || is_mon_shk(mon) || is_mon_smith(mon) || is_mon_npc(mon) || is_watch(mon->data)));
    result = hmon_hitmon(mon, obj, thrown, dieroll, obj_destroyed);
    if (is_mon_priest(mon) && !rn2(2))
        ghod_hitsu(mon);
    if (anger_guards)
    ...
    else 
    {
        ...
        if (is_weapon(obj) || obj->oclass == GEM_CLASS) 
        {
            ...
            else
            {
                ...
                if (is_obj_trapped(obj) && is_poisonable(obj))
                    ispoisoned = TRUE;
            }
        }
                    ...
                    */
                    ...
                    { /* ordinary egg(s) */
                        ...
                        if (touch_petrifies(mdat) && !stale_egg(obj))
                        {
                            ...
                            obj->oartifact = 0;
                            obj->speflags = 0;set_obj_known(obj, 0); set_obj_dknown(obj, 0); set_obj_bknown(obj, 0);
                            obj->owt = weight(obj);
                            if (thrown)
...
#undef useup_eggs
    ...
    if (jousting && obj && !isdisintegrated) 
    {
        ...
        if (jousting < 0) 
        {
            short break_threshold = min(obj->oerodeproof ? -2 : -1, -4 + greatest_erosion(obj) + (is_obj_cursed(obj) || is_obj_blessed(obj) ? -3 : 0));
    ...
    /* if a "no longer poisoned" message is coming, it will be last;
       is_obj_trapped(obj) was cleared above and any message referring to
...
damageum(struct monst *mdef, struct attack *mattk, struct obj *omonwep, int specialdmg)
{
 ...
 physical:
    ...
    case AD_SHRP:
        ...
        break;
    case AD_SLEE:
        if (!negated && !is_mon_sleeping(mdef) && sleep_monst(mdef, (struct obj*)0, &youmonst, rn1(3,8), mattk->mcadj, FALSE)) 
        {
            hit_tile = HIT_SLEEP;
            if (!Blind)
...
remove_monster_and_nearby_waitforu(struct monst *mdef)
{
    ...
    {
        if ((mtmp->mstrategy & STRAT_WAITFORU) != 0 && !is_mon_wiz(mtmp) && m_cansee(mtmp, mdef->mx, mdef->my))
        {
        ...
        if (!resists_blnd(mtmp)) {
            ...
            if (!DEADMONSTER(mtmp)) {
                if (!context.mon_moving)
                    setmangry(mtmp, TRUE);
                if (tmp < 9 && !is_mon_shk(mtmp) && rn2(4))
                {
