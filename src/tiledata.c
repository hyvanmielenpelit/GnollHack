...
NEARDATA struct ui_component_definition ui_tile_component_array[MAX_UI_TILES] = {
    ...
    {"status2",                 NO_REPLACEMENT, NO_ANIMATION, NO_ENLARGEMENT, min(min(MAX_UI_TILE_16_x_16_COMPONENTS, MAX_UI_TILE_COMPONENTS), max(0, MAX_STATUS_MARKS - MAX_UI_TILE_16_x_16_COMPONENTS)), 16, 16, {"eating", "frozen", "high-level", "",  "", "", "", "",  "", "", "", "",  "", "", "", "",  "", "", "", "",  "", "", "", "",  "", "", "", "",  "", "", "", ""} },
...
/* If updating, also update ZAP_INDEX_WITH_FIRST_TILE from tiledata.h */
    ...
    for (i = 1; i < MAX_REPLACEMENTS; i++)  /* replacement number, starts at 1 */
    {
        ...
        for (j = 0; j < max(0, min(replacements[i].number_of_tiles, MAX_TILES_PER_REPLACEMENT)); j++) /* tile number */
        {
            ...
            {
                l = replacements[i].missile_dir >= 0 ? NUM_MISSILE_DIRS : 1;
                for (k = 0; k < l; k++)
                {
                    tilemaparray[rgl_cnt + replacement_offsets[i] /* replacements[i].glyph_offset */ + GLYPH_REPLACEMENT_OFF] = tile_count;
                    rgl_cnt++;
                }
            }
            tile_count++;
