...
const char *lastR;
{
    ...
    if (len + len_lastR + len_qsfx > lenlimit)
    {
        ...
        if (len < lenlimit) 
        {
            Strncpy(&qbuf[len], lastR ? lastR : "", lenlimit - len);
            qbuf[lenlimit] = '\0';
            len = strlen(qbuf);
            if (qsuffix && qbuf != qsuffix && len < lenlimit)
            {
    ...
    else 
    {
        ...
        /* format the object */
        bufp = short_oname(obj, func, altfunc, len <= lenlimit ? lenlimit - len : 0);
