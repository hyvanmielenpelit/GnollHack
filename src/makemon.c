...
STATIC_OVL
void
m_initthrow(mtmp, otyp, oquan, poisoned, special_enchantment)
struct monst *mtmp;
int otyp, oquan;
boolean poisoned;
...
register struct monst *mtmp;
{
	...
	case S_GNOLL:
		...
		case PM_GNOLL:
			...
			if (!rn2(5))
			{
				...
				m_initthrow(mtmp, GNOLLISH_QUARREL, 6 + rnd(20), !rn2(20), 0);
			}
			break;
		case PM_GNOLL_LORD:
			if (rn2(7))
			...
			if (!rn2(3))
			{
				...
				m_initthrow(mtmp, GNOLLISH_QUARREL, 11 + rnd(15), !rn2(10), 0);
			}
			break;
		case PM_GNOLL_KING:
			(void)mongets(mtmp, GNOLLISH_STUDDED_LEATHER_ARMOR);
    ...
    case S_HUMAN:
        if (is_mercenary(ptr)) {
            ...
            case 0:
                ...
                m_initthrow(mtmp, ELVEN_ARROW, 12, FALSE, 0);
                break;
            case 1:
    ...
    case S_KOP:
        ...
        if (!rn2(4))
            m_initthrow(mtmp, CREAM_PIE, 2, FALSE, 0);
        if (!rn2(3))
    ...
    case S_ORC:
        ...
        case PM_URUK_HAI:
            ...
            if (!rn2(3)) {
                ...
                m_initthrow(mtmp, ORCISH_ARROW, 12, TRUE, 0);
            }
            if (!rn2(3))
    ...
    case S_KOBOLD:
        if (!rn2(4))
            m_initthrow(mtmp, DART, 12, !rn2(20), 0);
        break;
    ...
    default:
        ...
        case 4:
            if (strongmonst(ptr))
                ...
                m_initthrow(mtmp, DAGGER, 3, FALSE, 0);
            break;
        case 5:
            if (strongmonst(ptr))
...
static NEARDATA char syms[] = {
    ...
    AMULET_CLASS, TOOL_CLASS,      ROCK_CLASS,   GEM_CLASS,    SPBOOK_CLASS,  REAGENT_CLASS,
