...
on_locate(void)
{
    ...
    if (is_qstatus_killed_nemesis()) {
        return;
    } else if (!is_qstatus_first_locate()) {
        if (from_above)
        {
...
on_goal(void)
{
    if (is_qstatus_killed_nemesis()) {
        ...
        if (quest_status.made_goal < 7)
            quest_status.made_goal++;
    }
}
...
{
    if (!is_qstatus_killed_nemesis()) {
        set_qstatus_killed_nemesis(TRUE);
...
finish_quest(struct obj *obj)
{
    ...
    if (is_uhave_amulet()) { /* unlikely but not impossible */
        ...
        qt_pager_ex((struct monst*)0, !is_qstatus_got_thanks() ? QT_OFFEREDIT : QT_OFFEREDIT2, ATR_NONE, CLR_MSG_HINT, FALSE);
...
{
    boolean res = TRUE;
    if (!is_qstatus_in_battle() || !is_qstatus_met_nemesis())
    {
        if (is_uhave_questart())
            qt_pager(mtmp, QT_NEMWANTSIT);
        else if (quest_status.made_goal == 1 || !is_qstatus_met_nemesis())
            qt_pager(mtmp, QT_FIRSTNEMESIS);
        else if (quest_status.made_goal < 4)
            qt_pager(mtmp, QT_NEXTNEMESIS);
        else if (quest_status.made_goal < 7)
...
chat_with_guardian(struct monst *mtmp, boolean dopopup)
{
    ...
    if (is_uhave_questart() && is_qstatus_killed_nemesis())
    {
...
quest_talk(struct monst *mtmp, boolean nearby)
{
    ...
    if (mtmp->m_id == quest_status.leader_m_id && nearby) 
    {
    ...
    {
    case MS_NEMESIS:
        if(nearby || (!is_qstatus_met_nemesis() && cansee(mtmp->mx, mtmp->my) && distu(mtmp->mx, mtmp->my) <= 64))
...
quest_stat_check(struct monst *mtmp)
{
    if (mtmp->data->msound == MS_NEMESIS)
        set_qstatus_in_battle((mon_can_move(mtmp)
