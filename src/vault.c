...
restfakecorr(struct monst *grd)
{
    ...
    if (clear_fcorr(grd, FALSE)) {
        set_mon_isgd(grd, 0); /* dmonsfree() should delete this mon */
        mongone(grd);
    }
}
...
grddead(struct monst *grd)
{
    ...
    if (dispose)
        set_mon_isgd(grd, 0); /* for dmonsfree() */
    return dispose;
}
...
findgd(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (is_mon_isgd(mtmp) && on_level(&(EGD(mtmp)->gdlevel), &u.uz))
            return mtmp;
    }
...
uleftvault(struct monst *grd)
{
    ...
    if (!grd || !is_mon_isgd(grd) || DEADMONSTER(grd)) {
    ...
    if ((money_cnt(invent) || hidden_gold())
        ...
        if (is_peaceful(grd)) {
            ...
            set_mon_mpeaceful(grd, 0); /* bypass setmangry() */
        }
        ...
        for (y = u.uy - dd; y <= u.uy + dd; ly = y, y++) {
            ...
            for (x = u.ux - dd; x <= u.ux + dd; lx = x, x++) {
                ...
                if (guard && ((x == guard->mx && y == guard->my)
                              || (is_mon_isgd(guard) && in_fcorridor(guard, x, y))))
                    continue;
...
invault(void)
{
    ...
    if (++u.uinvault % VAULT_GUARD_TIME == 0 && !guard) {
        ...
        if (!(guard = makemon(&mons[PM_GUARD], x, y, MM_EGD)))
            return;
        set_mon_isgd(guard, 1);
        set_mon_mpeaceful(guard, 1);
...
//            crm->doormask = D_NODOOR;
 ...
 cleanup:
        ...
        debugpline2("gd_move: %scleanup%s",
                    is_mon_isgd(grd) ? "" : "final ",
                    is_mon_isgd(grd) ? " attempt" : "");
