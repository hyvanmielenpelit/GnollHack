...
//static int forge_shield_of_reflection_func(struct monst*);
//static int forge_crystal_plate_mail_func(struct monst*);
//static int forge_adamantium_full_plate_mail_func(struct monst*);
//static int forge_mithril_full_plate_mail_func(struct monst*);
//static int forge_orichalcum_full_plate_mail_func(struct monst*);
//static int forge_plate_mail_func(struct monst*);
//static int forge_bronze_plate_mail_func(struct monst*);
//static int forge_field_plate_mail_func(struct monst*);
//static int forge_full_plate_mail_func(struct monst*);
//static int forge_iron_sling_bullets_func(struct monst*);
//static int forge_ex_iron_sling_bullets_func(struct monst*);
//static int forge_el_iron_sling_bullets_func(struct monst*);
//static int forge_silver_sling_bullets_func(struct monst*);
//static int forge_ex_silver_sling_bullets_func(struct monst*);
//static int forge_el_silver_sling_bullets_func(struct monst*);
...
{
    if (!mtmp || !is_mon_shk(mtmp) || !has_eshk(mtmp))
        return 0;
...
dosounds(void)
{
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_COURT) && !rn2(200)) {
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SWAMP) && !rn2(200)) {
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_VAULT) && !rn2(200)) {
        if (!(sroom = search_special(VAULT))) {
            /* strange ... */
            set_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_VAULT, 0);
            return;
        }
        if (gd_sound())
            switch (rn2(2) + hallu) {
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_BEEHIVE) && !rn2(200)) 
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            if (DEADMONSTER(mtmp))
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_LIBRARY) && !rn2(200)) 
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            if (DEADMONSTER(mtmp))
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_DRAGONLAIR) && !rn2(200))
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            if (DEADMONSTER(mtmp))
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_MORGUE) && !rn2(200)) {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
            if (DEADMONSTER(mtmp))
                continue;
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_BARRACKS) && !rn2(200)) {
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_ARMORY) && !rn2(200)) {
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_ZOO) && !rn2(200)) {
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SHOP) && !rn2(200)) 
    {
        if (!(sroom = search_special(ANY_SHOP))) {
            /* strange... */
            set_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SHOP, 0);
            return;
        }
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_DESERTEDSHOP) && !rn2(200))
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
            if (DEADMONSTER(mtmp))
                continue;
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_TEMPLE) && !rn2(200)
        && !(Is_astralevel(&u.uz) || Is_sanctum(&u.uz))) 
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            if (DEADMONSTER(mtmp))
                continue;
            if (is_mon_priest(mtmp) && inhistemple(mtmp)
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_SMITHY) && !rn2(200) && !Deaf)
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            if (DEADMONSTER(mtmp))
                continue;
            if (is_mon_smith(mtmp) && inhissmithy(mtmp)
                /* priest must be active */
                && mon_can_move(mtmp)
                /* hero must be outside this temple */
    ...
    if (get_flag(level.flags.bitflags, LEVEL_BITFLAGS_HAS_NPC_ROOM) && !rn2(200) && !Deaf)
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            if (DEADMONSTER(mtmp))
                continue;
            if (is_mon_npc(mtmp) && in_his_npc_room(mtmp)
                /* npc must be active */
                && mon_can_move(mtmp)
                /* hero must be outside this temple */
...
domonnoise(struct monst *mtmp, boolean dopopup, boolean fromchatmenu, boolean disallow_npcnoise)
{
    ...
    if (is_mon_shk(mtmp))
        msound = MS_SELL;
    else if (is_mon_smith(mtmp))
        msound = MS_SMITH;
    else if (is_mon_npc(mtmp) && !disallow_npcnoise)
        msound = MS_NPC;
    ...
    case MS_PRIEST:
    {
        const char* temple_god = iflags.using_gui_sounds ? (mon_aligntyp(mtmp) == A_NONE ? Moloch : "our almighty god") :
            ((is_mon_priest(mtmp) && has_epri(mtmp)) ? align_gname(mtmp->mextra->epri->shralign) : "our almighty god");
...
#if 0
        if (!Hallucination || (is_mon_shk(mtmp) && !rn2(2))) {
            shk_chat(mtmp);
        } else {
    ...
    case MS_NPC: /* pitch, pay, total */
        if (is_peaceful(mtmp))
        {
            if (is_mon_npc(mtmp) && has_enpc(mtmp))
            {
...
bark_here:
        ...
        {
            if (mtmp->mtame
                && (is_confused(mtmp) || is_fleeing(mtmp) || is_mon_trapped(mtmp)
    ...
    case MS_MEW:
        if (is_tame(mtmp)) {
            if (is_confused(mtmp) || is_fleeing(mtmp) || is_mon_trapped(mtmp)
                || mtmp->mtame < 5)
            {
                pline_msg = "yowls.";
                chat_line = 3;
    ...
    case MS_HUMANOID:
        ...
        else if (mtmp->mtame && !is_mon_minion(mtmp)
...
dochat(void)
{
    ...
    if ((!mtmp || is_mon_undetected(mtmp))
...
{
    if (!mtmp || 
        ((!canspotmon(mtmp) || is_mon_undetected(mtmp) || M_AP_TYPE(mtmp) == M_AP_FURNITURE || M_AP_TYPE(mtmp) == M_AP_OBJECT) && !is_tame(mtmp)))
    {
        play_sfx_sound(SFX_GENERAL_NOTHING_THERE);
    ...
    unsigned was_ritual_known = get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN);
    unsigned had_heard_of_ritual = get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_HEARD_OF_INVOCATION_RITUAL);
    unsigned was_elbereth_known = get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ELBERETH_KNOWN);
    ...
    if (!mon_can_move(mtmp) && !is_mon_priest(mtmp)) {
    ...
    do
    {
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].general_flags & NPC_FLAGS_ALSO_NORMAL_HELLO) !=0)
        {
        ...
        {
            if (!is_mon_gd(mtmp) && !(mtmp->data->mflags6 & M6_HATCHLING) && (is_izchak(mtmp, TRUE) || mtmp->rumorsleft >= 0))
            {
                if(is_izchak(mtmp, TRUE))
                    Strcpy(available_chat_list[chatnum].name, get_flag(mtmp->mon_bitflags, MON_BITFLAGS_TOLD_RUMOR) ? "Ask what is further on his mind" : "Ask what is on his mind");
                else
                    Strcpy(available_chat_list[chatnum].name, get_flag(mtmp->mon_bitflags, MON_BITFLAGS_TOLD_RUMOR) ? "Ask for further adventuring advice" : "Ask for adventuring advice");
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].service_flags & NPC_SERVICE_GIVE_QUANTUM_QUESTS) != 0)
        {
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].service_flags & NPC_SERVICE_GIVE_STARTING_QUESTS) != 0)
        {
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].service_flags & NPC_SERVICE_GIVE_ADVANCED_QUESTS) != 0)
        {
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].service_flags & NPC_SERVICE_GIVE_GNOMISH_QUESTS) != 0)
        {
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].service_flags & NPC_SERVICE_GIVE_ORCISH_QUESTS) != 0)
        {
        ...
        if (is_speaking(mtmp->data) && is_peaceful(mtmp) && is_mon_npc(mtmp) && has_enpc(mtmp))
        {
        ...
        if (has_edog(mtmp) && is_tame(mtmp) && is_peaceful(mtmp))
        {
            ...
            if (mtmp->data->mlet == S_DOG && !mtmp->mstaying && get_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTOMOVE) && mtmp != u.usteed)
            {
            ...
            if (!mtmp->mstaying && get_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTOMOVE) && mtmp != u.usteed)
            {
            ...
            if (mtmp->minvent && droppables(mtmp) && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISSUMMONED) && !is_mon_partymember(mtmp))
            {
            ...
            if (OBJ_AT(mtmp->mx, mtmp->my) && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISSUMMONED) && !is_mon_partymember(mtmp))
            {
        ...
        if (is_peaceful(mtmp) && !(is_tame(mtmp) && !is_mon_partymember(mtmp))
            && !is_mon_gd(mtmp)
            //&& !is_mon_priest(mtmp)
            && !is_watch(mtmp->data)
            && msound != MS_ORACLE
        ...
        /* Priest */
        if (is_peaceful(mtmp) && (msound == MS_PRIEST || is_mon_priest(mtmp)))
        {
        ...
        /* Shopkeeper */
        if (msound == MS_SELL || is_mon_shk(mtmp))
        {
            //if(1)
            {
                if(is_peaceful(mtmp))
        ...
        /* Smith */
        if (msound == MS_SMITH || is_mon_smith(mtmp))
        {
            if (!is_peaceful(mtmp))
            {
                Strcpy(available_chat_list[chatnum].name, "Ask for reconciliation");
        ...
        /* NPCs */
        if (msound == MS_NPC || is_mon_npc(mtmp))
        {
            if (!is_peaceful(mtmp))
            {
                Strcpy(available_chat_list[chatnum].name, "Ask for reconciliation");
...
end_of_chat_here:
    if (!was_elbereth_known && get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_ELBERETH_KNOWN))
    {
    ...
    if (!get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOKED) && !(get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BELL) && get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_BOOK) && get_flag(u.uachieve.bitflags, UACHIEVE_BITFLAGS_MENORAH)))
    {
        if (!was_ritual_known && !had_heard_of_ritual && (get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_INVOCATION_RITUAL_KNOWN) || get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_HEARD_OF_INVOCATION_RITUAL)))
        {
...
do_chat_whoareyou(struct monst *mtmp)
{
    ...
    else if (is_mon_priest(mtmp) || msound == MS_PRIEST)
    {
    ...
    else if (is_mon_npc(mtmp) && has_enpc(mtmp))
    {
        boolean saved_know_mname = is_mon_u_know_mname(mtmp);
        if (iflags.using_gui_sounds)
        {
...
do_chat_rumors(struct monst *mtmp)
{
    ...
    if (mtmp->data->msound == MS_ORACLE || mtmp->data == &mons[PM_ORACLE])
    {
        ...
        popup_talk_line(mtmp, ansbuf);
        mtmp->rumorsleft = -1;
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_TOLD_RUMOR, TRUE);
        return 1;
    }
    ...
    char ansbuf[BUFSZ];
    char rumorbuf[BUFSZ] = "";
    int truth_core = is_mon_shk(mtmp) || is_mon_smith(mtmp) || (mtmp->data->mflags6 & M6_ELDER) || is_angel(mtmp->data) || (is_mon_npc(mtmp) && has_enpc(mtmp) && (npc_subtype_definitions[ENPC(mtmp)->npc_typ].general_flags & NPC_FLAGS_HAS_TRUE_RUMORS) != 0) ? 1
        : (is_mon_priest(mtmp) && has_epri(mtmp) && EPRI(mtmp)->shralign == A_NONE) || (is_mon_minion(mtmp) && has_emin(mtmp) && EMIN(mtmp)->min_align == A_NONE) || is_demon(mtmp->data) ? -1
        : is_mon_priest(mtmp) || is_mon_minion(mtmp) ? 1 : 0;
    ...
    if (mtmp->rumorsleft == 0 || !*rumorbuf)
    {
        ...
        pline("%s answers:", noittame_Monnam(mtmp));
        Sprintf(ansbuf, "Unfortunately, I don't have any %s advice for you.", get_flag(mtmp->mon_bitflags, MON_BITFLAGS_TOLD_RUMOR) ? "further" : "useful");
        if (is_death)
            (void)ucase(ansbuf);
    ...
    else
    {
        ...
        pline("%s answers:", noittame_Monnam(mtmp));
        if (get_flag(mtmp->mon_bitflags, MON_BITFLAGS_TOLD_RUMOR))
...
do_chat_pet_sit(struct monst *mtmp)
{
    ...
    if (mtmp->mtame > 5 || (mtmp->mtame > 0 && rn2(mtmp->mtame + 1)))
    {
        ...
        mtmp->mstaying = 1 + rn2(5) + mtmp->mtame + (get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISFAITHFUL) ? 100 : 0);
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTOMOVE, 0);
...
do_chat_pet_good_boy_girl(struct monst *mtmp)
{
    ...
    if (is_animal(mtmp->data) && mtmp->mtame > 2 && rn2(mtmp->mtame) && mon_can_move(mtmp) && (!has_edog(mtmp) || (has_edog(mtmp) && (EDOG(mtmp)->abuse <= 0 || !rn2(EDOG(mtmp)->abuse + 2)))))
    {
        ...
        if (mtmp->mtame >= 15 && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISFAITHFUL) && !rn2(max(2, 25 - mtmp->mtame)))
        {
            set_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISFAITHFUL, 1);
            becamefaithful = TRUE;
        }
...
do_chat_pet_pet(struct monst *mtmp)
{
    ...
    {
        int color = NO_COLOR;
        if (!rn2(is_domestic(mtmp->data) ? 20 : 200) && !(mtmp->data->mflags2 & M2_HOSTILE) && !(mtmp->data->geno & G_UNIQ) && !is_mon_wiz(mtmp) && mtmp->cham < LOW_PM)
        {
...
do_chat_pet_stay(struct monst *mtmp)
{
    ...
    if (mtmp->mtame > 5 || (mtmp->mtame > 0 && rn2(mtmp->mtame + 1)))
    {
        ...
        mtmp->mstaying = 15 + rn2(20) + 10 * mtmp->mtame + (get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISFAITHFUL) ? 1000 : 0);
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTOMOVE, 0);
    }
    else
        Sprintf(pbuf, "%s stares at you but does nothing.", noittame_Monnam(mtmp));
...
do_chat_pet_standup(struct monst *mtmp)
{
    ...
    if (mtmp->mtame > 0 && mtmp->mstaying)
    {
        ...
        mtmp->mstaying = 0;
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTOMOVE, 1);
    }
    else
        Sprintf(pbuf, "%s stares at you but does nothing.", noittame_Monnam(mtmp));
...
do_chat_pet_dropitems(struct monst *mtmp)
{
    ...
    boolean should_have_edog = !is_mon_minion(mtmp);
    ...
    {
        mtmp->mcarrying= 0;
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTODROP, 1);
...
do_chat_pet_pickitems(struct monst *mtmp)
{
    ...
    if (should_have_edog && edog)
    {
        ...
        for (i = 0; obj && i < 20; i++, obj = level.objects[omx][omy])
        {
            ...
            if (carryamt > 0 && !is_obj_cursed(obj) && !get_flag(mtmp->mon_bitflags, MON_BITFLAGS_ISSUMMONED) && !is_mon_partymember(mtmp) && !m_unpaid_item_no_pickup(mtmp, obj)
                && could_reach_item(mtmp, obj->ox, obj->oy))
            {
                ...
                if (!shkpreaction)
                {
                    ...
                    set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MWANTSTODROP, 0);
                }
            }
        }
    ...
    for (x = u.ux - 1; x <= u.ux + 1; x++)
    {
        for (y = u.uy - 1; y <= u.uy + 1; y++)
        {
            ...
            if (isok(x, y))
            {
                ...
                {
                    if (is_mon_priest(mon) && has_epri(mon) && is_peaceful(mon))
                    {
                        priest = mon;
                    }
                }
...
//                char accel = def_oc_syms[(int)otmp->oclass].sym;
    ...
    {
        if (!Deaf && is_speaking(mtmp->data) && !is_mon_shk(mtmp) && (is_undead(mtmp->data) || is_demon(mtmp->data) || (mtmp->data->maligntyp < 0 && mtmp->data->difficulty > 10) ))
        {
    ...
    if ((pick_count = select_menu(win, PICK_ANY, &pick_list)) > 0)
    {
        ...
        if (is_peaceful(mtmp) && buy_count > 0) 
        {
            ...
            if (!Deaf && !is_mon_shk(mtmp) && (is_undead(mtmp->data) || is_demon(mtmp->data) || (mtmp->data->maligntyp < 0 && mtmp->data->difficulty > 10)))
            {
            ...
            else if (!Deaf && (is_speaking(mtmp->data) || (is_mon_shk(mtmp) && !muteshk(mtmp))))
            {
...
m_sellable_item(struct obj *otmp, struct monst *mtmp)
{
    ...
    if (!otmp->owornmask 
        ...
        && ((otmp->speflags & (SPEFLAGS_GRABBED_FROM_YOU | SPEFLAGS_INTENDED_FOR_SALE)) || is_mon_npc(mtmp) || is_mon_smith(mtmp) ||
...
{
    if (!mtmp || !is_mon_smith(mtmp) || !mtmp->mextra || !ESMI(mtmp))
        return 0;
...
{
    if (!mtmp || !is_mon_npc(mtmp) || !mtmp->mextra || !ENPC(mtmp))
        return 0;
...
{
    if (!mtmp || !m_speak_check(mtmp) || !is_mon_npc(mtmp) || !has_enpc(mtmp))
        return 0;
    ...
    for (mtmp2 = fmon; mtmp2; mtmp2 = mtmp2->nmon) 
    {
        if (DEADMONSTER(mtmp2))
            continue;
        if (dist2(mtmp->mx, mtmp->my, mtmp2->mx, mtmp2->my) < distance && is_mon_canmove(mtmp2))
        {
            if (mtmp2->data->mlet == S_NYMPH && is_mon_canmove(mtmp2)
                && dist2(mtmp->mx, mtmp->my, mtmp2->mx, mtmp2->my) < distance)
            {
                set_mon_sleeping(mtmp2, 0);
                set_mon_peaceful(mtmp2, 1);
                set_flag(mtmp2->mon_bitflags, MON_BITFLAGS_MAVENGE, 0);
...
{
    if (!mtmp || !is_mon_priest(mtmp) || !mtmp->mextra || !EPRI(mtmp))
        return 0;
...
service_identify(struct monst *mtmp, int64_t id_cost)
{
    ...
    if (is_mon_shk(mtmp) && has_eshk(mtmp))
    ...
    if (n > 0)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (shkcredit > 0L && is_mon_shk(mtmp) && has_eshk(mtmp))
            {
...
sell_to_npc(struct obj *obj, struct monst *mtmp, int items_left_in_list UNUSED, boolean auto_yes)
{
    ...
    struct obj *otmp = 0;
    int res = 0;
    boolean is_smith = is_mon_smith(mtmp) && has_esmi(mtmp);
...
item_enchant_service_query(struct monst *mtmp, int enchant_type, int64_t service_cost)
{
    ...
    case 0:
        ...
        special_dialogue_sound_id = is_mon_smith(mtmp) ? (int)SMITH_LINE_WOULD_YOU_LIKE_TO_ENCHANT_A_WEAPON : 0;
        break;
    case 1:
        selectable_item_categories = getobj_enchant_armor_objects;
        no_mood_string = "enchanting an armor";
        special_dialogue_sound_id = is_mon_smith(mtmp) ? (int)SMITH_LINE_WOULD_YOU_LIKE_TO_ENCHANT_AN_ARMOR : 0;
        break;
    case 2:
        selectable_item_categories = getobj_enchant_accessory_objects;
        no_mood_string = "enchanting an accessory";
        special_dialogue_sound_id = is_mon_smith(mtmp) ? 0 : is_mon_npc(mtmp) ? (int)NPC_LINE_WOULD_YOU_LIKE_TO_ENCHANT_AN_ACCESSORY : 0;
        break;
    default:
...
forge_special_func(struct monst *mtmp, int forge_source_otyp, int64_t forge_source_quan, int forge_dest_otyp, int64_t quan, int exceptionality, uchar material, boolean initialize)
{
    ...
    play_monster_special_dialogue_line(mtmp, is_mon_smith(mtmp) ? (int)SMITH_LINE_LETS_HAVE_A_LOOK : (int)NPC_LINE_LETS_HAVE_A_LOOK);
    if (iflags.using_gui_sounds)
        Sprintf(talkbuf, "%s says: \"Let's have a look.\"", noittame_Monnam(mtmp));
    else
        Sprintf(talkbuf, "%s says: \"Let's have a look at %s.\"", noittame_Monnam(mtmp), yname(otmp));
    ...
    {
        //play_sfx_sound(SFX_ENCHANT_ITEM_GENERAL_FAIL);
        play_monster_special_dialogue_line(mtmp, is_mon_smith(mtmp) ? (int)SMITH_LINE_SORRY_THIS_IS_NOT_AN_ITEM_THAT_I_CAN_FORGE_INTO_THE_REQUESTED_ITEM : (int)NPC_LINE_SORRY_THIS_IS_NOT_A_COMPONENT_THAT_I_CAN_FORGE_INTO_THE_REQUESTED_ITEM);
    ...
    if (otmp->quan < quan_needed)
    {
        ...
        if (iflags.using_gui_sounds)
        {
            play_monster_special_dialogue_line(mtmp, is_mon_smith(mtmp) ? (int)SMITH_LINE_SORRY_YOU_NEED_MORE_COMPONENTS_TO_FORGE_THE_REQUESTED_ITEM : (int)NPC_LINE_SORRY_YOU_NEED_MORE_COMPONENTS_TO_FORGE_THE_REQUESTED_ITEM);
...
//#endif
    ...
    if (craftedobj)
    {
        ...
        play_sfx_sound(SFX_BUY_FROM_NPC);
        play_monster_special_dialogue_line(mtmp, is_mon_smith(mtmp) ? (int)SMITH_LINE_THANK_YOU_FOR_USING_MY_SERVICES : (int)NPC_LINE_THANK_YOU_FOR_USING_MY_SERVICES);
        Strcpy(talkbuf, "Thank you for using my services.");
        popup_talk_line_ex(mtmp, talkbuf, ATR_NONE, NO_COLOR, TRUE, FALSE);
    }
    else
