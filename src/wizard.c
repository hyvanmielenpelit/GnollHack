...
static NEARDATA const int nasties[] = {
    ...
    PM_BLACK_DRAGON, PM_RED_DRAGON, PM_VAMPIRE_LORD, PM_VAMPIRE_KING, PM_VAMPIRE_MAGE, PM_KING_WRAITH,
...
amulet(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (DEADMONSTER(mtmp))
            continue;
        if (is_mon_wiz(mtmp) && is_mon_sleeping(mtmp) && !rn2(40)) {
...
you_have_item(uint64_t mask)
{
    switch (mask) {
    case M3_WANTSAMUL:
        return (boolean) get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET);
    case M3_WANTSBELL:
        return (boolean) get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BELL);
    case M3_WANTSCAND:
        return (boolean) get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_MENORAH);
    case M3_WANTSBOOK:
        return (boolean) get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_BOOK);
    case M3_WANTSARTI:
        return (boolean) get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_QUESTART);
    default:
        break;
    }
    return 0;
        ...
        if (you_have_item(mask))
            ...
            return STRAT(STRAT_GROUND, otmp->ox, otmp->oy, mask);
                 ...
                 && (otyp != AMULET_OF_YENDOR
                     || (!is_mon_wiz(mtmp2) && !inhistemple(mtmp2))))
...
tactics(struct monst *mtmp)
{
    ...
    case STRAT_HEAL: /* hide and recover */
        ...
        choose_stairs(&sx, &sy);
        set_flag(mtmp->mon_bitflags, MON_BITFLAGS_MAVENGE, 1); /* covetous monsters attack while fleeing */
        if (In_W_tower(mtmp->mx, mtmp->my, &u.uz)
            || (is_mon_wiz(mtmp) && !sx && !mon_has_amulet(mtmp)))
        {
...
aggravate(void)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (in_w_tower != In_W_tower(mtmp->mx, mtmp->my, &u.uz))
            continue;
        if (is_mon_wiz(mtmp)) /* Exclude Wizard of Yendor from aggravation */
            continue;
...
clonewiz(void)
{
    ...
    if ((mtmp2 = makemon(&mons[PM_WIZARD_OF_YENDOR], u.ux, u.uy, MM_PLAY_SUMMON_ANIMATION | MM_SUMMON_NASTY_ANIMATION | MM_PLAY_SUMMON_SOUND | MM_ANIMATION_WAIT_UNTIL_END))
        != 0) {set_mon_sleeping(mtmp2, 0); mtmp2->mtame = 0; set_mon_peaceful(mtmp2, 0);
        mtmp2->mon_flags |= MON_FLAGS_CLONED_WIZ;
        if (!get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET) && rn2(2)) { /* give clone a fake */
...
summon_nasties(struct monst *summoner)
{
    ...
    else
    {
        ...
        if (summoner)
        {
            if (summoner != &youmonst && (is_mon_wiz(summoner) || summoner->m_lev >= 50))
                summon_num = rnd(3) + 1;
...
summon_level_appropriate_monsters(struct monst *summoner)
{
    ...
    count = 0;
    int ml = 0;
    if (summoner && !is_mon_wiz(summoner))
    {
        ml = summoner->m_lev;
    ...
    for (i = 1; i <= summon_num; i++)
    {
        mtmp = makemon_limited((struct permonst*) 0, bypos.x, bypos.y, 
            MM_ANGRY | MM_PLAY_SUMMON_ANIMATION | MM_SUMMON_MONSTER_ANIMATION | MM_PLAY_SUMMON_SOUND | MM_ANIMATION_WAIT_UNTIL_END, 0UL, 
    ...
    if (!context.no_of_wizards) {
        ...
        while ((mtmp = *mmtmp) != 0) {
            if (is_mon_wiz(mtmp)
                ...
                elapsed /= 50L;
                if (is_mon_sleeping(mtmp) && rn2((int) elapsed + 1))
                    set_mon_sleeping(mtmp, 0);
                if (mtmp->mfrozen == 1) /* would unfreeze on next move */
                    mtmp->mfrozen = 0, set_mon_canmove(mtmp, 1);
...
wizdead(void)
{
    ...
    if (!get_flag(u.uevent.bitflags, UEVENT_BITFLAGS_UKILLED_WIZARD)) {
        set_flag(u.uevent.bitflags, UEVENT_BITFLAGS_UKILLED_WIZARD, TRUE);
...
cuss(struct monst *mtmp)
{
    ...
    if (is_mon_wiz(mtmp))
    {
        ...
        else if (get_flag(u.uhave.bitflags, UHAVE_BITFLAGS_AMULET) && !rn2(SIZE(random_insult)))
        {
        ...
        else
        {
            ...
            verbalize_ex(ATR_NONE, CLR_MSG_GOD, "%s %s!",
               ...
               && !(is_mon_minion(mtmp) && EMIN(mtmp)->renegade)) 
    {
        int cuss_rnd = 1;
        do
        {
