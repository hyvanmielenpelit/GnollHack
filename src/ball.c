...
placebc()
{
    ...
    u.bglyph = u.cglyph = levl[u.ux][u.uy].hero_memory_layers.glyph; /* pick up glyph */
...
int already_blind;
{
    ...
    if (already_blind || u.uswallow) {
        u.cglyph = u.bglyph = levl[u.ux][u.uy].hero_memory_layers.glyph;
        return;
    }
    ...
    newsym(uchain->ox, uchain->oy);
    u.cglyph = levl[uchain->ox][uchain->oy].hero_memory_layers.glyph;
    ...
    if (u.bc_order == BCPOS_DIFFER) { /* different locations */
        ...
        if (ball_on_floor) {
            ...
            u.bglyph = levl[uball->ox][uball->oy].hero_memory_layers.glyph;
    ...
    if (Blind)
    {
        ...
        if (!before)
        {
            if ((control & BC_CHAIN) && (control & BC_BALL)) 
            {
                ...
                u.bglyph = levl[ballx][bally].hero_memory_layers.glyph;
                u.cglyph = levl[chainx][chainy].hero_memory_layers.glyph;
            ...
            else if (control & BC_BALL) 
            {
                ...
                u.bglyph = (ballx != chainx || bally != chainy)
                               ? levl[ballx][bally].hero_memory_layers.glyph
                               : u.cglyph;
            ...
            else if (control & BC_CHAIN) 
            {
                ...
                u.cglyph = (ballx != chainx || bally != chainy)
                               ? levl[chainx][chainy].hero_memory_layers.glyph
                               : u.bglyph;
...
xchar x, y;
{
    if (Blind) {
        ...
        u.bc_order = bc_order();
        /* pick up glyph */
        u.bglyph = (u.bc_order) ? u.cglyph : levl[x][y].hero_memory_layers.glyph;
    }
    ...
    if (x != u.ux || y != u.uy) {
        ...
        if (Blind) {
            ...
            u.bc_felt = 0; /* feel nothing */
            /* pick up new glyph */
            u.cglyph = (u.bc_order) ? u.bglyph : levl[u.ux][u.uy].hero_memory_layers.glyph;
        }
