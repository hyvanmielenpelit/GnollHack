...
placebc()
{
    ...
    u.bglyph = u.cglyph = levl[u.ux][u.uy].layers.glyph; /* pick up glyph */
...
unplacebc()
{
    ...
    if (!carried(uball)) {
        ...
        if (Blind && (u.bc_felt & BC_BALL)) /* drop glyph */
            levl[uball->ox][uball->oy].layers.glyph = u.bglyph;

        newsym(uball->ox, uball->oy);
    }
    obj_extract_self(uchain);
    if (Blind && (u.bc_felt & BC_CHAIN)) /* drop glyph */
        levl[uchain->ox][uchain->oy].layers.glyph = u.cglyph;
...
int already_blind;
{
    ...
    if (already_blind || u.uswallow) {
        u.cglyph = u.bglyph = levl[u.ux][u.uy].layers.glyph;
        return;
    }
    ...
    newsym(uchain->ox, uchain->oy);
    u.cglyph = levl[uchain->ox][uchain->oy].layers.glyph;
    ...
    if (u.bc_order == BCPOS_DIFFER) { /* different locations */
        ...
        if (ball_on_floor) {
            ...
            u.bglyph = levl[uball->ox][uball->oy].layers.glyph;
    ...
    if (Blind) {
        ...
        if (!before) {
            if ((control & BC_CHAIN) && (control & BC_BALL)) {
                ...
                u.bglyph = levl[ballx][bally].layers.glyph;
                u.cglyph = levl[chainx][chainy].layers.glyph;
                ...
                u.cglyph = (ballx != chainx || bally != chainy)
                               ? levl[chainx][chainy].layers.glyph
                               : u.bglyph;
...
xchar x, y;
{
    if (Blind) {
        ...
        u.bc_order = bc_order();
        /* pick up glyph */
        u.bglyph = (u.bc_order) ? u.cglyph : levl[x][y].layers.glyph;
    }
    ...
    if (x != u.ux || y != u.uy) {
        ...
        if (Blind) {
            ...
            u.bc_felt = 0; /* feel nothing */
            /* pick up new glyph */
            u.cglyph = (u.bc_order) ? u.bglyph : levl[u.ux][u.uy].layers.glyph;
        }
