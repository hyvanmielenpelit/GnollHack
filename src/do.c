    ...
    if (objects[obj->otyp].oc_flags & O1_CANNOT_BE_DROPPED_IF_CURSED && obj->cursed) {
        ...
        if (*word) {
            ...
            if (!strcmp(word, "throw") && obj->quan > 1L)
                obj->corpsenm = 1;
            play_sfx_sound(SFX_GENERAL_CANNOT);
            pline_ex(ATR_NONE, CLR_RED, "For some reason, you cannot %s%s %s%s!", word,
...
dodown()
{
    ...
    {
        play_sfx_sound(SFX_GENERAL_CANNOT);
        You_ex(ATR_NONE, CLR_RED, "are %s, and cannot go down.",
...
doup()
{
    ...
    if (u.ustuck) {
        play_sfx_sound(SFX_GENERAL_CANNOT);
        You_ex(ATR_NONE, CLR_RED, "are %s, and cannot go up.",
            !u.uswallow ? "being held" : is_animal(u.ustuck->data)
                                             ? "swallowed"
                                             : "engulfed");
        return 1;
...
xchar portal; /* 1 = Magic portal, 2 = Modron portal down (find portal up), 3 = Modron portal up (find portal down), 4 = Modron portal (random destination) */
{
    ...
    else 
    {
                 ...
                 || quest_status.leader_is_dead)) 
        {
            ...
            {
                u.uevent.qcalled = 1;
                com_pager((struct monst*)0, 2); /* main "leader needs help" message */
            }
            else 
            {          /* reminder message */
                com_pager((struct monst*)0, Role_if(PM_ROGUE) ? 4 : 3);
            }
        }
