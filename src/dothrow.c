...
get_multishot_stats(struct monst *magr, struct obj *otmp, struct obj *weapon, uchar thrown)
{
    ...
    if (!otmp || otmp == uarmg)
    {
        ...
        if (isyou)
        {
            skilllevel = adjusted_limited_skill_level(P_MARTIAL_ARTS, FALSE, TRUE, TRUE);
        }
        else
        {
            switch (magr->mnum)
...
hurtle_step(genericptr_t arg, int x, int y)
{
    ...
    if (!Passes_walls || !(may_pass = may_passwall(x, y))) 
    {
        ...
        if (IS_ROCK(levl[x][y].typ) || closed_door(x, y) || odoor_diag) 
        {
            ...
            if (odoor_diag)
                You_ex(ATR_NONE, CLR_MSG_WARNING, "hit the door edge!");
        ...
        if (levl[x][y].typ == IRONBARS)
        {
            ...
            play_player_ouch_sound(MONSTER_OUCH_SOUND_OUCH);
            You_ex(ATR_NONE, CLR_MSG_WARNING, "crash into some iron bars.  Ouch!");
            dmg = rnd(2 + *range);
        ...
        if ((obj = sobj_at(BOULDER, x, y)) != 0) 
        {
            ...
            if (iflags.using_gui_sounds)
                delay_output_milliseconds(2 * ANIMATION_FRAME_INTERVAL);
            play_player_ouch_sound(MONSTER_OUCH_SOUND_OUCH);
            You_ex(ATR_NONE, CLR_MSG_WARNING, "bump into a %s.  Ouch!", xname(obj));
            dmg = rnd(2 + *range);
...
thitmonst(struct monst *mon, struct obj *obj, boolean is_golf, uchar *hitres_ptr)
{
    ...
    int tohit_skill_bonus = weapon_skill_hit_bonus((struct obj*)0, P_NONE, FALSE, FALSE, 2, 0, TRUE, TRUE, FALSE, TRUE);
    ...
    if (obj->oclass == WEAPON_CLASS || is_weptool(obj) || obj->oclass == GEM_CLASS) 
    {
        ...
        else if (is_ammo(obj) || is_golf_swing_with_stone)
        {
            ...
            else if (uwep)
            {
                ...
                nonpolytmp += weapon_skill_hit_bonus(uwep, is_golf_swing_with_stone ? P_THROWN_WEAPON : P_NONE, FALSE, FALSE, TRUE, 0, TRUE, FALSE, FALSE, TRUE); //Players get skill bonuses
//                if (uwep->oartifact)
//                    tmp += spec_abon(uwep, mon);
        ...
        { /* thrown non-ammo or applied polearm/grapnel */
            ...
            nonpolytmp += weapon_skill_hit_bonus(obj, is_golf_swing_with_stone ? P_THROWN_WEAPON : P_NONE, FALSE, FALSE, TRUE, 0, TRUE, TRUE, FALSE, TRUE);
        }
