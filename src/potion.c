...
static int64_t itimeout(int64_t);
static void ghost_from_bottle(void);
static short mixtype(struct obj *, struct obj *);
static short mixtype_core(struct obj*, struct obj*);

/* force `val' to be within valid range for intrinsic timeout value */
static int64_t
itimeout(int64_t val)
{
    if (val >= TIMEOUT)
        val = TIMEOUT;
    else if (val < 1)
...
int64_t
itimeout_incr(int64_t old, int incr)
{
...
/* Sick is overloaded with both fatal illness and food poisoning (via
   ...
   the other at a time.  They should become separate intrinsics.... */
void
make_food_poisoned(int64_t xtime, const char *cause, boolean talk, int killerhintidx)
{
    struct kinfo* kptr;
 ...
 */
void
make_mummy_rotted(int64_t xtime, const char *cause, boolean talk, int killerhintidx)
{
    struct kinfo* kptr;
...
void
make_slimed(int64_t xtime, const char *msg, int killedby, const char *killername, int killerhintidx)
{
    int64_t old = Slimed;
    struct kinfo* kptr;
...
void
make_stoned(int64_t xtime, const char *msg, int killedby, const char *killername, int killhintidx)
{
    int64_t old = Stoned;
    struct kinfo* kptr;
...
void
make_blinded(int64_t xtime, boolean talk)
{
    int64_t old = Blinded;
 ...
 *   mask: nonzero if resistance status should change by mask
 */
boolean
...
dodrink(void)
{
    ...
    if (!gone)
    {
        ...
        if (is_obj_unpaid(otmp) && costly_spot(u.ux, u.uy))
        {
            debugprint_pos();
...
peffects(struct obj *otmp)
{
        ...
        if (is_obj_cursed(otmp)) {
            ...
            if (otmp->otyp == POT_RESTORE_ABILITY)
            {
                if (u.ulevel < u.ulevelmax) {
                    do {
                        pluslvl(FALSE);
                    } while (u.ulevel < u.ulevelmax && is_obj_blessed(otmp));
                }
                if (u.ubasehpdrain < 0) {
    ...
    case SPE_IMPROVED_MONSTER_DETECTION:
        if (is_obj_blessed(otmp) || otmp->otyp == SPE_IMPROVED_MONSTER_DETECTION)
        {
            int x, y;
    ...
    case POT_POISON:
        ...
        else 
        {
            if (!Fixed_abil) 
            {
                ...
                (void)adjattrib(typ, Poison_resistance ? -1 : (is_obj_blessed(otmp) ? -rnd(max(1,extra_data1)) : -d(2, max(1, extra_data1))), 1);
            }
    ...
    case POT_GAIN_ABILITY:
        ...
        {      /* If blessed, increase all; if not, try up to */
            ...
            int prev_ability = -1;
            int abilities_to_increase = (is_obj_blessed(otmp) ? 2 : 1);
            i = -1;   /* increment to 0 */
            if (isdiluted && !rn2(2))
            {
                /* Nothing */
    ...
    case POT_LIGHTNING_SPEED:
        ...
        if (Wounded_legs && !is_obj_cursed(otmp) && !u.usteed) {
            heal_legs(0);
            unkn++;
            break;
        }
    ...
    case POT_HEALING:
        ...
        healup(duration, is_obj_blessed(otmp) ? extra_data1 : 0,
    ...
    case POT_FULL_HEALING:
        ...
        if (is_obj_blessed(otmp) && u.ulevel < u.ulevelmax) {
    ...
    { /* M. Stephenson */
        ...
        /* old: num = rnd(5) + 5 * is_obj_blessed(otmp) + 1;
    ...
    case POT_ACID:
        if (Acid_immunity) {
            ...
            pline_ex(ATR_NONE, is_obj_blessed(otmp) ? CLR_MSG_WARNING : CLR_MSG_NEGATIVE, "This burns%s!",
                  is_obj_blessed(otmp) ? " a little" : is_obj_cursed(otmp) ? " a lot"
                                                             : " like acid");
            dmg = duration;
...
void
healup(int nhp, int nxtra, boolean curesick, boolean cureblind, boolean curehallucination, boolean curestun, boolean cureconfusion)
{
...
void
strange_feeling(struct obj *obj, const char *txt, boolean dopopup)
{
    char buf[BUFSZ] = "";
    ...
    if (is_obj_unpaid(obj) && costly_spot(u.ux, u.uy))
    {
        debugprint_pos();
        char* o_shop = in_rooms(u.ux, u.uy, SHOPBASE);
        struct monst* shkp = 0;
...
/*
 * Parameters:
 *   objphrase: "Your widget glows" or "Steed's saddle glows"
 */
/* handle item dipped into water potion or steed saddle splashed by same */
boolean
H2Opotion_dip(struct obj *potion, struct obj *targobj, boolean useeit, const char *objphrase)
{
...
#define COST_none (-1)
    ...
    if (is_obj_blessed(potion)) 
    {
        if (is_obj_cursed(targobj)) 
        {
            func = uncurse;
        ...
        else if (!is_obj_blessed(targobj))
        {
            func = bless;
            textcolor = Hallucination ? CLR_MSG_HALLUCINATED : CLR_MSG_POSITIVE;
    ...
    else if (is_obj_cursed(potion)) 
    {
        if (is_obj_blessed(targobj))
        {
            func = unbless;
...
void
potionhit(struct monst *mon, struct obj **obj_ptr, int how)
{
    if (!mon || !obj_ptr)
        return;
    ...
    else 
    {
        ...
        if (((mon->worn_item_flags & W_SADDLE)
            ...
            && (!rn2(10)
                || (obj->otyp == POT_WATER
                    && ((rnl(10) > 7 && is_obj_cursed(obj))
                        || (rnl(10) < 4 && is_obj_blessed(obj)) || !rn2(3)))))
            hit_saddle = TRUE;
        ...
        case POT_ACID:
            if (!Acid_immunity) 
            {
                ...
                Sprintf(dcbuf, "This burns%s!",
                      is_obj_blessed(obj) ? " a little"
                                   : is_obj_cursed(obj) ? " a lot" : "");
                pline_ex1(ATR_NONE, is_obj_blessed(obj) ? CLR_MSG_WARNING : CLR_MSG_NEGATIVE, dcbuf);
                dmg = duration;
...
do_illness: /* Pestilence's potion of healing effect */
        ...
        case POT_ACID:
            if (!is_mon_immune_to_acid(mon) && !check_ability_resistance_success(mon, A_DEX, objects[obj->otyp].oc_mc_adjustment)) {
                ...
                if (!is_silent(mon->data))
                    wake_nearto(tx, ty, (int)mon->data->mlevel * 10);
                deduct_monster_hp(mon, adjust_damage(d(is_obj_cursed(obj) ? 4 : 3, is_obj_blessed(obj) ? 6 : 8), &youmonst, mon, AD_ACID, ADFLAGS_NONE));
                if (DEADMONSTER(mon)) {
                    if (your_fault)
                        killed(mon);
                    else
    ...
    else if (is_obj_dknown(obj) && !objects[obj->otyp].oc_name_known
...
potionbreathe(struct obj *obj, const char *introline)
{
    ...
    case POT_GAIN_ABILITY:
        ...
        {
            if (is_obj_blessed(obj) && !Fixed_abil)
            {
    ...
    case POT_WATER:
        if (u.umonnum == PM_GREMLIN) {
            ...
            if (is_obj_blessed(obj) && youmonst.data == &mons[u.ulycn])
                you_unwere(FALSE);
            else if (is_obj_cursed(obj) && !Upolyd)
                you_were();
        }
        break;
    case POT_ACID:
...
static short
mixtype_core(struct obj *o1, struct obj *o2)
{
    switch (o1->otyp) {
    case POT_POISON:
    case POT_SICKNESS:
...
dodip(void)
{
 ...
 into? [abdeghjkmnpqstvwyzBCEFHIKLNOQRTUWXZ#-# or ?*] "));
        ...
        if (yn_query(qbuf) == 'y')
        {
            ...
            else
            {
                ...
                if (obj->otyp == POT_ACID)
                    set_obj_in_use(obj, 1);
                if (water_damage(obj, 0, TRUE) != ER_DESTROYED && is_obj_in_use(obj))
                    useup(obj);
                if (!rn2(20))
                {
    ...
    if (!useupliquidonly && !potion->oartifact && !is_obj_indestructible(potion))
        set_obj_in_use(potion, TRUE); /* assume it will be used up */
    ...
    else if (obj->otyp == POT_POLYMORPH || potion->otyp == POT_POLYMORPH)
    {
        ...
        else if (potion->otyp == POT_POLYMORPH)
        {
            ...
            if (!obj)
            {
                ...
                set_obj_in_use(potion, FALSE); /* didn't go poof */
                return 1;
            } 
        ...
        else if (obj->otyp == POT_POLYMORPH) /* Just in case instead of else */
        {
            ...
            set_obj_in_use(potion, FALSE); /* didn't go poof, it's being polymorphed */
        ...
        && !obj_currently_allows_object_to_be_dipped_into_it(potion))
    {
        ...
        else if (Is_proper_container(potion) && !is_obj_olocked(potion))
        {
    ...
    else if (obj->oclass == POTION_CLASS && obj->otyp != potion->otyp)
    {
        ...
        else
        {
            ...
            if (is_obj_cursed(obj) || obj->otyp == POT_ACID || !rn2(10))
            {
                ...
                /* it would be better to use up the whole stack in advance
                   ...
                   around for potionbreathe() [and we can't set is_obj_in_use(obj)
    ...
    {
        if ((potion->otyp == POT_POISON) && !is_obj_opoisoned(obj))
        {
            char buf[BUFSZ];
        ...
        else if (is_obj_opoisoned(obj) && (potion->otyp == POT_HEALING
    ...
    if (potion->otyp == POT_OIL) 
    {
        ...
        if (is_obj_lamplit(potion)) { /* burning */
 ...
 more_dips:
    ...
    if ((obj->otyp == UNICORN_HORN || obj->otyp == AMETHYST)
        ...
        set_obj_blessed(singlepotion, 0);
        if (mixture == POT_WATER)
        {
            set_obj_cursed(singlepotion, 0);
            singlepotion->odiluted = 0;
        }
        else
            set_obj_cursed(singlepotion, is_obj_cursed(obj)); /* odiluted left as-is */
        set_obj_bknown(singlepotion, FALSE);
        if (Blind) {
            set_obj_dknown(singlepotion, FALSE);
        } else {
            set_obj_dknown(singlepotion, !Hallucination);
            if (mixture == POT_WATER && is_obj_dknown(singlepotion))
...
int
refill_obj_with_oil(struct obj *obj, struct obj *potion)
{
    if (!potion)
        return 0;
    ...
    if (is_obj_lamplit(obj) || is_obj_lamplit(potion))
    {
...
void
convert_magic_lamp_to_oil_lamp(struct obj *obj)
{
    if (!obj)
        return;
