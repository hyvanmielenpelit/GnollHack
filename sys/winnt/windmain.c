...
char FDECL(windows_yn_function_ex, (int, int, int, int, const char *, const char *, const char *, int, const char*, const char*, uint64_t));
...
#define OOPS_BUFSZ 512
    ...
    if ((fd = open(fq_lock, 0)) == -1) {
        ...
        raw_print(oops);
        nh_bail(EXIT_FAILURE, (const char*)0, FALSE);
    }
