...
static char FDECL(and_yn_function_ex, (int, int, int, const char *, const char *, const char *, CHAR_P, const char*));
...
void quit_possible()
{
	if(quit_if_possible)
	{
		...
		if(!SaveAndExit())
		{
			if(and_yn_function_ex(YN_STYLE_GENERAL, ATR_NONE, NO_COLOR, (const char*)0, "Error saving game. Quit anyway?", ynchars, 'n', yndescs) == 'y')
				nh_terminate(EXIT_SUCCESS);
		}
	}
}
...
winid and_create_nhwindow_ex(int type, int style, int glyph, struct extended_create_window_info)
{
...
void and_status_flush()
{
	...
	static enum statusfields fieldorder_line2[] = {
		BL_LEVELDESC, BL_HP, BL_HPMAX, BL_ENE, BL_ENEMAX, BL_AC, BL_MC_LVL, BL_MC_PCT, BL_MOVE, BL_UWEP, BL_UWEP2, BL_XP,
...
void and_display_file(const char *name, BOOLEAN_P complain)
{
	...
	if(f)
	{
		winid datawin = and_create_nhwindow_ex(NHW_TEXT, 0, NO_GLYPH, zerocreatewindowinfo);
		boolean empty = TRUE;
...
char and_yn_function_ex(int style, int attr, int color, const char* title, const char *question, const char *choices, CHAR_P def, const char* resp_desc)
{
	char ch;
	char message[BUFSZ];
	char res_ch[2];
