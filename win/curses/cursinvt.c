...
curses_update_inv(void)
{
    ...
    display_inventory_with_header(NULL, FALSE, NULL, 1, TRUE, FALSE, FALSE);
...
void
curses_add_inv(int y, int glyph UNUSED, char accelerator, int attr, int nhcolor, const char *str, const char* attrs, const char* colors)
{
    WINDOW *win = curses_get_nhwin(INV_WIN);
    int color = NO_COLOR;
    int x = 0;
