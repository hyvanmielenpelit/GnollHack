...
struct window_procs curses_procs = {
    ...
    (WC2_DARKGRAY | WC2_HITPOINTBAR
     ...
     | WC2_SUPPRESS_HIST | WC2_AUTOSTATUSLINES),
...
curses_print_glyph(winid wid, XCHAR_P x, XCHAR_P y, struct layer_info layers)
{
    ...
    if ((special & MG_STONE_INVERSE) && iflags.use_inverse) {
        attr = A_REVERSE;
    }
...
*/
char
curses_yn_function_ex(int style UNUSED, int attr, int color, const char* title UNUSED, const char *question, const char *choices, CHAR_P def)
{
