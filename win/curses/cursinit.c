...
curses_create_main_windows(void)
{
       ...
       based on alignment positions. */
    {
        ...
        int statusheight = (iflags.wc2_statuslines < 3) ? 2 : (iflags.wc2_statuslines > 7) ? 8 : iflags.wc2_statuslines;
...
curses_character_dialog(const char **choices, const char *prompt)
{
    ...
    identifier.a_void = 0;
    curses_start_menu_ex(wid, GHMENU_STYLE_CHARACTER, 0, 0UL);
