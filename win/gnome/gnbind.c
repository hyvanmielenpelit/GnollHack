...
struct window_procs Gnome_procs = {
    ...
    donull,
#endif
    gnome_print_glyph, gnome_issue_gui_command, gnome_raw_print, gnome_raw_print_bold, gnome_nhgetch,
    ...
    genl_preference_update, genl_getmsghistory_ex, genl_putmsghistory_ex,
...
void
gnome_issue_gui_command(int initid)
{
    return;
}
...
*/
char
gnome_yn_function_ex(int style, int attr, int color, int glyph, const char* title, const char *question, const char *choices, CHAR_P def, const char* resp_desc, const char* introline, unsigned long ynflags)
{
    int ch;
    int result = -1;
    char message[BUFSZ];
...
void
gnome_delay_output_intervals(int intervals)
{
    if (gnome_windowlist[WIN_MESSAGE].win != NULL) {
        gtk_signal_emit(GTK_OBJECT(gnome_windowlist[WIN_MESSAGE].win),
            ghack_signals[GHSIG_DELAY], (guint)intervals * (flags.animation_frame_interval_in_milliseconds > 0 ? flags.animation_frame_interval_in_milliseconds : ANIMATION_FRAME_INTERVAL));
    }
}
