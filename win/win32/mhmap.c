...
paintTile(PNHMapWindow data, int i, int j, RECT * rect)
{
    ...
    boolean ispet = !!(data->map[i][j].monster_flags & LMFLAGS_PET);
    boolean ispeaceful = !!(data->map[i][j].monster_flags & LMFLAGS_PEACEFUL);
    boolean isyou = !!(data->map[i][j].monster_flags & LMFLAGS_YOU);
    boolean issteed = !!(data->map[i][j].monster_flags & LMFLAGS_RIDDEN);
    ...
    */
    ...
    for (int draw_index = 0; draw_index < DRAW_ORDER_SIZE; draw_index++)
    {
        ...
        int source_dir_idx = draw_order[draw_index].source_dir_index;
            ...
            int relevant_darkening_cmap = is_lev_has_special_tileset(darkening_i, darkening_j) ? (int)level.locations[darkening_i][darkening_j].special_tileset : current_cmap;
            ...
            boolean is_worm_tail = !!(data->map[enl_i][enl_j].monster_flags & LMFLAGS_WORM_TAIL);
            ...
            if ((base_layer == LAYER_OBJECT || base_layer == LAYER_COVER_OBJECT) && m_here && (M_AP_TYPE(m_here) == M_AP_OBJECT))
            {
                ...
                if (!sensed)
                {
                    ...
                    mimic_obj.gui_glyph = maybe_get_replaced_glyph(mimic_obj.glyph, enl_i, enl_j, data_to_replacement_info(mimic_obj.glyph,
                        is_obj_drawn_in_front(&mimic_obj) ? LAYER_COVER_OBJECT : LAYER_OBJECT, &mimic_obj, (struct monst*)0, 0UL, 0UL, 0UL, MAT_NONE, 0));
                }
            }
            ...
            for (int layer_round = 0; layer_round < layer_rounds; layer_round++)
            {
                ...
                if (source_dir_idx > 0 && enlarg_idx == -1 && tile_move_idx == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            /* Worm */
                            boolean is_adj_worm_tail = !!(data->map[adj_x][adj_y].monster_flags & LMFLAGS_WORM_TAIL);
                            boolean is_adj_worm_seen = !!(data->map[adj_x][adj_y].monster_flags & LMFLAGS_WORM_SEEN);
                ...
                else if (base_layer == LAYER_LEASH && enlarg_idx == -1 && cansee(enl_i, enl_j))
                {
                    ...
                    if (something_to_draw)
                    {
                        ...
                        set_mon_leashed(&tether_mon, TRUE);
                ...
                if (glyph != NO_GLYPH)
                {
                    ...
                    if (!skip_drawing)
                    {
                        ...
                        if (((base_layer == LAYER_OBJECT || base_layer == LAYER_COVER_OBJECT) && otmp_round &&
                             (is_obj_poisoned(otmp_round) || otmp_round->elemental_enchantment > 0 || otmp_round->mythic_prefix > 0 || otmp_round->mythic_suffix > 0 || otmp_round->oeroded || otmp_round->oeroded2 || otmp_round->exceptionality > 0))
                            ||
                            ...
                            )
                        {
                            ...
                            int src_y = 0;
                            int cnt = 0;
                            int poisoned = (base_layer == LAYER_MISSILE ? data->map[enl_i][enl_j].missile_poisoned : is_obj_poisoned(otmp_round));
                    ...
                    if (!skip_darkening)
                    {
                        ...
                        if (!cansee(darkening_i, darkening_j)  || ascension_radiance || (data->map[darkening_i][darkening_j].layer_flags & LFLAGS_SHOWING_MEMORY))
                        {
                            //if (1) // Always true
                            {
                                ...
                                {
                                    boolean is_lit_unknown_wall = is_lev_waslit(darkening_i, darkening_j) && IS_NON_STONE_WALL(levl[darkening_i][darkening_j].typ) && wall_angle(&levl[darkening_i][darkening_j]) == S_stone;
                                    if (!is_lev_waslit(darkening_i, darkening_j) || is_lit_unknown_wall)
                                    {
                ...
                else if (base_layer == LAYER_GENERAL_UI && enlarg_idx == -1 && tile_move_idx == 0)
                {
                        ...
                        //if (1) // Always true
                        {
                            ...
                            for (int statusorder_idx = 0; statusorder_idx < SIZE(statusmarkorder); statusorder_idx++)
                            {
                                ...
                                case STATUS_MARK_TRAPPED:
                                    if ((loc_is_you && u.utrap) || (!loc_is_you && is_mon_trapped(mtmp)))
                                        display_this_status_mark = TRUE;
                                    break;
                                ...
                                case STATUS_MARK_INVENTORY:
                                    if (!loc_is_you && ispet && !is_mon_partymember(mtmp) && count_unworn_items(mtmp->minvent) > 0)
                                        display_this_status_mark = TRUE;
                                    break;
                                default:
                                    break;
                    ...
                    if (data->map[i][j].monster_flags & LMFLAGS_KILLED) //glyph_is_dying_monster(monster_glyph) || glyph_is_female_dying_monster(monster_glyph) || glyph_is_dying_player(monster_glyph))
                    {
                    ...
                    else if (data->map[i][j].monster_flags & LMFLAGS_BEING_HIT)
                    {
...
static void dirty(PNHMapWindow data, int x, int y, boolean usePrinted)
{
    ...
    if (data->map[x][y].monster_flags & LMFLAGS_WORM_TAIL)
    {
        int rx = x, ry = y;
    ...
    if (data->map[x][y].monster_flags & LMFLAGS_TETHERED)
    {
        int ux = data->map[x][y].leash_mon_x[MAXLEASHED], uy = data->map[x][y].leash_mon_y[MAXLEASHED];
        if (isok(ux, uy))
        {
    ...
    if (data->map[x][y].monster_flags & LMFLAGS_U_TETHERED)
    {
    ...
    for (enum layer_types layer_idx = LAYER_FLOOR; layer_idx < MAX_LAYERS; layer_idx++)
    {
        ...
        {            
            ...
            else
            {
                ...
                int tile_animation_idx = get_tile_animation_index_from_glyph(glyph);
                boolean is_dropping_piercer = m_at(x, y) && (data->map[x][y].monster_flags & LMFLAGS_DROPPING_PIERCER);
