    ...
    if (NHMENU_HAS_GLYPH(*item)) 
    {
        if (!IS_MAP_ASCII(iflags.wc_map_mode))
        {
            ...
            else
            {
                ...
                /* Item property marks */
                if (is_obj_opoisoned(&(item->object_data)) || item->object_data.elemental_enchantment > 0 || item->object_data.oeroded || item->object_data.oeroded2
                    || item->object_data.exceptionality > 0 || item->object_data.mythic_prefix > 0 || item->object_data.mythic_suffix > 0)
                {
                    ...
                    int src_y = 0;
                    int cnt = 0;
                    int poisoned = (is_obj_opoisoned(&(item->object_data)));
