...
onDrawItem(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if (NHMENU_HAS_GLYPH(*item)) 
    {
        if (!IS_MAP_ASCII(iflags.wc_map_mode))
        {
            ...
            boolean is_full_size = !!(glyphtileflags[glyph] & GLYPH_TILE_FLAG_FULL_SIZED_ITEM) || (!glyph_is_object(glyph) && !(glyphtileflags[glyph] & GLYPH_TILE_FLAG_HALF_SIZED_NONITEM));
            ...
            else
            {
                ...
                if (item->object_data.opoisoned || item->object_data.elemental_enchantment > 0 || item->object_data.oeroded || item->object_data.oeroded2 
                    ...
                    || item->object_data.mythic_quality_prefix > 0 || item->object_data.mythic_quality_suffix > 0)
                {
                    int y_start = 0;
                    int x_start = 0;
                    int mark_width = 8;
                    ...
                    int mythic_quality_prefix = (item->object_data.mythic_quality_prefix);
                    int mythic_quality_suffix = (item->object_data.mythic_quality_suffix);
