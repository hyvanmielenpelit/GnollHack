...
namespace GnollHackX.Perf
{
    ...
    public static class GHPerfStats
    {
        ...
        /* Frame pacing metrics from an interval series and the vsync period actually in
           effect. Definitions:
             jankPct        share of intervals exceeding 1.5 x vsync
             hitchRatio     sum over intervals exceeding 2 x vsync of (interval - vsync),
                            divided by the window length: ms of hitch per second, the
                            Apple hitch time ratio
             hitch2xCount   intervals exceeding 2 x vsync
             hitch4xCount   intervals exceeding 4 x vsync
        ...
        public static PacingMetrics ComputePacing(float[] intervalsMs, double vsyncMs)
        {
            ...
            if (vsyncMs > 0)
            {
                ...
                int h2 = 0, h4 = 0;
                double t15 = 1.5 * vsyncMs, t2 = 2.0 * vsyncMs, t4 = 4.0 * vsyncMs;
                for (int i = 0; i < n; i++)
                {
                ...
                m.OverrunP50 = (float)(m.P50 - vsyncMs);
                m.OverrunP95 = (float)(m.P95 - vsyncMs);
                m.OverrunP99 = (float)(m.P99 - vsyncMs);
            }
            return m;
        }
        ...
        /* Bootstrap CI of statistic(b) - statistic(a), resampling each arm independently.
           A difference whose interval excludes zero is the plan's criterion for "changed". */
        ...
        /* Below this product of sample sizes, and with no ties, the p-value is computed
           ...
           At the plan's 5 runs per arm the approximation is poor, and the exact test is
           what R's wilcox.test and SciPy's method="exact" give. */
        ...
        public static double MannWhitneyExactTwoSidedP(int m, int n, int uMin)
        {
            ...
            if (uMin > uMax)
                uMin = uMax;
            /* freq[i][j][u] would be large; iterate over j (size of B) with rolling arrays
               over i. f[i][u] for current j. */
