...
namespace GnollHackX.Performance
{
    /* UI-thread latency probe. A repeating dispatcher timer fires every IntervalMs on
       ...
       clamp at zero. A tick later than the measured refresh period (or VsyncMs before
       one is measured) counts as late. Samples are added to the histogram only while a
       measurement window is open; ticks outside a window still re-anchor. On Windows the
       default timer resolution is about 15.6 ms, so lateness below that is noise there.
    ...
    public static class GHUiThreadProbe
    {
        ...
        /* Late threshold until the frame timeline has measured a refresh period */
        public static float VsyncMs = 16.667f;

        private static readonly GHPerformanceStatistics.StreamingHistogram _latency = new GHPerformanceStatistics.StreamingHistogram();
        ...
        private static void Tick()
        {
            ...
            double refreshMs = GHFrameTimeline.MeasuredRefreshPeriodMs;
            float lateThresholdMs = refreshMs > 0 ? (float)refreshMs : VsyncMs;

            Interlocked.Increment(ref _tickCount);
            Interlocked.Exchange(ref _lastLatencyBits, FloatToBits(lateMs));
            if (lateMs > lateThresholdMs)
