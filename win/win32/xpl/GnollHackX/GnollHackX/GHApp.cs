...
namespace GnollHackX
{
    ...
    public static class GHApp
    {
        ...
        private const string AiSnapshotNativeTextSuffix = ".ai.txt"; /* AIFILE in sysconf */
        ...
        public static async Task<AiSnapshotResult> GenerateAiSnapshotTextAsync()
        {
            ...
            if (!Path.IsPathRooted(textpath))
                textpath = Path.Combine(GHPath, textpath);
            /* sysconf writes the path with forward slashes, which the Windows share API rejects */
        ...
        private static void InitializeScreenResolutions()
        {
            ...
            {
                SupportedResolutions.AddRange(supportedResolutions);
                /* Add recommended */
                UpdateRecommendedScreenResolution(UseGPU);
                if (RecommendedScreenResolution != null)
        ...
        public static void UpdateRecommendedScreenResolution(bool usingGPU)
        {
            ...
            ScreenResolutionItem oldRecommended = RecommendedScreenResolution;
            if (usingGPU || SupportedResolutions.Count == 0)
            {
            ...
            if (oldRecommended != null && RecommendedScreenResolution != null && oldRecommended != RecommendedScreenResolution)
            {
                int idx = ScreenResolutionItems.IndexOf(oldRecommended);
                if (idx >= 0)
                    ScreenResolutionItems[idx] = RecommendedScreenResolution;
            }
#endif
        }
        ...
        public static void InitializeUIBitmaps()
        {
            ...
            using (SKPaint bmpPaint = new SKPaint())
            {
                ...
                using (var redcanvas = new SKCanvas(redbitmap))
                {
                    ...
                    redcanvas.DrawImage(_orbFillBitmap, 0, 0
#if GNH_MAUI
                ...
                using (var bluecanvas = new SKCanvas(bluebitmap))
                {
                    ...
                    bluecanvas.DrawImage(_orbFillBitmap, 0, 0
#if GNH_MAUI
                    , SKSamplingOptions.Default
#endif
                    , bmpPaint);
            ...
            using (SKPaint bmpPaint = new SKPaint())
            {
                ...
                using (var redcanvas = new SKCanvas(redbitmap))
                {
                    ...
                    redcanvas.DrawImage(_batteryFrameBitmap, 0, 0
#if GNH_MAUI
                    , SKSamplingOptions.Default
#endif
                    , bmpPaint);
            ...
            using (SKPaint bmpPaint = new SKPaint())
            {
                ...
                using (var redcanvas = new SKCanvas(redbitmap))
                {
                    ...
                    redcanvas.DrawImage(_diskBitmap, 0, 0
#if GNH_MAUI
                    , SKSamplingOptions.Default
#endif
                    , bmpPaint);
            ...
            using (SKPaint bmpPaint = new SKPaint())
            {
                ...
                using (var yellowcanvas = new SKCanvas(yellowbitmap))
                {
                    ...
                    yellowcanvas.DrawImage(_diskBitmap, 0, 0
#if GNH_MAUI
                    , SKSamplingOptions.Default
#endif
                    , bmpPaint);
...
//#endif
            ...
            {
                string suffixText = triedAndFailedCopying ? " after failing to copy the file to cache directory" : "";
                await DisplayMessageBox(page, "File Sharing Failure", "GnollHack could not share file \'" + sharePath + "\'" + suffixText + ": " + ex.Message, "OK");
                return false;
            }
            return true;
        }
        ...
        public static bool ReplayShouldCallFunction
        {
            get
            {
                lock (_replayLock)
                {
                    //!IsReplaySearching || (GoToTurn >= 0 && ReplayTurn >= GoToTurn - 1)
                    return !(_replayGotoTurn >= 0 || _replaySearchRegexString != null) || (_replayGotoTurn >= 0 && _replayTurn >= _replayGotoTurn - 1);
                }
            }
        }
        ...
        public static Page PageFromTopOfModalNavigationStack()
        {
            ...
            try
            {
                ...
                //topPage = (stack?.Count ?? 0) <= 0 ? null : stack[stack.Count - 1];
                topPage = Navigation?.ModalStack?.LastOrDefault(); /* Same using Linq */
            }
            catch (Exception ex)
            {
                Debug.WriteLine(ex);
