...
namespace GnollHackCommon
{
    ...
    public delegate void AddExtendedMenuCallback(int winid, int glyph, Int64 identifier, char accel, char groupaccel, int attributes, [MarshalAs(UnmanagedType.LPStr)] string text, byte presel, int color, int maxcount,
        UInt64 oid, UInt64 mid, char heading_groupaccel, char special_mark, ulong menuflags, byte dataflags, obj otmpdata, objclassdata otypdata);
    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    ...
    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    public delegate void OpenSpecialViewCallback(int viewtype, [MarshalAs(UnmanagedType.LPStr)] string text);
