...
//       - .gnollhackrc tile and font size settings
...
void NetHackQtMapWindow::paintEvent(QPaintEvent* event)
{
	...
	for (int j=garea.top(); j<=garea.bottom(); j++) {
        ...
        layers.glyph = g;
    	(void)mapglyph(layers, &och, &color, &special, i, j);
		ch = (uchar)och;
#ifdef TEXTCOLOR
...
void NetHackQtBind::qt_getlin_ex(int style, int attr, int color, const char *prompt, char *line, const char* placeholder, const char* linesuffix)
{
    ...
    NetHackQtStringRequestor requestor(keybuffer, promptbuf);
    if (!requestor.Get(line)) {
	line[0]=0;
    }
}
...
{
    NetHackQtDelay delay(intervals * (flags.animation_frame_interval_in_milliseconds > 0 ? flags.animation_frame_interval_in_milliseconds : ANIMATION_FRAME_INTERVAL) / 3);
    delay.wait();
}
...
struct window_procs Qt_procs = {
    ...
    NetHackQtBind::qt_delay_output_intervals,
