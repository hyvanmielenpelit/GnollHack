...
while polymorphed into a tentacled one, is considered eating
...
operating systems. On MS-DOS and Windows, it is \mbox{``defaults.gnh''} in
the same folder as \mbox{{\it nethack.exe\/}} or \mbox{{\it nethackW.exe\/}}.
On Unix, Linux and Mac OS X it is \mbox{``.gnollhackrc''} in the user's home
...
\verb@U@ & S\verb+_+under                   &	(under hulk)\\
...
only parameter.
%.pg
While it is not difficult for experienced users to edit the {\it defaults.gnh\/}
...
Archaeologists, Barbarians, Cavemen, Healers, Knights, Priests, Rogues, Samurai,
