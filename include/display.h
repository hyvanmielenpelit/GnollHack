...
#define mon_visible(mon) \
    (/* The hero can see the monster IF the monster                                    */ \
     (!is_invisible(mon) || See_invisible)      /*     1. is not invisible             */ \
        && !is_mon_undetected(mon)                    /*     2. not an undetected hider      */ \
        && !(is_mon_buried(mon) || u.uburied))        /* AND 3. neither you nor it is buried */
...
#define is_special_peaceful(mon) \
    ((is_mon_shk(mon)) || (is_mon_priest(mon)) || (is_mon_npc(mon)) || (is_mon_smith(mon)) || (is_mon_gd(mon)) || (mon)->wormno || ((mon)->data->geno & G_UNIQ) || (mon)->mnum == urole.ldrnum)
...
#define GLYPH_SIMPLE_DOODAD_OFF  (MAX_SPECIAL_EFFECTS + GLYPH_SPECIAL_EFFECT_OFF)
#define GLYPH_MIRRORABLE_DOODAD_OFF  (MAX_SIMPLE_DOODAD_TILES + GLYPH_SIMPLE_DOODAD_OFF)
#define GLYPH_CURSOR_OFF  (NUM_DOODAD_MIRRORINGS * MAX_MIRRORABLE_DOODAD_TILES + GLYPH_MIRRORABLE_DOODAD_OFF)
...
#define maybe_display_usteed(otherwise_self)                            \
    ((u.usteed && mon_visible(u.usteed)) == TRUE                                \
     ? (is_mon_female(u.usteed) == TRUE ? female_mon_to_glyph(u.usteed, rn2_on_display_rng) : mon_to_glyph(u.usteed, rn2_on_display_rng))  \
     : (otherwise_self))
