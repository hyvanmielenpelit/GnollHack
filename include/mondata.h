...
#define mon_has_bloodlust(mtmp) (has_bloodlust((mtmp)->data) || (get_flag(mtmp)->mon_bitflags, MON_BITFLAGS_HASBLOODLUST))
#define mon_disregards_own_health(mtmp) (does_disregard_own_health((mtmp)->data) || (get_flag(mtmp)->mon_bitflags, MON_BITFLAGS_DISREGARDS_OWN_HEALTH))
#define mon_disregards_enemy_strength(mtmp) (does_disregard_enemy_strength((mtmp)->data) || (get_flag(mtmp)->mon_bitflags, MON_BITFLAGS_DISREGARDS_ENEMY_STRENGTH))
#define is_protector(mtmp) ((get_flag(mtmp)->mon_bitflags, MON_BITFLAGS_ISPROTECTOR))
...
#define fades_upon_death(ptr) (((ptr)->mflags5 & M5_FADES_UPON_DEATH) != 0L)
#define mon_fades_upon_death(m) fades_upon_death((m)->data)
...
#define is_sleeping(mon) \
    (has_sleeping(mon) || (is_mon_sleeping(mon)))
...
#define is_peaceful(mon) \
    (is_charmed_or_controlled(mon) || (is_mon_peaceful(mon)))
...
#define is_fleeing(mon) \
    (is_fearful(mon) || (is_mon_fleeing(mon)))
...
#define mon_can_move(mon) \
    ((is_mon_canmove(mon)) && !is_sleeping(mon) && !is_paralyzed(mon))
